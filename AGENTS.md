...
- Shared code lives under `common-sdk`, which現在分成 `core`, `spring-mvc`, `open-api`, `spring-security`, `mysql`, `redis`, `kafka`, `test`, `observability` 等子模組。
...
- Targeted build: `mvnd -pl <module> -am verify` (e.g., `mvnd -pl common-sdk/open-api/interface -am verify`).
