...
public abstract class BaseMySqlTestContainer {
    ...
    @Container
    protected static final MySQLContainer<?> MYSQL = new MySQLContainer<>(DockerImageName.parse("mysql:8.4")) // 指定固定版本以避免 CI 漂移
...
//                () -> MYSQL.getJdbcUrl().replace("/test", "/" + db));
    ...
    private static void ensureContainersStarted() {
        Startables.deepStart(Stream.of(MYSQL).filter(container -> !container.isRunning())).join();
    }
