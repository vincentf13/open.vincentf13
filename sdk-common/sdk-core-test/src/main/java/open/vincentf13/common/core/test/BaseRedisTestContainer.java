...
public abstract class BaseRedisTestContainer {
    ...
    @Container
    public static final GenericContainer<?> REDIS = new GenericContainer<>(DockerImageName.parse("redis:7.2")) // 輕量快
    ...
    private static void ensureContainersStarted() {
        Startables.deepStart(Stream.of(REDIS).filter(container -> !container.isRunning())).join();
    }
