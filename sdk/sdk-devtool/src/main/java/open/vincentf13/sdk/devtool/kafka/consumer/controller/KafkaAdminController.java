...
public class KafkaAdminController {
   ...
   *
   * <p>此接口允許在服務運行時，將指定的 Topic Partition 重置到任意 Offset。 由於 Spring Kafka 容器限制，此操作會暫時停止並重啟相關的 Listener
   * Container。
   *
   * <p>參數詳細說明：
   *
   ...
   * </table>
   *
   * API 請求範本 (.http): ### 重置 Kafka Offset 範例 (指定 Offset) POST
   * http://localhost:8080/infra/kafka/seek Content-Type: application/json
   *
   * <p>{ "topic": "exchange.order", "partition": 0, "offset": 100 }
   *
   * <p>### 重置 Kafka Offset 範例 (回退一格) POST http://localhost:8080/infra/kafka/seek Content-Type:
   * application/json
   *
   * <p>{ "topic": "exchange.order", "partition": 0 }
   */
  @PostMapping("/seek")
  @PublicAPI
        ...
        while (true) {
          try {
            ...
            if ((e.getCause() instanceof org.apache.kafka.common.errors.UnknownMemberIdException
                    || e.getCause()
                        instanceof org.apache.kafka.common.errors.RebalanceInProgressException)
                && attempt < 10) {
              attempt++;
