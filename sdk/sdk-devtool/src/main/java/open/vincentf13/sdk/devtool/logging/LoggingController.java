...
public class LoggingController {
   ...
   * </table>
   *
   * API 請求範本 (.http): ### 切換 Kafka/MVC/Feign/MyBatis 全部為 DEBUG POST
   * http://localhost:8080/api/logging Content-Type: application/json
   *
   * <p>{ "kafka": true, "mvc": true, "feign": true, "mybatis": true }
   *
   * <p>### 僅切換 Kafka 與 MyBatis 為 INFO POST http://localhost:8080/api/logging Content-Type:
   * application/json
   *
   * <p>{ "kafka": false, "mybatis": false }
   */
  @PostMapping
  @PublicAPI
  public OpenApiResponse<Map<String, String>> updateLogging(
      @RequestBody LoggingToggleRequest request) {
  ...
  private void applyLevel(String loggerName, Boolean enabled, Map<String, String> updated) {
    if (enabled == null) {
      return;
    }
  ...
  private void applyMybatisLevel(Boolean enabled, Map<String, String> updated) {
    if (enabled == null) {
      return;
    }
    LogLevel level = enabled ? LogLevel.DEBUG : LogLevel.INFO;
  ...
  public record LoggingToggleRequest(Boolean kafka, Boolean mvc, Boolean feign, Boolean mybatis) {}
}
