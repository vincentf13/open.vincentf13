...
public class CorsConfig {
    ...
    public CorsWebFilter corsWebFilter() {
        ...
        config.setExposedHeaders(Arrays.asList(OpenConstant.Header.REQUEST_ID.value(), OpenConstant.Header.TRACE_ID.value(), "Content-Disposition")); // 回傳 Access-Control-Expose-Headers。允許前端 JS 讀取這些回應標頭，例如取得檔名（Content-Disposition）或追蹤 ID。 "*" 在帶憑證時無效。任何網域都讀不到非 safelisted 的回應標頭，必須改成逐一列出要暴露的標頭。
