...
public class AnnotationBasedAuthorizationManager
  ...
  public AuthorizationDecision check(
    ...
    if (handlerMethod == null) {
      return new AuthorizationDecision(
          isAuthenticated(authenticationSupplier.get())); // 找不到處理器時退回一般認證邏輯
    }
    ...
    boolean requireAll =
        requirements.stream().anyMatch(AuthRequirement::requireAll); // 判斷是否需同時滿足全部模式
  ...
  private List<AuthRequirement> resolveRequirements(HandlerMethod handlerMethod) {
    ...
    merged.addAll(AnnotatedElementUtils.findAllMergedAnnotations(method, AuthRequirement.class));
    merged.addAll(
  ...
  private EnumSet<AuthType> resolveAvailableAuthTypes(
  ...
  private void collectInterfaceRequirements(
      Class<?> type, Method method, LinkedHashSet<AuthRequirement> target, Set<Class<?>> visited) {
    ...
    for (Class<?> iface : type.getInterfaces()) {
      ...
      if (interfaceMethod != null) {
        target.addAll(
            AnnotatedElementUtils.findAllMergedAnnotations(interfaceMethod, AuthRequirement.class));
      }
      target.addAll(AnnotatedElementUtils.findAllMergedAnnotations(iface, AuthRequirement.class));
  ...
  private void collectSuperclassMethodRequirements(
      Method method, LinkedHashSet<AuthRequirement> target) {
    ...
    while (declaringClass != null && declaringClass != Object.class) {
      ...
      if (superMethod != null) {
        target.addAll(
            AnnotatedElementUtils.findAllMergedAnnotations(superMethod, AuthRequirement.class));
      }
      target.addAll(
          AnnotatedElementUtils.findAllMergedAnnotations(declaringClass, AuthRequirement.class));
