...
public final class OpenMySqlTestContainer {
  ...
  private static Connection createAdminConnection() {
    try {
      ...
      Connection connection =
          DriverManager.getConnection(MYSQL.getJdbcUrl(), rootUser(), rootPassword());
      connection.setAutoCommit(true);
      return connection;
    } catch (SQLException ex) {
      throw new IllegalStateException(
          "Failed to obtain admin connection for MySQL test container", ex);
    }
  }
  ...
  private static Connection realDatabaseConnection() throws SQLException {
    ...
    if (isBlank(url)) {
      throw new IllegalStateException(
          "spring.datasource.url is required when MySQL testcontainer is disabled");
    }
  ...
  private static final class ToggleableMySqlContainer
      extends MySQLContainer<ToggleableMySqlContainer> {
