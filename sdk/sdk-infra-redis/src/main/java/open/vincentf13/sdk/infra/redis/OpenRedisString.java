...
/**
 * 封裝 RedisTemplate 與 StringRedisTemplate 的通用服務 - 同槽位檢查以保證 Cluster 下 Pipeline/MULTI/Lua 的可行性 -
 * 單語句優先原子操作；多步操作提供 Lua 與 Pipeline 版本 - 提供 scan 全主節點遍歷（Cluster）與 cache-aside 模式
 */
public final class OpenRedisString {
  ...
  public static <T> T getOrLoad(String key, Duration ttl, Class<T> type, Supplier<T> loader) {
    ...
    synchronized (RedisUtil.intern(key)) { // 簡易擊穿保護：單機級別
      ...
      if (val == null) {
        // 穿透保護：空值短 TTL
        setAsyncFireAndForget(
            key,
            "",
            withJitter(Duration.ofSeconds(Math.max(30, Math.min(ttl.toSeconds() / 10, 300)))));
      } else {
   ...
   */
  public static CompletableFuture<Boolean> setAsync(
      String key, Object value, Duration ttl, int retry) {
  ...
  public static void setBatch(Collection<KeyValueTtl> entries) {
    ...
    StringRedisTemplate stringRedis = stringRedisTemplate();
    SessionCallback<Object> callback =
        new SessionCallback<>() {
          @Override
                ...
                .forEach(
                    entry -> {
                      Duration ttlWithJitter =
                          withJitter(Objects.requireNonNull(entry.ttl(), "ttl is required"));
                      ops.opsForValue()
                          .set(
                              Objects.requireNonNull(entry.key(), "key is required"),
                              OpenObjectMapper.toJson(entry.value()),
                              ttlWithJitter);
                    });
            return null;
          }
    ...
    entries.stream()
        ...
        .forEach(
            entry -> {
              ...
              grouped
                  .computeIfAbsent(RedisClusterUtil.slot(key), k -> new ArrayList<>())
                  .add(entry);
            });
    grouped
        .values()
   ...
   *   );
   */
  public static Map<String, Object> getBatchCluster(
      Collection<Map<String, Class<?>>> keyTypeMappings) {
  ...
  private static Map<String, Class<?>> flattenKeyTypeMappings(
      Collection<Map<String, Class<?>>> keyTypeMappings) {
