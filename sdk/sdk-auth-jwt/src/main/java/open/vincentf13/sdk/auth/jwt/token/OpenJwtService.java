...
public class OpenJwtService {
    ...
    public GenerateTokenInfo generateAccessToken(String sessionId,
        ...
        String tokenValue =
                encoder.encode(JwtEncoderParameters.from(headers, builder.build())).getTokenValue();
        OpenLog.debug(
                log,
                JwtEvent.JWT_ACCESS_ISSUED,
                "subject",
                ...
                email,
                "sessionId",
                sessionId == null ? "<legacy>" : sessionId);
        return new GenerateTokenInfo(tokenValue, issuedAt, expiresAt, TokenType.ACCESS, sessionId);
    }
    
    public GenerateTokenInfo generateRefreshToken(String sessionId,
                                                  String subject) {
        JwtEncoder encoder = requireEncoder();
        Instant issuedAt = Instant.now();
    ...
    public Optional<JwtToken> parseAccessToken(String tokenValue) {
        try {
            ...
            if (tokenType != TokenType.ACCESS) {
                OpenLog.warn(
                        log, JwtEvent.JWT_INVALID_TYPE, "expected", TokenType.ACCESS, "actual", tokenType);
            ...
            JwtToken authentication =
                    new JwtToken(user, tokenValue, granted, sessionId, jwt.getIssuedAt(), jwt.getExpiresAt());
            return Optional.of(authentication);
        } catch (JwtException ex) {
            OpenLog.warn(log, JwtEvent.JWT_INVALID, ex, "reason", ex.getMessage());
            return Optional.empty();
        }
    }
    ...
    public Optional<RefreshToken> parseRefreshToken(String tokenValue) {
        try {
            ...
            if (tokenType != TokenType.REFRESH) {
                OpenLog.warn(
                        log, JwtEvent.JWT_INVALID_TYPE, "expected", TokenType.REFRESH, "actual", tokenType);
                return Optional.empty();
            }
            String sessionId = jwt.getClaimAsString(SESSION_ID_CLAIM);
            return Optional.of(
    ...
    private TokenType resolveTokenType(Jwt jwt) {
        ...
        try {
            ...
            OpenLog.warn(
                    log,
                    JwtEvent.JWT_UNKNOWN_TYPE,
                    ex,
                    "value",
                    raw,
                    "tokenId",
                    jwt.getId() == null ? UUID.randomUUID() : jwt.getId());
            return TokenType.ACCESS;
        }
    }
    ...
    public record GenerateTokenInfo(
            String token, Instant issuedAt, Instant expiresAt, TokenType tokenType, String sessionId) {
    }
}
