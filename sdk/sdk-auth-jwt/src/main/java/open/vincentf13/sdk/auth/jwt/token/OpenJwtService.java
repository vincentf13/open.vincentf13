...
public class OpenJwtService {
    ...
    public OpenJwtService(JwtProperties properties,
    ...
    public GenerateTokenInfo generateRefreshToken(String sessionId,
        ...
        JwsHeader headers = JwsHeader.with(MacAlgorithm.HS256).build();
        String tokenValue = encoder.encode(JwtEncoderParameters.from(headers, builder.build())).getTokenValue();
        OpenLog.debug(JwtEvent.JWT_REFRESH_ISSUED,
                      "subject", subject,
                      "sessionId", sessionId == null ? "<legacy>" : sessionId);
    ...
    private TokenType resolveTokenType(Jwt jwt) {
        ...
        try {
            ...
            OpenLog.warn(JwtEvent.JWT_UNKNOWN_TYPE, ex, "value", raw, "tokenId", jwt.getId() == null ? UUID.randomUUID() : jwt.getId());
            return TokenType.ACCESS;
        }
    }
    ...
    public record GenerateTokenInfo(String token, Instant issuedAt, Instant expiresAt, TokenType tokenType,
                                    String sessionId) {
    }
}
