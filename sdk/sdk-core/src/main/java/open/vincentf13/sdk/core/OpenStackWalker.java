...
/**
 Utility helpers around {@link StackWalker} to surface caller information without exposing JDK
 specifics.
 */
public final class OpenStackWalker {
    ...
    /**
     ...
     @param framesToSkip number of frames to drop starting from the current method (negative values
     treated as zero)
    ...
    /**
     Resolve operation name by preferring the implementation referenced by the provided Supplier
     (lambda/method reference). Falls back to stack walking when lambda inspection is not possible.
     */
    public static String resolveOperation(Supplier<?> supplier,
                                          int fallbackSkip) {
