...
public final class Metrics {
  ...
  private static final ConcurrentHashMap<String, LongTaskTimer> CACH_LTT =
  ...
  //   public static void bindKafkaMetrics(org.apache.kafka.clients.producer.Producer<?, ?> p,
  // String
  // clientId) {
  //       new KafkaClientMetrics(p).bindTo(REG);
  //   }
  //
  //   public static void bindKafkaMetrics(org.apache.kafka.clients.consumer.Consumer<?, ?> c,
  // String
  // clientId) {
  ...
  private static Counter counter(String name, String... tags) {
    ...
    return CACH_COUNTER.computeIfAbsent(key, k -> Counter.builder(name).tags(tags).register(REG));
  }

  private static Timer timer(String name, String... tags) {
    String key = key(name, tags);
    return CACH_TIMER.computeIfAbsent(
        key,
        k ->
            Timer.builder(name)
                .tags(tags)
  ...
  private static LongTaskTimer ltt(String name, String... tags) {
    String key = key(name, tags);
    return CACH_LTT.computeIfAbsent(key, k -> LongTaskTimer.builder(name).tags(tags).register(REG));
  }
