...
public class ObjectMapperConfig {
    ...
    private static void customize(ObjectMapper mapper) {
        mapper.configure(DeserializationFeature.FAIL_ON_UNKNOWN_PROPERTIES, false);     // 反序列化忽略未知欄位，防禦外部欄位變動
        ...
        mapper.setPropertyNamingStrategy(PropertyNamingStrategies.LOWER_CAMEL_CASE);    // 屬性使用駝峰名稱，與後端一致方便維護
        ...
        // BigDecimal
        mapper.configure(JsonGenerator.Feature.WRITE_BIGDECIMAL_AS_PLAIN, true);       // 控制 BigDecimal 序列化時的輸出格式，避免以科學記號（Scientific Notation）表示金額或精確數值。
        mapper.enable(DeserializationFeature.USE_BIG_DECIMAL_FOR_FLOATS);              // 用途：解析浮點數時使用 BigDecimal 取代 Double。特別適合金融、會計、計量單位等需高精度的場景。
        
        // 效能
        // mapper.registerModule(new AfterburnerModule());                             // 可選效能模組，在 JDK 17+ 收益有限且偶有相容性議題，除非量測證實再開。
        
        // 安全性
        mapper.enable(JsonParser.Feature.STRICT_DUPLICATE_DETECTION);                 // 用途：偵測 JSON 物件中重複欄位名稱，若出現相同 key 會拋出例外。能防止潛在的資料覆蓋或攻擊手法（例如 JSON 混淆攻擊）。
        mapper.enable(DeserializationFeature.FAIL_ON_READING_DUP_TREE_KEY);           // 用途：和 STRICT_DUPLICATE_DETECTION 類似，但作用於「讀取樹狀節點 (JsonNode)」階段。若同一層節點有重複 key，拋出錯誤。 確保 readTree() 也具備防重能力。
        mapper.enable(DeserializationFeature.FAIL_ON_TRAILING_TOKENS);                // 反序列化完成後若 JSON 中仍有未處理的多餘內容，直接報錯。啟用後解析時會拋出例外，避免資料格式不乾淨或混入多筆 JSON。
