...
public final class OpenObjectMapper {
    ...
    /**
     JSON bytes → 物件。
     */
    public static <T> T fromBytes(byte[] jsonBytes,
                                  Class<T> type) {
        if (jsonBytes == null)
            return null;
        try {
     ...
     */
    public static <S, T> List<T> convertList(List<S> source,
                                             Class<T> targetType) {
        if (source == null || source.isEmpty()) {
            return List.of();
        }
        return source.stream().map(item -> convert(item, targetType)).filter(Objects::nonNull).toList();
    }
    
    /**
     合併 JSON 到既有物件（部分更新）。
     */
    public static <T> T update(T target,
                               String patchJson) {
        if (target == null || patchJson == null)
            return target;
        try {
