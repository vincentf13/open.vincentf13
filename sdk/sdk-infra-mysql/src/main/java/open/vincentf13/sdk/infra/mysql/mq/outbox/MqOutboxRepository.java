...
public class MqOutboxRepository {
  ...
  public <T> void appendWithSeq(
      String topic, Long key, T message, Map<String, Object> headers, Long seq) {
    Assert.hasText(topic, "topic must not be blank");
    Assert.notNull(key, "key must not be null");
    Assert.notNull(message, "message must not be null");
    OpenValidator.validateOrThrow(message);
  ...
  public boolean exists(String topic, Long key) {
    Assert.hasText(topic, "topic must not be blank");
    Assert.notNull(key, "key must not be null");
    MqOutboxPO criteria = MqOutboxPO.builder().aggregateType(topic).aggregateId(key).build();
