...
事件驅動流程與原 `/api/risk/pre-check` 試算邏輯一致，只是觸發點改為 Kafka：
...
4. **驗證槓桿與試算預扣保證金**：
   - 取 `requested_leverage = max(existing_leverage, clientLeverage)`，若超出 `max_leverage` 立即回覆 `MarginPreCheckFailed`。
   - `required_margin = max(order_notional / requested_leverage, order_notional * initial_margin_rate)`。
5. **檢查可用資金**：透過 `account-ledger` 或內部錢包快照取得 `margin_balance`，計算 `available_margin = margin_balance - used_margin`；不足時回傳失敗並附上缺口。
6. **模擬下單後指標**：
