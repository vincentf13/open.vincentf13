...
2. **啟動初始化**：服務啟動時，針對常用週期（1m/5m/1h/1d）為每組 instrument + period 從 `kline_buckets` 讀取「涵蓋當前時間（`bucket_start ≤ now < bucket_end`）」的 bucket 填入記憶體；若該區間尚無紀錄，則以最新成交價或 0 值建立預設 bucket，避免首次查詢為空。
...
7. **事件＆推播**：寫入成功後發布 `KlineClosed` 事件供報表/下游重算，同時透過 `/ws/market/kline` 推播最新 K 線 (暫標TODO 不做)；常用週期（1m/5m/1h/1d）亦將最近 N 根緩存在 Redis/LRU 供查詢。

> 以上流程讓每個週期都有一致的 OHLCV，支援延遲補件並以事件 + WebSocket 雙軌輸出。
