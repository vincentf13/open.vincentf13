...
- 指數價（`index_price`）
    - 依據多個外部現貨交易所的最新成交或加權中間價，使用固定權重或動態權重平均：
      ...
      $index\_price = \sum_i weight_i * external\_price_i$
...
- 標記價（`mark_price`）
    - 以指數價為基礎，調整資金費率與現貨/期貨價差形成的溢價指數：
      ...
      $premium\_index = clamp\left(\frac{fair\_price - index\_price}{index\_price},-0.05\%,0.05\%\right)$ 
      $funding\_basis = clamp(interest\_rate\_diff,\ -0.05\%,\ 0.05\%)$ 
      $mark\_price = index\_price * \left(1 + clamp(premium\_index + funding\_basis, -0.5\%, 0.5\%)\right)$
