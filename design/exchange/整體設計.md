...
sequenceDiagram
    ...
    Note over Matching: {instrumentId, tradeId, orderId(maker), counterpartyOrderId(taker), price, quantity, makerUserId, takerUserId, makerFee, takerFee, executedAt}
...
|     | 撮合成交產生成交明細 | matching.trade-executed    | `TradeExecuted`    | instrumentId, tradeId, orderId(maker), counterpartyOrderId(taker), price, quantity, makerUserId, takerUserId, makerFee, takerFee, executedAt | account-ledger, positions, market-data |
...
| **`TradeExecuted`**    | 兩筆訂單撮合成交     | `instrumentId`, `tradeId`, `orderId`(maker), `counterpartyOrderId`(taker), `price`, `quantity`, `makerUserId`, `takerUserId`, `makerFee`, `takerFee`, `executedAt` | `Market-Data`, `Account-Ledger`, `Positions` |
...
* Positions 服務的處理:
  ...
  - 若是平倉，就把對應的保證金 + PnL 金額透過 deltaAvailable/deltaReserved 反映在事件裡。
    Positions 只要以 referenceId=tradeId 找出先前鎖定的關倉量（或直接用事件金額折算），即可知道實際平掉多少。
	  - TradeExecuted 事件本身帶 instrumentId / tradeId / orderId(maker) / counterpartyOrderId(taker) / price / quantity / makerUserId / takerUserId / makerFee / takerFee / executedAt …。Positions 消費這個事件並在本地 position_events（或任何你做的投影表）寫下一筆記錄。 這樣就有 tradeId -> 某倉位、某筆預約 的映射。
