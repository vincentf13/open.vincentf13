...
1. 必須先有倉位，才能調整槓桿
2. Position 服務將目標槓桿、用戶/商品（userId + instrumentId）、倉位數量以及最新 `margin`、`markPrice` 一併帶入 `LeveragePrecheckRequest`。
3. `risk-margin` 依照 `risk_limits` 重新計算：
	...
	- 以上二式，代表調整槓桿後不得低於初始保證金率與維持保證金率
4. 若任一步驟不符，回傳 `allow=false` 並附缺口／建議槓桿；通過則 `allow=true`，Position 得以更新 `positions.leverage`。
