...
---
### leverage 與 margin、margin_ratio、equity 是如何得到的?
...
## equity
• equity = 已實現盈虧 + 未實現盈虧 + 餘額。
簡化後會把已實現盈虧視為已反映在 margin_balance 裡，因此實作常用 equity = margin_balance + unrealized_pnl。
  - margin_balance 來源是 account-ledger 的「衍生品保證金帳戶」餘額：
      1. 下單或成交後，MarginPreCheckPassed 事件會指示 ledger 凍結或解凍資金；
         ledger 底層持續維護 available_balance / frozen_balance / total_balance。
  ...
  - unrealized_pnl 由 positions 模組計算並帶在 PositionUpdated 事件裡（同檔 2013 說明）。風控服務消費事件後，直接使用該
