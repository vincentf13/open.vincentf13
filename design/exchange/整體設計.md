...
│  ├─ sdk-auth/                         # 共用 Spring Security 基本防護，驗證與授權由 sdk-auth-jwt 與 sdk-auth-server 模塊負責
...
## user
- `POST /api/user`：註冊
- `GET /api/user/me`：查詢主檔基本資料。

## auth

- `POST /api/auth-credential`：建立憑證
- 下述 REST 端點由 `sdk-auth-server` 提供預設實作；服務引入該模組後即可直接啟用。
	- `POST /api/login`：驗證憑證 → 簽發 Access/Refresh Token，同時在 Redis 建立 session（`sessionId`、`userId`、到期時間等）。
	- `POST /api/token/refresh`：使用 Refresh Token 取得新的 Access Token，並延長 Redis session 到期時間。
	- `POST /api/logout`：作廢 Refresh Token/Session（刪除 Redis session）。
...
- **客戶端生成**：由 `sdk-contract` 底下的 `exchange-*-sdk` 自動產生 Feign/Web 客戶端，避免手寫 SDK 帶來維護成本。
