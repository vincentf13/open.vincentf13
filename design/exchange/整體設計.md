...
4. 當倉位價值 = 0 時，代表首次開倉前配置的槓桿，回傳通過 `allow=true`
5. positions.margin 被視為衍生欄位：當真正發生成交、倉位數量／均價改變時，Position 服務才會依 margin = abs(entry_price / quantity) / leverage 重新計算並入庫，同步更新 margin_ratio（設計說明 design/exchange/整體設計.md:3025-3033）。單純    修改槓桿並不會驅動這個流程，因此不會自動調整 margin。
...
7. 試算預扣保證金    `required_margin = max(order_notional / requested_leverage, order_notional * initial_margin_rate)`。
	- 查詢 `account` 服務接口 GET `/api/ledger/balances?userId=`，判斷保證金餘額是否足夠，若不足則拒絕

8. 試算成交後指標**：
	- 最新倉位成本價： `simulated_notional = current_notional + order_notional`。
	- `simulated_margin_ratio = required_margin / simulated_notional`；若低於 `maintenance_margin_rate + buffer` 則拒絕。
