...
CREATE TABLE auth_credentials (
    id                  BIGINT          NOT NULL COMMENT '憑證ID (Snowflake)',
    user_id             BIGINT          NOT NULL COMMENT '用戶ID (外鍵關聯 users.id)',
    credential_type     VARCHAR(20)     NOT NULL COMMENT '憑證類型: PASSWORD, API_KEY, TOTP, FIDO',
    secret_hash         VARCHAR(255)    NOT NULL COMMENT '憑證密文 (BCrypt/Argon2 雜湊)',
    salt                VARCHAR(64)     NULL     COMMENT '加密鹽值 (部分演算法需要)',
    status              VARCHAR(20)     NOT NULL DEFAULT 'ACTIVE' COMMENT '憑證狀態: ACTIVE, LOCKED, EXPIRED, REVOKED',
    expires_at          DATETIME        NULL     COMMENT '過期時間 (NULL表示永不過期)',
    failed_attempts     INT             NOT NULL DEFAULT 0 COMMENT '連續失敗次數 (用於自動鎖定)',
    ...
    PRIMARY KEY (id),
    UNIQUE KEY uk_user_type (user_id, credential_type) COMMENT '同一用戶同類型憑證唯一'
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='登入憑證表 - 支援多種認證方式';
...
```sql
CREATE TABLE users (
    id              BIGINT          NOT NULL COMMENT '用戶ID (Snowflake)',
    external_id     VARCHAR(64)     NOT NULL COMMENT '外部顯示ID (UUID)',
    email           VARCHAR(255)    NOT NULL COMMENT '電子郵箱 (唯一)',
    status          VARCHAR(20)     NOT NULL DEFAULT 'ACTIVE' COMMENT '帳號狀態: ACTIVE, SUSPENDED, CLOSED',
    kyc_tier        INT             NOT NULL DEFAULT 0 COMMENT 'KYC等級: 0=未驗證, 1=基礎, 2=進階',
    created_at      DATETIME        NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '註冊時間',
    updated_at      DATETIME        NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新時間',
    PRIMARY KEY (id),
    UNIQUE KEY uk_email (email) COMMENT '郵箱唯一',
    UNIQUE KEY uk_external_id (external_id) COMMENT '外部ID唯一'
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='用戶主檔表';
...
#### `auth_credentials_prepare` - 註冊補償表

```sql
CREATE TABLE user_registration_prepare (
    id              BIGINT          NOT NULL AUTO_INCREMENT COMMENT '記錄ID',
    user_id         BIGINT          NOT NULL COMMENT '用戶ID',
    email           VARCHAR(255)    NOT NULL COMMENT '郵箱',
    password_hash   VARCHAR(255)    NOT NULL COMMENT '密碼雜湊 (待寫入auth服務)',
    salt            VARCHAR(64)     NULL     COMMENT '鹽值',
    status          VARCHAR(20)     NOT NULL DEFAULT 'PREPARE' COMMENT '狀態: PREPARE, COMPLETED, FAILED',
    retry_count     INT             NOT NULL DEFAULT 0 COMMENT '重試次數',
    last_retry_at   DATETIME        NULL     COMMENT '最後重試時間',
    error_message   TEXT            NULL     COMMENT '失敗原因',
    created_at      DATETIME        NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '創建時間',
    completed_at    DATETIME        NULL     COMMENT '完成時間',
    PRIMARY KEY (id),
    UNIQUE KEY uk_user_id (user_id) COMMENT '每個用戶只能有一筆PREPARE記錄'
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='用戶註冊補償表 - Saga模式';
