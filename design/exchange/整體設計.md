...
* Positions 服務的處理:
  ...
  - 若是平倉，就把對應的保證金 + PnL 金額透過 deltaAvailable/deltaReserved 反映在事件裡。
    Positions 只要以 referenceId=tradeId 找出先前鎖定的關倉量（或直接用事件金額折算），即可知道實際平掉多少。
	  - TradeExecuted 事件本身帶 tradeId / buyOrderId / sellOrderId / price / quantity …。Positions 消費這個事件並在本地 position_events（或任何你做的投影表）寫下一筆記錄。 這樣就有 tradeId -> 某倉位、某筆預約 的映射。
...
8. `simulated_equity` = `倉位的` `margin` `+` `unrealized_pnl` ，加上 `required_margin`
9. 試算成交後指標：
...
#### 查詢開/平倉
account-ledger 消費 TradeExecuted 時，要先根據 orderId + userId + instrumentId 去查之前保存的 intent（或再次查詢 Position 服務當前倉位），再決定應套用的分錄情境：
...
## `GET /api/market/mark-price/{instrumentId}`
