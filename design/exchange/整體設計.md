...
2. **鎖定風控基準**：查詢 `risk_limits`、`risk_snapshots` 與 `positions`（必要時 `FOR UPDATE`）以取得當前倉位狀態與佔用保證金。
...
1. 試算預扣保證金    `required_margin = max(order_notional / requested_leverage, order_notional * initial_margin_rate)`。
2. **檢查可用資金**：透過 `account-ledger` 或內部錢包快照取得 `margin_balance`，計算 `available_margin = margin_balance - used_margin`；不足時回傳失敗並附上缺口。
3. **模擬下單後指標**：
