...
|事件驅動|消費 `TradeExecuted`、`LedgerEntryCreated` 等事件來更新倉位|
...
##### 倉位更新後
- 計算未實現盈虧：`unrealized_pnl = (mark_price - entry_price) * qty * contract_size`
- 計算強平價（根據槓桿與保證金率）。
...
```
unrealized_pnl = (mark_price - entry_price) * position_qty
```
...
Risk-Margin 消費事件
   ↓
更新該用戶的實際倉位資訊（從 Positions）
   ↓
重新計算風險指標
   ↓
若達風險閾值 → 觸發 Liquidation 檢查與事件
```
...
#### 1. 接收成交事件 `TradeExecuted`

Kafka Consumer 收到 `TradeExecuted` 後執行：
- 根據 `instrument_id` 查找對應的 `risk_limits`（初始 / 維持保證金率、最大槓桿）。
- 查出成交雙方（buyer/seller）的當前倉位與帳戶資產。
- 計算此次成交導致的**保證金佔用變化**與**槓桿變動**。
...
|**margin_ratio**|`(equity / notional)`|維持比率|
...
- 由強平任務（worker）下發「平倉委託」或「直接撮合」以關閉倉位。
    

---

### 關鍵特性
- **風險指標來源**：positions + ledger + market-data。
- **mark_price 驅動**：所有風控以標記價為準。
- **事件驅動更新**：不輪詢，而是消費事件或接收推送。
...
#### 標記價與指數價 (Mark Price / Index Price)
- 若交易對對應外部現貨市場：
    - `index_price` = 外部現貨加權價（取自外部交易所 API）
    - `mark_price` = index_price ± premium_adjustment
- 結果推送給：
    - `positions`（重新計算浮動盈虧）
    - `risk-margin`（更新強平價與風險率）
