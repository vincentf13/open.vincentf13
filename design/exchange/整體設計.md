...
#### reference_type
- `TRADE`：撮合的成交明細，指向 `trade_id`。用於處理部分成交與最終成交的冪等。
- `CANCEL_REQUEST`：撤單請求流程的業務標識，如 `cancel_request_id`。
- `RISK_CHECK`：風控前置校驗請求，指向風控服務生成的 `risk_request_id` 或評估單號。
- `LEDGER_FREEZE`：資金凍結請求，指向帳本側的凍結請求 ID 或先行建立的預留 ID。
- `LEDGER_UNFREEZE`：撤單或失敗後解凍請求的 ID。
- `LEDGER_SETTLEMENT`：成交結算入帳對應的分錄 ID（或成對分錄的聚合 ID）。帳本側原生就有 `reference_type/reference_id` 與分錄主鍵，可互相對齊。
- `LIQUIDATION`：強平結算的關聯 ID（來自清算/強平子流程）。
- `FUNDING_FEE`：資金費率結算批次或明細 ID。
- `INSURANCE_FUND`：保險基金注入或爪回的事件 ID。
...
# 對照表（示例）
- `RISK_CHECK_PASSED/FAILED` → `reference_type=RISK_CHECK`，`reference_id=risk_request_id`。
- `FUNDS_FROZEN/FUNDS_FROZEN_FAILED` → `reference_type=LEDGER_FREEZE`，`reference_id=freeze_request_id`（成功時還可在 `payload` 填入分錄或預留額度的標識）。
- `ORDER_PARTIALLY_FILLED/ORDER_FILLED` → `reference_type=TRADE`，`reference_id=trade_id`。
- 結算入帳 → `reference_type=LEDGER_SETTLEMENT`，`reference_id=entry_id` 或結算批次 ID。
- 強平 → `reference_type=LIQUIDATION`；資金費率 → `reference_type=FUNDING_FEE`；保險基金 → `reference_type=INSURANCE_FUND`。
    

# 結論
- 擴充 `reference_type` 以覆蓋帳本、強平、資金費率與保險基金等場景。
- 失敗事件同樣填寫 `reference_id`，使用該次請求的全域唯一 ID。
- 如需 UUID，相容性考量可放寬 `reference_id` 型別。
- 你現有索引與唯一鍵設計已支援以上策略，僅需補齊枚舉與生成規範。
...
CREATE TABLE trade_tickers (
    ...
    side                    VARCHAR(10)     NOT NULL COMMENT '成交方向 (以Taker為準): BUY, SELL',
...
### ledger_entries 表 - 雙分錄紀錄

```sql
CREATE TABLE ledger_entries (
    entry_id            BIGINT          NOT NULL COMMENT '分錄ID (Snowflake)',
    account_id          BIGINT          NOT NULL COMMENT '帳戶ID (用戶錢包/保證金帳戶)',
    user_id             BIGINT          NOT NULL COMMENT '用戶ID (冗餘欄位便於查詢)',
    asset               VARCHAR(20)     NOT NULL COMMENT '資產類型: USDT, BTC, ETH等',
    amount              DECIMAL(20,8)   NOT NULL COMMENT '金額 (正數為增加, 負數為減少)',
    direction           VARCHAR(10)     NOT NULL COMMENT '方向: DEBIT(借), CREDIT(貸)',
    balance_after       DECIMAL(20,8)   NOT NULL COMMENT '本次變動後的餘額',
    entry_type          VARCHAR(50)     NOT NULL COMMENT '分錄類型: TRADE_SETTLEMENT, FEE, FUNDING_RATE, DEPOSIT, WITHDRAWAL, LIQUIDATION, TRANSFER',
    reference_type      VARCHAR(50)     NOT NULL COMMENT '關聯類型: TRADE, ORDER, DEPOSIT, WITHDRAWAL, FUNDING',
    reference_id        BIGINT          NOT NULL COMMENT '關聯ID (trade_id, order_id等)',
    counterparty_entry  BIGINT          NULL     COMMENT '對手分錄ID (雙分錄配對)',
    description         VARCHAR(255)    NULL     COMMENT '描述說明',
    metadata            JSON            NULL     COMMENT '額外元數據',
    event_time          DATETIME        NOT NULL COMMENT '業務事件時間',
    created_at          DATETIME        NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '記錄創建時間',
    PRIMARY KEY (entry_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='雙分錄紀錄表 - 借貸必須平衡供審計';

CREATE INDEX idx_account_asset ON ledger_entries(account_id, asset, event_time DESC) COMMENT '查詢帳戶資產變動歷史';
CREATE INDEX idx_user_asset ON ledger_entries(user_id, asset, event_time DESC) COMMENT '查詢用戶資產變動';
CREATE INDEX idx_reference ON ledger_entries(reference_type, reference_id) COMMENT '通過業務ID查找分錄';
CREATE INDEX idx_entry_type ON ledger_entries(entry_type, event_time DESC) COMMENT '按分錄類型查詢';
CREATE INDEX idx_counterparty ON ledger_entries(counterparty_entry) COMMENT '查找配對分錄';
CREATE INDEX idx_event_time ON ledger_entries(event_time DESC) COMMENT '時序查詢所有分錄';
```
...
CREATE TABLE position_events (
    ...
    user_id             BIGINT          NOT NULL COMMENT '用戶ID (冗餘欄位便於查詢)',
    instrument_id       BIGINT          NOT NULL COMMENT '交易對ID (冗餘欄位便於查詢)',
    event_type          VARCHAR(50)     NOT NULL COMMENT '事件類型: POSITION_OPENED, POSITION_INCREASED, POSITION_DECREASED, POSITION_CLOSED, LIQUIDATION_TRIGGERED, MARK_PRICE_UPDATED',
...
CREATE TABLE risk_limits (
    ...
    position_size_max           DECIMAL(20,8)   NULL     COMMENT '倉位規模上限 (USDT, NULL表示無上限)',
    ...
    max_position_value          DECIMAL(20,8)   NULL     COMMENT '單一倉位最大名義價值限制',
...
CREATE TABLE market_snapshots (
    ...
    volume_24h          DECIMAL(20,8)   NOT NULL DEFAULT 0 COMMENT '24小時成交量',
    turnover_24h        DECIMAL(20,8)   NOT NULL DEFAULT 0 COMMENT '24小時成交額',
