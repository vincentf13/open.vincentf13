...
| 發布場景 | Topic | 事件名稱 | 事件內容 | 消費者 |
|----------|-------|----------|----------|--------|
| 平倉訂單需預先鎖倉 | positions.reserve-requested | `PositionReserveRequested` | orderId, userId, instrumentId, side, intent, quantity | positions |
...
在「事件類型 (event_type)」表格中，凡由事件驅動的狀態變化，event_type 一律使用事件名稱（如 PositionReserved、FundsFrozen、
    TradeExecuted…）。
  - 由 REST/API 觸發的則 OrderApi.create、OrderCommandService.requestCancel 等「介面類別.方法」

| 事件類型                                            | Payload 內容                                 | Actor              |
| ----------------------------------------------- | ------------------------------------------ | ------------------ |
| OrderApi.create                               | {side,type,price,quantity,time_in_force} | USER:{userId}    |
| OrderEventPublisher.publishOrderSubmitted       | {order_id, status: SUBMITTED}              | ORDER_SERVICE    |
| PositionReserved                                | {position_effect}                          | POSITION_SERVICE |
| PositionReserveRejected                         | {reason}                                   | POSITION_SERVICE |
| MarginPreCheckFailed                            | {reason}                                   | RISK_SERVICE     |
| FundsFrozen                                     | {frozen_amount, asset}                     | LEDGER_SERVICE   |
| FundsFreezeFailed                               | {reason}                                   | LEDGER_SERVICE   |
| FundsUnfrozen                                   | {cancelled_at, unfrozen_amount}            | LEDGER_SERVICE   |
| TradeExecuted                                   | {price, quantity, fee}                     | MATCHING_ENGINE  |
| OrderCommandService.requestCancel               | {requested_at}                             | USER:{userId}    |
| OrderEventPublisher.publishOrderCancelRequested | {order_id, reason}                         | ORDER_SERVICE    |
#### reference_type
  - 若事件源自後端服務寫入的資料列，就用「服務名.表名.ref欄位，」當 reference_type
  - 若事件由 API 直接觸發（如 OrderApi.create、OrderCommandService.requestCancel），就用「介面類別.方法.ref欄位」當 reference_type，
    reference_id 填該請求的唯一識別（requestId 或 orderId/clientOrderId）。
...
1.  **快速響應**:
    ...
    * 若:
	...
	-  **Intent = REDUCE/CLOSE**：
		...
		* `Order` 服務將該訂單轉為 Position Saga，寫入 Outbox 發送 `positions.reserve-requested (PositionReserveRequested)`，內容包含 orderId/userId/instrumentId/side/intent/quantity。
...
| order | positions.reserve-requested | `PositionReserveRequested` | 平倉前凍結倉位 (userId, instrumentId, side, quantity, intentType) | `PositionReserved` / `PositionReserveRejected` | update `positions`<closing_reserved_quantity> | order-service | order-service | - |
...
| HTTP Method | Endpoint              | 說明      | 調用者   | 授權      | 輸入                                    | 輸出              | DB操作 | 緩存操作 | 服務調用 | 補償機制 |
| ----------- | --------------------- | ------- | ----- | ------- | ------------------------------------- | --------------- | ---- | ---- | ---- | ---- |
| POST        | `/api/risk/pre-check` | 下單前風險檢查 | order | private | userId, instrumentId, quantity, price | 是否允許下單、需凍結保證金金額 | -    | -    | -    | -    |

**事件消費**:

| 來源服務 | Topic | 事件 | 處理邏輯 | 事件發布 | DB操作 | 緩存操作 | 服務調用 | 補償機制 |
|----------|-------|------|----------|----------|--------|----------|----------|----------|
| order | order.submitted | `OrderSubmitted` | 根據委託資訊進行保證金試算 | `MarginPreCheckPassed` / `MarginPreCheckFailed` | - | - | - | - |
