...
註冊：[`POST /api/users`：註冊](#`POST%20/api/users`：註冊)
登入、refresh_token、登出：[[#POST `/api/auth/login`：登入]]
## 建立帳戶、入金
...
3. **行情**：`market-data` 
	1. 依 `TradeExecuted` 更新 Ticker、K 線；依 `OrderBookUpdated` 更新深度與最佳價
...
sequenceDiagram
    ...
    Note over Matching: {instrumentId, tradeId, buyOrderId, sellOrderId, price, quantity, fee, executedAt}
...
| 撮合成交產生成交明細 | matching.trade-executed    | `TradeExecuted`    | instrumentId, tradeId, buyOrderId, sellOrderId, price, quantity, buyerUserId, sellerUserId, fee, executedAt | account-ledger, positions, market-data |
...
| **`TradeExecuted`**    | 兩筆訂單撮合成交     | `instrumentId`, `tradeId`, `buyOrderId`, `sellOrderId`, `price`, `quantity`, `buyerUserId`, `sellerUserId`, `fee`, `executedAt` | `Market-Data`, `Account-Ledger`, `Positions` |
...
| 來源服務     | Topic                      | 事件                 | 處理邏輯                                            | 事件發布                               | DB操作 | 緩存操作         | 服務調用 | 補償機制 |
| -------- | -------------------------- | ------------------ | ----------------------------------------------- | ---------------------------------- | ---- | ------------ | ---- | ---- |
| matching | matching.orderbook-updated | `OrderBookUpdated` | 重建指定 instrument 的買賣盤深度、bestBid/bestAsk/midPrice | -                                  | -    | 更新內存深度快照     | -    | -    |
...
結果存入 `ticker_cache[instrument_id]`，供 REST/WebSocket 查詢使用。
...
* Positions 服務的處理:
  ...
  - 若是平倉，就把對應的保證金 + PnL 金額透過 deltaAvailable/deltaReserved 反映在事件裡。
    Positions 只要以 referenceId=tradeId 找出先前鎖定的關倉量（或直接用事件金額折算），即可知道實際平掉多少。
	  - TradeExecuted 事件本身帶 instrumentId / tradeId / buyOrderId / sellOrderId / price / quantity / buyerUserId / sellerUserId / fee / executedAt …。Positions 消費這個事件並在本地 position_events（或任何你做的投影表）寫下一筆記錄。 這樣就有 tradeId -> 某倉位、某筆預約 的映射。
