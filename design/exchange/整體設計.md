...
| -------------- | -------------------------------------------------------------------- | ------------------------------------- |
| **開倉 (INCREASE)** | - **SPOT_MAIN 扣除**: 從凍結中解除，並扣除交易價值 + 手續費。       <br/> - **ISOLATED_MARGIN 增加**: 增加交易價值。 | `TRADE_SETTLEMENT_SPOT_MAIN` (Debit)   <br/> `TRADE_SETTLEMENT_ISOLATED_MARGIN` (Credit) <br/> `FEE` (Debit)        |
| **平倉 (REDUCE/CLOSE)** | - **ISOLATED_MARGIN 扣除**: 扣除平均開倉成本 + 已實現盈虧。   <br/> - **SPOT_MAIN 增加**: 增加（平均開倉成本 + 已實現盈虧 - 手續費）。 | `TRADE_SETTLEMENT_ISOLATED_MARGIN` (Debit) <br/> `TRADE_SETTLEMENT_SPOT_MAIN` (Credit)   <br/> `FEE` (Debit)        |
