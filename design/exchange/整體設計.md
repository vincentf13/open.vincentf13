...
| -------------- | ------------------------------------------------ | ---------------------------------------------- | ----------------------------------------------------- | --------------------- | ------------------------------------------------------------------- | ---- | ----------------- | --------- |
| positions      | positions.reserve-requested                      | `PositionReserved` / `PositionReserveRejected` | 成功→更新 `SUBMITTED` 並補發 `OrderSubmitted`；失敗→標記 `FAILED` | `OrderSubmitted`（成功時） | update `orders`<status=SUBMITTED, submitted_at=now>                 | -    | exchange-position | 預凍結失敗記錄原因 |
| risk-margin    | risk.margin-check-failed                         | `MarginPreCheckFailed`                         | 風控拒絕→訂單 `FAILED`                                      | -                     | update `orders`<status=FAILED>                                      | -    | risk-margin       | -         |
| account-ledger | ledger.funds-frozen / ledger.funds-freeze-failed | `FundsFrozen` / `FundsFreezeFailed`            | 成功→改 `ACCEPTED` 並發 `OrderCreated`；失敗→`FAILED`         | `OrderCreated`（成功時）   | update `orders`<status=ACCEPTED, submitted_at=now>                  | -    | account-ledger    | -         |
| matching       | matching.trade-executed                          | `TradeExecuted`                                | 更新成交量、均價、手續費與 `filled_at`                             | -                     | update `orders`<status=PARTIAL_FILLED/FILLED, filled_at=executedAt> | -    | matching          | -         |
