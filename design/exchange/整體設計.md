...
> REST 契約中的 DTO 僅封裝資料欄位，所有枚舉集中於對應的 `...rest/api/enums/` 套件，由 server 與 client 共用，避免散落於 `dto` 套件造成重複定義。
...
| 完成  | 來源服務            | Topic                        | 事件                       | 處理邏輯               | 事件發布                                 | DB操作                                                                     | 緩存操作  | 服務調用  | 補償機制  |
| --- | --------------- | ---------------------------- | ------------------------ | ------------------ | ------------------------------------ | ------------------------------------------------------------------------ | ----- | ----- | ----- |
|     | risk-margin     | risk.margin-check-passed     | `MarginPreCheckPassed`   | 凍結保證金並記錄資產變動       | `FundsFrozen`<br>`FundsFreezeFailed` | insert `ledger_entries`<freeze><br/>update `accounts`<reserved_balance>  | -     | -     | -     |
