...
2. **取得風控基準**：查詢 `risk_limits`、`risk_snapshots`，如快照時間落後 `OrderSubmitted` 事件時間，則即時呼叫 `exchange-position` 的「倉位即時查詢 API GET `/api/positions/{userId}/{instrumentId}`」取得最新倉位
3. 以 `SELECT ... FOR UPDATE` 鎖定 snapshot。


4. **驗證槓桿**：`requested_leverage = max(existing_leverage, clientLeverage)`，若超出 `max_leverage` 立即回覆 `MarginPreCheckFailed`。
...
6. 試算預扣保證金    `required_margin = max(order_notional / requested_leverage, order_notional * initial_margin_rate)`。
7. 取得 `equity`：先呼叫 ledger 的可用資金查詢（例如 `/api/ledger/balances?userId=`）取得最新餘額，再與倉位事件提供的 unrealized PnL 合成 equity，並寫回 snapshot 以供後續流程共用。
