...
- 行情
	- WebSocket 推送即時的成交資訊 (最新價、24h 漲跌幅、成交量等
	- WebSocket 推送即時的訂單簿深度數據
	- WebSocket 推送即時的訂單成交信息
	- K 線圖:
...
3. **撮合**：`matching` 消費 `OrderCreated` 進行撮合；
4. 行情：成交後的 `TradeExecuted`/`OrderPlaced`/`OrderCancelled` 事件供 `market-data` 更新 Ticker、深度、K 線，再以 `MarkPriceUpdated` 等事件推送給 `positions`、`risk-margin` 與前端。
5. **倉位**：`positions` 消費撮合與 `ledger` 事件調整持倉、預留數量並發布 `PositionUpdated`；
	1. 消費 `MarkPriceUpdated`
6. 風險回饋：`risk-margin` 消費 `PositionUpdated` 與 `MarkPriceUpdated`，更新 `risk_snapshots`，必要時再發布 `RiskUpdate` 或進一步限制下單。
7. 強平：當 `risk-margin` 偵測 `margin_ratio` 低於門檻時，將任務寫入 `liquidation_queue` 並經由清算流程發送 `LiquidationTriggered`，最終透過 ledger/positions 完成強平與結算。
8. **資金費率**：`market-data` 定期產出 `FundingRateUpdated` 給 ledger 結算；
