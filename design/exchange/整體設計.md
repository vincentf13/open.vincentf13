...
1. **下單與預檢**：Client 透過 Gateway 呼叫 `order-service` 建立委託 → `OrderSubmitted` 發佈後由 `risk-margin` 接手，讀取倉位/行情快照計算名義價值與保證金 → 風險檢查通過則進入下一階段，失敗則發送 `MarginPreCheckFailed` 讓 `order-service` 標記 `FAILED`。
2. **資金凍結**：風控通過的訂單會由 `order-service` 發布 `OrderSubmitted` 給 `account-ledger`；ledger 按所需保證金鎖定資產並回傳 `FundsFrozen`（成功）或 `FundsFreezeFailed`（失敗）。成功時 `order-service` 轉為 `ACCEPTED` 並發布 `OrderCreated`。
3. **資金帳務**：`account-ledger` 
	1. 先接收 `OrderSubmitted` 鎖定保證金，
	2. 消費 `TradeExecuted`、`LiquidationTriggered`、`FundingRateUpdated` 等事件記錄分錄
	3. 發佈 `LedgerEntryCreated` / `FundsFrozen` / `FundsUnfrozen`，將資金變動同步到 order 與 positions 形成閉環。
4. **撮合**：`matching` 消費 `OrderCreated` 進行撮合；成交、掛單、撤單事件將持續驅動後續模組。
5. **行情**：`market-data` 
	1. 依 `TradeExecuted`/`OrderPlaced`/`OrderCancelled` 更新 Ticker、深度、K 線
	2. 以 `MarkPriceUpdated` 等事件推送給 `positions`、`risk-margin` 與前端。
6. **倉位**：`positions` 
	1. 消費撮合、`ledger` 事件以及 `MarkPriceUpdated`，重算未實現盈虧並發布 `PositionUpdated`；
7. 風控與強平：`risk-margin` 
...
8. **資金費率**：`market-data` 定期產出 `FundingRateUpdated` 給 ledger 結算，ledger 調整資金後再將結果事件發回 order/positions 形成完整閉環。
...
### 帳戶淨值 equity
- `equity` `=` `margin_balance` `+` `realized_pnl` `+` `unrealized_pnl`   (不包含倉位佔用保證金)
...
| HTTP Method | Endpoint                                 | 說明                   | 調用者           | 授權      | 輸入                                                       | 輸出                                                                                        | DB操作                                     | 緩存操作 | 服務調用                                  | 補償機制 |
| ----------- | ---------------------------------------- | -------------------- | ------------- | ------- | -------------------------------------------------------- | ----------------------------------------------------------------------------------------- | ---------------------------------------- | ---- | ------------------------------------- | ---- |
| POST        | `/api/positions/intent`                  | 判斷 BUY/SELL 是否為加倉/平倉 | order-service | jwt     | PositionIntentRequest(userId,instrumentId,side,quantity) | PositionIntentResponse(intentType, existingQty, requiresReserve)                          | -                                        | -    | -                                     | -    |
...
| POST        | `/api/positions/{positionId}/leverage`   | 調整倉位槓桿倍數             | risk-margin   | private | path positionId、body: targetLeverage                     | PositionLeverageResponse（appliedLeverage, effectiveAt）                                    | update `positions`<leverage, updated_at> | -    | 呼叫 risk `/api/risk/precheck/leverage` | -    |
...
### 倉位價值
position_value = notional = entry_price * quantity
...
### margin
  - 倉位保證金欄位: 一旦預檢完成且帳本凍結成功，positions 收到成交事件後查倉位、計算新 quantity/entry_price，再依 `margin` `=` `abs(entry_price` `*` `quantity)` `/` `leverage` 算出當前應占用的保證金並更新 positions.margin 與 `margin_ratio` `=` `(margin / position_value)` 來衡量槓桿安全度 ；。
...
### liquidation_price
以下是「完全直接替換成中文名稱」的版本，**格式與結構完全相同，只把英文變中文**，可直接貼進你的筆記。
...
（注意：倉位數量 S 為負數時，此公式會自動計算出正確的空單強平價）
...
如需，我也能提供「已代換中文變數」的 Mermaid 計算流程圖版本。
...
## POST `/api/positions/{positionId}/leverage`

Position 服務收到此請求時，會呼叫 `POST /api/risk/precheck/leverage` 進行預檢，確認目標槓桿符合 `risk_limits` 與保證金要求；僅在風控允許時更新 `positions.leverage` 並寫入事件。
...
4. 更新 `margin`、`margin_ratio`
	 `margin` `=` `abs(entry_price` `*` `quantity)` `/` `leverage` 算出當前應占用的保證金並更新 positions.margin 與 `margin_ratio` `=` `(margin / position_value)`
5. 計算未實現盈虧 (Unrealized PNL):
       * 從本地 `MarkPriceCache` 中讀取最新的標記價格 (`Mark Price`) (該價格由 `MarkPriceUpdated` 事件更新)。
         若暫無資料則保留上一筆標記價，待事件抵達時再刷新。
       * 根據新的倉位狀態計算浮動盈虧。
           * `未實現盈虧` `=` `(標記價格` `-` `新平均開倉價)` `*` `新數量` `*` `合約面值`
   6. 重新計算強平價 (`Liquidation` `Price`): 根據更新後的倉位、槓桿和保證金率，重新計算強平價格。
   7. 持久化儲存:
       * 將更新後的倉位資訊 (數量、均價、未實現盈虧等) 儲存到 `positions` 資料表中。
       * 在 `position_events` 表中新增一筆事件紀錄，用於審計和追蹤。
   8. 發布 `PositionUpdated` 事件: 倉位狀態更新後，發布 PositionUpdated 事件，
      通知 risk-margin (風控) 等下游服務進行相應的風險計算更新。
   9. 平倉時，檢查若倉位數量為0 且 若已消費 `LedgerEntryCreated` 而有已實現盈虧，將該倉位狀態更新為 CLOSED。並發佈 `PositionClosed`  事件
...
| ----------- | ----------------------------- | -------------- | --------- | ------- | -------------------------------------------------------------------------------------------- | ----------------------------------------------------------- | ---- | ---- |
| POST        | `/api/risk/precheck/leverage` | 槓桿調整預檢，回傳允許與缺口 | positions | private | LeveragePrecheckRequest(positionId,targetLeverage,side,quantity,equity,usedMargin,markPrice) | LeveragePrecheckResponse(allow, deficit, suggestedLeverage) | -    | -    |
...
## 消費 `OrderSubmitted`：下單前預檢流程

1. **接收事件**：listener 取得 `OrderSubmitted` payload（包含 userId、instrumentId、price/quantity、side、type 等）。
2. **取得風控基準**：查詢 `risk_limits`、`risk_snapshots`
3. 以 `SELECT ... FOR UPDATE` 鎖定 snapshot。

4. 取得倉位快照： POSITION `/api/positions/{userId}/{instrumentId}`
5. **驗證槓桿**：`requested_leverage = max(existing_leverage, clientLeverage)`，若超出 `max_leverage` 立即回覆 `MarginPreCheckFailed`。
	- 預檢時要用較高的槓桿值來計算所需保證金，所以先取得「倉位現況的槓桿 existing_leverage」以及「用戶本次下單想設定的槓桿 clientLeverage」，兩者取最大值(max(...)) 變成 requested_leverage。這樣可以確保：
		- 如果用戶打算把槓桿調得比現在更高，系統會按較高的槓桿（也就是較低的保證金）進行試算並檢查是否允許。
		- 如果用戶想降低槓桿，仍需至少維持目前倉位的槓桿水準，避免透過一次下單降低整體保證金要求。

6. **計算委託名義價值**：`order_notional = price * quantity`（市價單改用最新 `mark_price`）。
	- `mark_price`：呼叫 `GET /api/market/mark-price/{instrumentId}` 取得即時標記價。

7. 試算預扣保證金    `required_margin = max(order_notional / requested_leverage, order_notional * initial_margin_rate)`。
8. 取得 `equity`：
	1. 先呼叫 ledger 的可用資金查詢： `/api/ledger/balances?userId=`）取得最新餘額
	2. 再與倉位快照提供的 unrealized PnL 合成 equity。

9. 試算成交後指標**：
	- 最新倉位成本價： `simulated_notional = current_notional + order_notional`。
...
整個預檢與 HTTP 呼叫解耦，`OrderSubmitted` 成為唯一入口，可自然串接撮合前置流程並確保有序重試。資金凍結仍由賬本服務在 `MarginPreCheckPassed` 後完成。

## 槓桿調整預檢：`POST /api/risk/precheck/leverage`

1. Position 服務將目標槓桿、倉位方向/數量以及最新 `equity`、`usedMargin`、`markPrice` 一併帶入 `LeveragePrecheckRequest`。
2. `risk-margin` 依照 `risk_limits` 重新計算槓桿後的名義價值、所需保證金與模擬 `margin_ratio`：
   - 先驗證目標槓桿是否在 `max_leverage` 允許範圍內。
   - 以 `notional = markPrice * quantity`、`requiredMargin = max(notional / targetLeverage, notional * initial_margin_rate)` 計算需求。
   - 以 `equity - usedMargin` 檢查可用資金是否足夠，並模擬 `equity / notional` 是否仍高於 `maintenance_margin_rate + buffer`。
