...
當 `matching` 撮合成功發布成交事件後，`positions` 消費它來更新倉位的：`quantity`、`entry_price`、`margin`、`margin_ratio`
...
4. 更新 `margin`、`margin_ratio`
	 `margin = cost_basis / leverage = abs(entry_price * quantity) / leverage`，並以 `margin_balance + unrealized_pnl` 取得 `equity`，計算 `margin_ratio = equity / market_notional` 後一併更新。
...
5. positions.margin 被視為衍生欄位：當真正發生成交、倉位數量／均價改變時，Position 服務才會依 `margin = abs(entry_price * quantity) / leverage`（亦即 `cost_basis / leverage`）重新計算並入庫，同步更新 `margin_ratio = equity / market_notional`（詳見 design/exchange/整體設計.md:3025-3033）。單純修改槓桿並不會驅動這個流程，因此不會自動調整 margin。
...
3. 以 `SELECT ... FOR UPDATE` 鎖定 snapshot：避免多筆訂單同時消耗同一份餘額，保障預檢與凍結資金一致性。

4. 取得倉位快照： POSITION `/api/positions/{userId}/{instrumentId}`
5. **驗證槓桿**：`existing_leverage`，若超出 `max_leverage` 立即回覆 `MarginPreCheckFailed`。
...
- **初始化 `risk_snapshots`**：
  ...
  2. 依當前倉位計算初始指標：
     ...
     - `liquidation_price` 依逐倉公式計算（詳見「### liquidation_price」，以 `marginPerUnit = isolatedMarginAmount / quantity` 推導 long/short 強平價）。
...
• equity = margin_balance + unrealized_pnl。已實現盈虧在逐倉模式下會立即結轉到 margin_balance，因此無需再另外加總。
...
綜合：risk-margin 先向 account-ledger 取 margin_balance，再加上 PositionUpdated 提供的 unrealized_pnl，即可得出
  equity，後續用於計算 margin_ratio = equity / market_notional 等風控指標。
