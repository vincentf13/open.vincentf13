...
│  ├─ sdk-spring-security/              # 共用 Security Filter、授權規則
...
│      └─ sdk-exchange-maching/         # 撮合子模組（REST 契約與客戶端）
│          ├─ rest-api/                 # OpenAPI 契約、DTO、Controller 介面
│          └─ rest-client/              # 依契約生成的 Feign/HTTP 客戶端
└─ services/                            # 具體業務服務
   ├─ pom.xml                           # 聚合子服務，統一打包
   ├─ service-user/                     # 使用者域服務
   │   ├─ domain/                       # Entity / Aggregate Root / Domain Service
   │   │   ├─ UserEntity.java                 # Domain Entity
   │   │   ├─ UserAggregate.java              # Aggregate Root，封裝使用者聚合狀態變更
   │   │   └─ UserDomainService.java          # Domain Service，處理跨聚合的純領域規則
   │   ├─ infra/                        # Repository / Mapper / PO（資料庫表對應）
   │   │   ├─ UserRepository.java             # Repository 介面，屏蔽底層 Mapper
   │   │   ├─ UserRepositoryImpl.java         # Repository 實作，組合 MyBatis Mapper
   │   │   ├─ UserMapper.java                 # MyBatis Mapper
   │   │   └─ UserPO.java                     # Persistence Object
   │   ├─ app/                          # Application Service（用例編排，DTO 與 Entity 映射）
   │   │   └─ UserApplicationService.java     # Application Service，用例流程協調
   │   └─ interface/                    # Controller / API 實作
   │       ├─ UserController.java            # REST Controller
   │       └─ UserApi.java                   # OpenAPI 契約介面
   ├─ service-order/                    # 委託與撮合前置服務
   │   ├─ domain/
   │   ├─ infra/
   │   ├─ app/
   │   └─ interface/
   └─ service-payment/                  # 資金結算相關服務
       ├─ domain/
       ├─ infra/
       ├─ app/
       └─ interface/
...
- 契約優先：先設計 OpenAPI / AsyncAPI，並透過 `sdk-exchange-*` 發佈 SDK 或範本實作。
