...
1. **下單與預檢**： [Saga 流程詳解](#Saga%20流程詳解)
2. **資金帳務**：`account-ledger` 
	1. **資金凍結**：風控通過的訂單會由 `order-service` 發布 `OrderSubmitted` 給 `account-ledger`；ledger 按所需保證金鎖定資產並回傳 `FundsFrozen`（成功）或 `FundsFreezeFailed`（失敗）。成功時 `order-service` 轉為 `ACCEPTED` 並發布 `OrderCreated`。
...
3. **撮合**：`matching` 消費 `OrderCreated` 進行撮合；成交、掛單、撤單事件將持續驅動後續模組。
4. **行情**：`market-data` 
