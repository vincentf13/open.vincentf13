...
2. **委託檢核**：`order` 接收到委託後先調用 `risk-margin` 進行預檢（可用保證金、限額），通過後才寫入委託事件。
3. **撮合發布**：`matching` 撮合成功後，將成交事件發布至事件匯流排，`market-data`、`account-ledger`、`risk-margin` 等下游即會平行訂閱處理。
4. **帳務與倉位**：`account-ledger` 以雙分錄記帳並再發布資產異動事件，`positions` 依此同步更新倉位與盈虧，並把強平訊號回饋給 `risk-margin`。
...
- `POST /api/auth/login`：回傳 Access/Refresh Token。
- `POST /api/auth/token/refresh`：刷新 Access Token。
- `POST /api/auth/logout`：作廢 Refresh Token/Session。
...
## 流程串接摘要
1. **註冊/登入**：使用者透過 Gateway 呼叫 `auth`，取得 JWT/Refresh Token。
2. **委託**：Gateway 驗證 JWT 後呼叫 `order`，後者先調用 `risk-margin` 檢查保證金，再發佈 `OrderCreated` 事件。
3. **撮合**：`matching` 撮合成功即發佈 `TradeExecuted`，`market-data`、`account-ledger`、`risk-margin` 平行消費。
4. **記帳/倉位**：`account-ledger` 發佈 `LedgerEntryCreated`，`positions` 消費後更新倉位並回饋強平訊息給 `risk-margin`。
5. **查詢**：前端可透過 Gateway 查詢委託、倉位；並從 `market-data` WebSocket 接收實時行情。
6. **賣出/平倉**：流程同上，重新建立賣出委託或執行撤單。
