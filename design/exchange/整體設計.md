...
|------------|----------|------|--------|------|------|------|---------|--------|----------|
| POST | `/api/auth/credentials` | 建立憑證 | user | private | AuthCredentialCreateRequest | AuthCredentialResponse | - | select `auth_credentials`<user_id, credential_type>; insert `auth_credentials`<user_id, credential_type, secret_hash, salt, status> | - |
...
| HTTP Method | Endpoint        | 說明         | 調用者     | 授權     | 輸入                                    | 輸出           | 事件發布 | DB操作                                                                                                                                                                                                                | 緩存操作 | 調用服務 | 調用服務API                           | 補償機制                                                                   |
| ----------- | --------------- | ---------- | ------- | ------ | ------------------------------------- | ------------ | ---- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---- | ---- | --------------------------------- | ---------------------------------------------------------------------- |
| POST        | `/api/users`    | 用戶註冊       | gateway | public | UserRegisterRequest (email, password) | UserResponse | -    | insert `users`<external_id,email,status>; insert `auth_credentials_pending`<user_id, credential_type, secret_hash, salt, status, retry_count>; update `auth_credentials_pending`<status, last_error, next_retry_at> | -    | Auth | `POST /api/auth/credentials` 建立憑證 | `UserRegistrationResumer` 排程任務：掃描 `auth_credentials_pending`，重試失敗的憑證建立 |
...
| account-ledger | ledger.funds-frozen | `FundsFrozen` | 更新委託狀態為可送撮合並發出 `OrderCreated` |
...
| 來源服務 | Topic | 事件 | 處理邏輯 |
|----------|-------|------|----------|
| order | order.submitted | `OrderSubmitted` | 根據委託資訊進行保證金試算 |
| positions | positions.updated | `PositionUpdated` | 更新用戶風險指標與狀態 |
| market-data | market.mark-price | `MarkPriceUpdated` | 重新計算維持保證金率與強平價 |
...
| 來源服務 | Topic | 事件 | 處理邏輯 |
|----------|-------|------|----------|
| risk-margin | risk.margin-check-passed | `MarginPreCheckPassed` | 依需求凍結保證金並記錄資產變動 |
| matching | matching.trade-executed | `TradeExecuted` | 進行雙分錄記帳、扣除凍結與計算手續費 |
| matching | matching.order-cancelled | `OrderCancelled` | 解凍對應資金並回補餘額 |
| market-data | market.funding-rate | `FundingRateUpdated` | 依資金費率結算多空支付 |
...
| 來源服務 | Topic | 事件 | 處理邏輯 |
|----------|-------|------|----------|
| order | order.created | `OrderCreated` | 將委託放入訂單簿並觸發撮合流程 |
...
| 來源服務 | Topic | 事件 | 處理邏輯 |
|----------|-------|------|----------|
| matching | matching.trade-executed | `TradeExecuted` | 調整倉位數量、均價與未實現盈虧 |
| account-ledger | ledger.entry-created | `LedgerEntryCreated` | 依資產結算結果更新已實現盈虧並關閉倉位 |
| market-data | market.mark-price | `MarkPriceUpdated` | 以最新標記價格重算未實現盈虧與強平價 |
...
| 來源服務 | Topic | 事件 | 處理邏輯 |
|----------|-------|------|----------|
| matching | matching.order-placed | `OrderPlaced` | 更新訂單簿深度與行情快照 |
| matching | matching.order-cancelled | `OrderCancelled` | 移除訂單並刷新深度圖 |
| matching | matching.trade-executed | `TradeExecuted` | 更新Ticker、生成K線、推送成交 |
