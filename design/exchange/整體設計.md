...
│          ├─ rest-api/                 # OpenAPI 契約、DTO、Controller 介面（含 UserStatus / AuthCredentialType 等共用枚舉）
...
└─ services/                            # 具體業務服務
   ...
   │   │   │   └─ (Enum via contract)        # AuthCredentialType / UserStatus 由 sdk 契約共用
   ...
   │   │       ├─ mapper/UserMapper.java     # MyBatis Mapper 介面，對應 XML 查詢語句
   │   │       ├─ po/UserPO.java             # 資料庫表映射物件，欄位與 users 表一一對應
   │   │       └─ repository/                   # Repository 介面集合，返回 Domain 實體
   │   │           ├─ AuthCredentialRepository.java    # 驗證憑證存取介面
   │   │           │   └─ AuthCredentialRepositoryImpl.java # 實作遵循 `RepositoryImpl` 命名
   │   │           └─ UserRepository.java             # 使用者主檔存取介面
   │   │               └─ UserRepositoryImpl.java     # 透過 OpenMapstruct 完成 UserPO ↔ User 轉換
   │   │                                                 # 並在 @Transactional 邊界由上層控制提交
   │   │                                         # 典型場景：提供 findByEmail, save(User) 供 Application Service 呼叫
