...
2. **鎖定風控基準**：查詢 `risk_limits`、`risk_snapshots`，如快照時間落後 `OrderSubmitted` 事件時間，則即時呼叫 `exchange-position` 的「倉位即時查詢 API GET `/api/positions/{userId}/{instrumentId}`」取得最新倉位，再以 `SELECT ... FOR UPDATE` 鎖定 snapshot。
- 鎖 risk_snapshots 是為了避免同一用戶/商品在多筆同時到達的 OrderSubmitted 事件上發生競態：預檢會依 snapshot 裡的 used_margin、simulated_notional 等資訊判斷是否還能再下單，若不加鎖，兩個並發事件可能各自讀到舊數值，雙雙判定「可下單」， 導致實際可用風險額度被重複使用。

3. **驗證槓桿**：`requested_leverage = max(existing_leverage, clientLeverage)`，若超出 `max_leverage` 立即回覆 `MarginPreCheckFailed`。
...
4. **計算委託名義價值**：`order_notional = price * quantity`（市價單改用最新 `mark_price`）。
   - risk-margin 平時透過 `MarkPriceUpdated` 事件維護快取；若快照 `updatedAt` 早於 `OrderSubmitted` 事件，可呼叫 `GET /api/market/mark-price/{instrumentId}` 取得即時標記價或延遲重試，避免使用過期行情。
5. 試算預扣保證金    `required_margin = max(order_notional / requested_leverage, order_notional * initial_margin_rate)`。
6. 取得 `equity`：先呼叫 ledger 的可用資金查詢（例如 `/api/ledger/balances?userId=`）取得最新餘額，再與倉位事件提供的 unrealized PnL 合成 equity，並寫回 snapshot 以供後續流程共用。
