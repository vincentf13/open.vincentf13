...
- Repository：  
	- 透過 OpenMapstruct 完成 UserPO ↔ User 轉換
...
主要任務是：接收委託事件 → 進入訂單簿 → 配對成交 → **發布所有影響訂單簿的事件**。
...
|market-data|**根據 `TradeExecuted` 更新 Ticker；根據 `OrderPlaced`/`OrderCancelled` 更新深度圖**|
...
開平倉處理流程:
   ...
   7. 發布 `PositionUpdated` 事件: 倉位狀態更新後，發布 PositionUpdated 事件，通知 risk-margin (風控)
      等下游服務進行相應的風險計算更新。
...
`Market-Data` 服務必須消費來自 `Matching` 引擎的全量訂單簿事件，才能確保數據的準確性。
