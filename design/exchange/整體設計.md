...
1. **快速響應**:
    ...
    * 若:
    ...
    - **Intent = REDUCE/CLOSE**：
        ...
        * `Order` 服務將該訂單轉為 Position Saga，寫入 Outbox 發送
          `order.position-reserve-requested (PositionReserveRequested)`，內容包含
          orderId/userId/instrumentId/side/intent/quantity。
...
僅在風控允許時更新 `positions.leverage` 並寫入事件。路徑參數使用 `instrumentId`，而 `userId` 由 JWT 取得、Body 僅需提交
`targetLeverage` 即可鎖定對應的逐倉倉位。
...
- 平倉時：
    - 檢查若倉位數量為0 且 若已消費 `LedgerEntryCreated` 而有已實現盈虧，將該倉位狀態更新為 CLOSED。並發佈
      `PositionClosed`  事件
...
- Positions 只要以 referenceId=tradeId 找出先前鎖定的關倉量（或直接用事件金額折算），即可知道實際平掉多少。
    - TradeExecuted 事件本身帶 instrumentId / quoteAsset / tradeId / orderId(maker) / counterpartyOrderId(taker) /
      price / quantity / makerUserId / takerUserId / makerFee / takerFee / executedAt …。Positions 消費這個事件並在本地
      position_events（或任何你做的投影表）寫下一筆記錄。 這樣就有 tradeId -> 某倉位、某筆預約 的映射。
    - 若你擔心跨 topic 傳輸導致順序顛倒，常見做法是讓 positions 在消費 LedgerEntryCreated 時檢查 position_events 若查不到該
      tradeId，就暫存在記憶體/DB 等待對應的 TradeExecuted 先落地；後續一旦 mapping 補齊再回放遺漏的
      LedgerEntryCreated，即可保持最終一致。這屬於典型的 out-of- order 處理策略，不需要把流程改成同步呼叫。
