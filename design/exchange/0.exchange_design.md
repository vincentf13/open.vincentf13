...
| 完成  | 發布場景             | Topic                       | 事件名稱                       |
| --- | ---------------- | --------------------------- | -------------------------- |
|     | 收到用戶/強平委託並完成基本校驗 | order.funds-freeze-requested | `FundsFreezeRequested`     |
...
| 完成  | 發布場景                          | Topic                        | 事件名稱                 |
| --- | ----------------------------- | ---------------------------- | -------------------- |
| ✔️  | 開倉保證金凍結成功                     | account.funds-frozen         | `FundsFrozen`        |
| ✔️  | 開倉保證金凍結失敗                     | account.funds-freeze-failed  | `FundsFreezeFailed`  |
| ✔️  | 消費 `TradeExecuted` 完成保證金結轉/返還 | account.trade-margin-settled | `TradeMarginSettled` |
...
| 完成  | Topic                        | 事件                       | 處理邏輯                                                                           | 事件發布                 |
| --- | ---------------------------- | ------------------------ | ------------------------------------------------------------------------------ | -------------------- |
| ✔️  | order.funds-freeze-requested | `FundsFreezeRequested`   | 下單提交後檢查並預處理資金（凍結/校驗邏輯待補）                                                       |                      |
| ✔️  | matching.trade-executed      | `TradeExecuted`          | 針對開倉成交：扣除凍結、計算手續費、進行雙分錄記帳，將 SPOT reserved 結轉至逐倉保證金，若是maker 成交退回超收手續費，發布保證金結轉事件 | `TradeMarginSettled` |
| ✔️  | positions.margin-released    | `PositionMarginReleased` | 針對平倉成交，將逐倉保證金轉回 SPOT available，紀錄已實現盈虧                                         |                      |
...
|---|---|---|---|---|
|**POST**|`/api/positions/intent`|判斷交易意圖（開倉/平倉/減倉），回傳 `PositionIntent`|Order Svc|JWT|
|**GET**|`/api/positions/{userId}/{instrumentId}`|查詢單一交易對的即時倉位快照|Risk Svc, Gateway|Private/JWT|
...
|**Topic**|**事件名稱**|**處理邏輯**|
|---|---|---|
|`positions.reserve-requested`|`PositionReserveRequested`|**平倉預檢**：檢查 `quantity - closing_reserved` 是否足夠。成功則增加 `closing_reserved_quantity` 並發布 `Reserved` 事件。|
|`account.trade-margin-settled`|`TradeMarginSettled`|**開倉入帳**：建立新倉位或增加持倉。更新 `margin` (從錢包轉入) 與 `entry_price`。|
|`matching.trade-executed`|`TradeExecuted`|**平倉結算**：減少持倉，計算 `realized_pnl`。發布 `PositionMarginReleased` 通知 Ledger 退還保證金。|
...
|**Topic**|**事件名稱**|**觸發場景**|
|---|---|---|
|`positions.reserve`|`PositionReserved`|平倉前，成功鎖定可用持倉數量|
|`positions.reserve`|`PositionReserveRejected`|平倉前，可用持倉不足，拒絕鎖定|
|`positions.margin-released`|`PositionMarginReleased`|平倉/減倉完成，通知 Ledger 釋放保證金並結算盈虧|
|`positions.updated`|`PositionUpdated`|倉位狀態變更（開倉、平倉、強平、行情更新）|
...
- **保證金率 (Margin Ratio)**：
    ...
    - 公式：`margin_ratio = equity / market_notional`
        ...
        - `market_notional` = `abs(mark_price * quantity)` (倉位市值)
...
- **未實現盈虧 (Unrealized PnL)**
    ...
    - 公式：`(mark_price - entry_price) * quantity * direction`
        - Long: `direction = 1`
        - Short: `direction = -1`
- **已實現盈虧 (Realized PnL)**
    - 用途：實際結算、記入帳本。
    - 觸發：`TradeExecuted` (平倉/減倉時)。
    - 公式：`(fill_price - entry_price) * trade_quantity * direction`
...
- 多單 (Long) 強平價：
    ...
    $$LiqPrice_{Long} = \frac{EntryPrice - \frac{Margin}{Quantity}}{1 - MMR}$$
- 空單 (Short) 強平價：
    
    $$LiqPrice_{Short} = \frac{EntryPrice + \frac{Margin}{Quantity}}{1 + MMR}$$
- 變數說明：
