...
|----|----------------------------|---------------------------|---------------------------------------------|------------------|------|
| ✔️ | order.submitted             | `OrderSubmitted`         | 基本校驗通過，待資金處理與撮合                           | -                | DB: update `orders`<status, submitted_at, updated_at, version> |
|    | ledger.entry-created       | `LedgerEntryCreated`     | 預扣或結算餘額變動 → 更新餘額                           | -                | DB: update `orders`<status, updated_at, version> |
...
| OrderSubmitted  |                |              | ORDER_SERVICE   |
| OrderCreated    |                |              | ORDER_SERVICE   |
...
- **事件驅動**：使用事件名稱（OrderSubmitted、OrderCreated、TradeExecuted…）。
...
| 完成  | Topic                   | 事件                       | 處理邏輯               | 事件發布               | IO操作 |
| --- | ----------------------- | ------------------------ | ------------------ | ------------------ |------|
| ✔️  | matching.trade-executed | `TradeExecuted`          | 扣除凍結、計算手續費、進行雙分錄記帳 | `LedgerEntryCreated` | DB: insert `ledger_entries`<trade><br/>update `accounts`<available/reserved> |
