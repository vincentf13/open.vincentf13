...
- 若 Intent = REDUCE、CLOSE
  平倉流程:
    ...
    - **撮合**:
	    - Matching Svc 撮合成功，發布 `TradeExecuted`。
    - **結算 (異步)**:
        - Positions Svc 消費 `TradeExecuted`: 扣減持倉數量，計算 `magin 釋放金額`、 `Realized PnL`，併發布事件
        - Ledger Svc 消費 消費事件: 將保證金從 `Isolated Margin` 轉回 `Spot Available`，並紀錄盈虧。
        - Order Svc 消費 `TradeExecuted`: 更新 filled_quantity、remaining_quantity、avg_fill_price、fee、filled_at
...
| 完成 | Topic                   | 事件                   | 處理邏輯                | 事件發布          |
|----|-------------------------|----------------------|---------------------|----------------|
|    | matching.trade-executed | `TradeExecuted`      | 調整倉位數量、均價與未實現盈虧     | `PositionUpdated` |
