...
- 調用  `/api/positions/intent`
	...
	- 若為平倉，鎖定平倉數量，並返回 `closing_entry_price`
   ...
   平倉流程:
	- **倉位鎖定**:
        ...
        - Positions Svc 消費後檢查可平倉數量，
           ...
           order `PositionReserved` 
        - Order Svc 紀錄 `closing_entry_price`與 submitted_at，將狀態改為 `NEW`，發布 `OrderCreated` 給 Matching Svc。
...
|     | positions.reserve-rejected       | `PositionReserveRejected` | 平倉鎖倉失敗，狀態轉 `REJECTED` 並紀錄原因 | - |
...
|     | **Topic**                      | **事件名稱**                   | **處理邏輯**                                                                                                                                   |
| --- | ------------------------------ | -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ |
| ✔️  | `positions.reserve-requested`  | `PositionReserveRequested` | **平倉預檢**：檢查 `quantity - closing_reserved` 是否足夠。成功則增加 `closing_reserved_quantity` 並發布 `Reserved` 事件。<br>失敗則發布 `PositionReserveRejected` 事件。 |
...
|     | **Topic**                    | **事件名稱**                  | **觸發場景**                     |
| --- | ---------------------------- | ------------------------- | ---------------------------- |
| ✔️  | `positions.reserve`          | `PositionReserved`        | 平倉前，成功鎖定可用持倉數量               |
| ✔️  | `positions.reserve-rejected` | `PositionReserveRejected` | 平倉前，可用持倉不足，拒絕鎖定              |
