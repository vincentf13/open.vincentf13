...
- 若 **Intent = INCREASE**
   開倉流程:
	- **風控預檢**: Order Svc 呼叫  `/api/risk/orders/precheck`，
		...
		- fee 先以 Taker fee 預收
	- **資金凍結**:
		- 將狀態改為 `FREEZING_MARGIN` 並發布 `FundsFreezeRequested`。
...
| 完成  | Topic                     | 事件                       | 處理邏輯                                                                         | 事件發布                 |
| --- | ------------------------- | ------------------------ | ---------------------------------------------------------------------------- | -------------------- |
|     | order.funds-freeze-requested | `FundsFreezeRequested`   | 下單提交後檢查並預處理資金（凍結/校驗邏輯待補）                                                     |                      |
|     | matching.trade-executed   | `TradeExecuted`          | 針對開倉成交：扣除凍結、計算手續費、進行雙分錄記帳，將 SPOT reserved 結轉至逐倉保證金，maker 成交退回超收手續費，發布保證金結轉事件 | `TradeMarginSettled` |
|     | positions.margin-released | `PositionMarginReleased` | 針對平倉成交，將逐倉保證金轉回 SPOT available，紀錄已實現盈虧                                       |                      |
...
| 完成  | Topic                        | 事件                         | 處理邏輯                                                                                       | 事件發布                                           |
| --- | ---------------------------- | -------------------------- | ------------------------------------------------------------------------------------------ | ---------------------------------------------- |
|     | account.trade-margin-settled | `TradeMarginSettled`       | 開倉: 建立/更新逐倉倉位，記錄 entry_price 與累積已入帳保證金                                                     | `PositionUpdated`                              |
...
|     | matching.trade-executed      | `TradeExecuted`            | 平倉: 調整倉位數量、均價與未實現盈虧，計算釋放保證金與已實現損益並發布保證金釋放事件                                                | `PositionUpdated` / `PositionMarginReleased`   |
