...
- 單純修改槓桿並不會驅動 position.magin 的增減流程。
...
2. `risk-margin` 依照 `risk_limits` 重新計算：
    ...
    - 以`requiredMargin = max(notional / targetLeverage, notional * initial_margin_rate)` 計算需求。
      (代表調整槓桿後不得低於維持保證金率，且初始保證金率需高於維持保證金率)
    - 模擬 `requiredMargin / notional` 是否仍高於 `maintenance_margin_rate + buffer`。
    - 當 `notional` = 0 時，代表首次開倉前配置的槓桿，回傳通過 `allow=true`
3. 若任一步驟不符，回傳 `allow=false` 並附缺口／建議槓桿；
    通過則 `allow=true`，Position 得以更新 `positions.leverage`。
