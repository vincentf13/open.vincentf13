...
|----|----------------------------|---------------------------|---------------------------------------------|------------------|------|
| ✔️ | positions.reserved         | `PositionReserved`        | 預凍結成功 → 訂單 `SUBMITTED` 並補發 `OrderSubmitted` | `OrderSubmitted` | DB: update `orders`<status, submitted_at, updated_at, version, close_cost_price> |
