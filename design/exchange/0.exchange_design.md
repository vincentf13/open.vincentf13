...
- 若 **Intent = INCREASE**
   開倉流程:
    ...
    - **結算 (異步)**:
	    - Ledger Svc 消費 `TradeExecuted`: 將 `Spot Reserved` 轉入 `Isolated Margin` (正式扣款)。若結果為 maker ，將多收 fee 退回，並發布事件，告知 `TradeExecuted`信息與逐倉保證金額。 
        - Positions Svc 消費 事件: 建立新倉位，記錄  `entry_price` 與 `margin` 與更新相關信息。
...
| 完成 | Topic                   | 事件              | 處理邏輯               | 事件發布               |
|----|-------------------------|-----------------|--------------------|--------------------|
|    | matching.trade-executed | `TradeExecuted` | 扣除凍結、計算手續費、進行雙分錄記帳 | `LedgerEntryCreated` |
