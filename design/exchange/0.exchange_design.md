...
- 調用  `/api/positions/intent`，取得:  `PositionIntentType`：`INCREASE`（純開倉）、`REDUCE`（部分平倉）、`CLOSE`（全部平倉）。
	- 以 `CREATED` 狀態寫入 orders，並紀錄 `intent`
- 若 **Intent = INCREASE**
   開倉流程:
	- **風控預檢**: Order Svc 呼叫  `/api/risk/orders/precheck`，取得 requiredMargin 與 fee。( fee 先以 Taker fee 預收)
	- **資金凍結**:
    ...
    - **結算 (異步)**:
	    - Ledger Svc 消費 `TradeExecuted`: 將 `Spot Reserved` 轉入 `Isolated Margin` (正式扣款)。若結果為 maker ，將多收 fee 退回，發布事件 `TradeMarginSettled` ，告知 `TradeExecuted`事件信息與逐倉保證金額。 
        - Positions Svc 消費 `TradeMarginSettled` 事件: 建立新倉位，記錄  `entry_price` 與 `margin` 與更新相關信息。
        - Order Svc 消費 `TradeExecuted`: 更新 filled_quantity、remaining_quantity、avg_fill_price、fee、filled_at
- 若 Intent = REDUCE、CLOSE
  平倉流程:
	- **倉位鎖定**:
		- Order Svc 判斷為平倉單，轉為 `LOCKING_POSITION` 狀態並發布 `PositionReserveRequested` 事件。
        - Positions Svc 消費後檢查可平倉數量，成功回 `PositionReserved`告知  `closing_entry_price`，不足回 `PositionReserveRejected`。
        - Order Svc 收到 `PositionReserved` ，紀錄 `closing_entry_price`，紀錄 submitted_at，將狀態改為 `NEW`，發布 `OrderCreated` 給 Matching Svc。
           若收到 `PositionReserveRejected` 狀態改為 `rejected`，紀錄 rejected_reason。
    - **撮合**:
	    - Matching Svc 撮合成功，發布 `TradeExecuted`。
    - **結算 (異步)**:
        - Positions Svc 消費 `TradeExecuted`: 扣減持倉數量，計算 `magin 釋放金額`、 `Realized PnL`，發布事件 `PositionMarginReleased`，告知 `TradeExecuted`信息與`magin 釋放金額`、 `Realized PnL`
        - Ledger Svc 消費 `PositionMarginReleased` : 將保證金從 `Isolated Margin`扣除手續費後 轉回 `Spot Available`，並紀錄盈虧。
        - Order Svc 消費 `TradeExecuted`: 更新 filled_quantity、remaining_quantity、avg_fill_price、fee、filled_at
...
|    | DELETE      | `/api/orders/{orderId}` | 撤銷委託 | gateway                      | jwt           |
| ✔️ | GET         | `/api/orders/{orderId}` | 查詢訂單 | gateway , account-ledger     | jwt,private   |
...
|     | ledger.funds-frozen     | `FundsFrozen`   | 保證金凍結成功，狀態轉 `NEW` 並發布 `OrderCreated` | `OrderCreated` |
|     | ledger.funds-freeze-failed | `FundsFreezeFailed` | 保證金凍結失敗，狀態轉 `REJECTED` 並紀錄原因 | - |
...
| 完成 | HTTP Method | Endpoint               | 說明      | 調用者 | 授權    |
|----|-------------|------------------------|---------|------|-------|
|    | POST        | `/api/matching/cancel` | 撮合層執行撤單 | -    | private |
...
| ✔️ | GET         | `/api/market/mark-price/{instrumentId}` | 讀取最新標記價     | risk-margin / positions | public |
...
| 完成 | HTTP Method | Endpoint                              | 說明                   | 調用者         | 授權        |
|----|-------------|---------------------------------------|----------------------|-------------|-----------|
| ✔️ | GET         | `/api/ledger/balances?userId=&asset=` | 查詢 SPOT_MAIN 餘額/可用資金 | gateway,risk | jwt,private |
| ✔️ | POST        | `/api/ledger/deposits`                | 鏈上/法幣儲值入帳            | wallet-sync,admin | private     |
| ✔️️ | POST        | `/api/ledger/withdrawals`             | 出金申請與扣帳              | wallet,admin  | private     |
...
| 完成  | 發布場景                          | Topic                       | 事件名稱                 |
| --- | ----------------------------- | --------------------------- | -------------------- |
|     | 記錄任一資產異動                      | ledger.entry-created        | `LedgerEntryCreated` |
|     | 消費 `TradeExecuted` 完成保證金結轉/返還 | ledger.trade-margin-settled | `TradeMarginSettled` |
|     | 開倉保證金凍結成功                    | ledger.funds-frozen         | `FundsFrozen` |
|     | 開倉保證金凍結失敗                    | ledger.funds-freeze-failed  | `FundsFreezeFailed` |
...
| 完成 | Topic                   | 事件              | 處理邏輯               | 事件發布               |
|----|-------------------------|-----------------|--------------------|--------------------|
|    | matching.trade-executed | `TradeExecuted` | 扣除凍結、計算手續費、進行雙分錄記帳，將 SPOT reserved 結轉至逐倉保證金，maker 成交退回超收手續費，發布保證金結轉事件 | `LedgerEntryCreated` / `TradeMarginSettled` |
|    | positions.margin-released | `PositionMarginReleased` | 針對平倉成交，將逐倉保證金轉回 SPOT available，紀錄已實現盈虧 | `LedgerEntryCreated` |
...
| ✔️ | GET         | `/api/positions/{userId}/{instrumentId}` | 即時倉位快照                 | risk-margin   | private |
...
|    | ledger.entry-created    | `LedgerEntryCreated` | 依資產結算結果更新已實現盈虧並關閉倉位 | `PositionUpdated` |
|    | market.mark-price       | `MarkPriceUpdated`   | 以最新標記價格重算未實現盈虧與強平價  | `PositionUpdated` |
|    | ledger.trade-margin-settled | `TradeMarginSettled` | 建立/更新逐倉倉位，記錄 entry_price 與已入帳保證金，回寫倉位資訊 | `PositionUpdated` |
