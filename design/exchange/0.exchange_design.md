...
| **liquidation_price** | 逐倉公式：`Long = (entryPrice - marginPerUnit)/(1 - mmr)`、`Short = (entryPrice + marginPerUnit)/(1 + mmr)`，其中 `marginPerUnit = isolatedMarginAmount / (quantity * contract_size)`、`mmr = maintenance_margin_rate` | 強平價（依倉位方向計算） |
