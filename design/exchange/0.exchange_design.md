...
這是完整的專案程式碼，包含所有類別檔案。請將它們放在 `com.trading.engine` 套件目錄下。
...
public class MatchResult {
    public long takerOrderId;
    public Order originalTakerOrder; 
    public List<Trade> trades = new ArrayList<>();
    public List<OrderUpdate> updates = new ArrayList<>();
}
```
...
public class OrderBook {
    ...
    private void insertToBook(Order order) {
        ...
        OrderBucket bucket = book.computeIfAbsent(order.price, OrderBucket::new);
        bucket.add(order);
    ...
    public void serialize(DataOutputStream out) throws IOException {
        out.writeInt(orderIndex.size());
        for (Order order : orderIndex.values()) {
            out.writeLong(order.orderId);
            out.writeLong(order.price);
            out.writeLong(order.qty);
            out.writeLong(order.userId);
            out.writeBoolean(order.isBuy);
        }
    }
    ...
    public void deserialize(DataInputStream in) throws IOException {
        ...
        for (int i = 0; i < size; i++) {
            ...
            boolean isBuy = in.readBoolean();

            Order order = new Order(id, price, qty, userId, isBuy);
            insertToBook(order);
        }
    }
}
...
public class MatchingEngine implements AutoCloseable {
    ...
    private final List<MatchResult> resultBuffer;

    private static final String WAL_FILE = "engine.wal";
    private static final String SNAPSHOT_FILE = "engine.snapshot";
    private static final int GROUP_COMMIT_SIZE = 10;
    ...
    public MatchingEngine() throws IOException {
        ...
        boolean walExists = walFile.exists();
        this.walChannel = new RandomAccessFile(walFile, "rw").getChannel();
        this.walBuffer = ByteBuffer.allocateDirect(4 * 1024 * 1024); // 4MB
    ...
    public void processOrder(Order order) throws IOException {
        ...
        if (resultBuffer.size() >= GROUP_COMMIT_SIZE) {
            commitBatch();
        }
    }

    private void commitBatch() throws IOException {
        if (resultBuffer.isEmpty()) return;

        walBuffer.clear();
        for (MatchResult res : resultBuffer) {
            serializeResultToBuffer(res);
        }

        walBuffer.flip();
        while (walBuffer.hasRemaining()) {
            walChannel.write(walBuffer);
        }
        ...
        if (currentSequence % SNAPSHOT_INTERVAL == 0) {
            takeSnapshot();
        }
    }

    private void serializeResultToBuffer(MatchResult res) {
        walBuffer.putLong(res.takerOrderId);
        walBuffer.putInt(res.trades.size());
        walBuffer.putInt(res.updates.size());

        for (Trade t : res.trades) {
            walBuffer.putLong(t.makerOrderId);
            walBuffer.putLong(t.takerOrderId);
            walBuffer.putLong(t.price);
            walBuffer.putLong(t.quantity);
        }

        for (OrderUpdate u : res.updates) {
            walBuffer.putLong(u.orderId);
            walBuffer.putLong(u.remainingQty);
            walBuffer.put((byte) (u.isTaker ? 1 : 0));
        }
    }

    private void takeSnapshot() throws IOException {
        System.out.println(">>> Taking Snapshot at seq: " + currentSequence);

        File tempFile = new File(SNAPSHOT_FILE + ".tmp");
        try (DataOutputStream out = new DataOutputStream(new BufferedOutputStream(new FileOutputStream(tempFile)))) {
            ...
            orderBook.serialize(out);
        }

        File targetFile = new File(SNAPSHOT_FILE);
        ...
        System.out.println(">>> Snapshot Complete. WAL Truncated.");
    }

    private void loadSnapshot() throws IOException {
        File snapFile = new File(SNAPSHOT_FILE);
        if (!snapFile.exists()) {
            System.out.println("No snapshot found. Starting fresh.");
            return;
        }

        try (DataInputStream in = new DataInputStream(new BufferedInputStream(new FileInputStream(snapFile)))) {
            currentSequence = in.readLong();
            orderBook.deserialize(in);
            System.out.println("Snapshot loaded. Sequence: " + currentSequence);
        }
    }

    private void replayWAL() throws IOException {
        System.out.println("Checking WAL for replay...");
        walChannel.position(0);
        ...
        while (buffer.hasRemaining()) {
            ...
            long takerId = buffer.getLong();
            int tradeCount = buffer.getInt();
            int updateCount = buffer.getInt();

            int requiredBytes = (tradeCount * 32) + (updateCount * 17);
            ...
            // Skip Trades
            buffer.position(buffer.position() + (tradeCount * 32));

            MatchResult res = new MatchResult();
            res.takerOrderId = takerId;
            res.originalTakerOrder = new Order(takerId, 0, 0, 0, false);

            for (int i = 0; i < updateCount; i++) {
                long orderId = buffer.getLong();
                long remQty = buffer.getLong();
                boolean isTaker = buffer.get() == 1;

                if (isTaker && remQty > 0) {
                    res.originalTakerOrder.qty = remQty;
                }
                res.updates.add(new OrderUpdate(orderId, remQty, isTaker));
            }
        ...
        System.out.println("WAL Replay finished. Replayed batches: " + replayedCount);
    }
...
public class Main {
    public static void main(String[] args) {
        try {
            // 清理舊檔案以便測試
            new File("engine.wal").delete();
            new File("engine.snapshot").delete();

            System.out.println("=== 第一次啟動 (Fresh Start) ===");
            runEngineSimulation(1500);

            System.out.println("\n=== 第二次啟動 (Recovery) ===");
            runEngineSimulation(100);
    ...
    private static void runEngineSimulation(int orderCount) throws Exception {
        try (MatchingEngine engine = new MatchingEngine()) {
            // 模擬掛單 (Sell): 20000, 10000
            engine.processOrder(new Order(1, 20000, 10000, 101, false));

            // 模擬連續吃單 (Buy): 20000, 1
            for (int i = 0; i < orderCount; i++) {
                engine.processOrder(new Order(100 + i, 20000, 1, 200 + i, true));
            }
        }
    }
}
