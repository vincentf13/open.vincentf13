...
Matching Service 使用此表實現 Transactional Outbox 模式，確保數據一致性。

```sql
CREATE TABLE mq_outbox (
    event_id        VARCHAR(36)  NOT NULL COMMENT 'UUID 去重鍵',
    aggregate_type  VARCHAR(100) NOT NULL COMMENT '原始業務事件的類型 (e.g. Matching)',
    aggregate_id    BIGINT       NOT NULL COMMENT '原始業務事件的ID (e.g. TradeID)',
    event_type      VARCHAR(64)  NOT NULL COMMENT '事件類型 (e.g. TradeExecuted)',
    payload         JSON         NOT NULL COMMENT '事件內容',
    seq             BIGINT       NULL COMMENT '全局順序號 (Matching專用，用於WAL斷點續傳)',
    created_at      DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (event_id),
    UNIQUE KEY uk_seq (seq) COMMENT 'Matching 專用順序索引'
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COMMENT ='CDC Outbox 表';
```
