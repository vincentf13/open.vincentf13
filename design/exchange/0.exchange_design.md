...
- **服務邊界 (Data Ownership):** 撮合引擎 **只寫入自己的數據 (`trade_tickers`, `mq_outbox`)**，**絕不** 直接修改 Order Service 的 `orders` 表。
...
flowchart TD
    ...
    subgraph Async_Loader [異步執行緒 - Loader]
        ...
        DB_Tx --> SQL_Biz["Insert trade_tickers"]
...
class OrderNode {
    ...
    long price;
    long qty;            // 剩餘未成交數量
    long timestamp;      // 下單時間 (用於時間優先排序)
    long userId;
    OrderNode prev;      // 雙向鏈結指標
    OrderNode next;
}
```
...
class OrderBook {
    // 賣盤：價格低優先 (ASC)
    TreeMap<Long, OrderBucket> asks;
    // 買盤：價格高優先 (DESC)
    TreeMap<Long, OrderBucket> bids;
    
    // 方法：getBestBid(), getBestAsk(), addOrder(), removeOrder()
}
```
...
```
class OrderMap {
    // Key: OrderID, Value: Node Reference
    // 用於撤單時直接定位，無需遍歷 TreeMap
    HashMap<Long, OrderNode> index;
}
```

#### 撮合結果物件

這些物件封裝了「變化」，用於寫入 WAL 和生成 Outbox。
...
class OrderUpdate {
    long orderId;
    String status;       // FILLED, PARTIALLY_FILLED, CANCELLED, REJECTED, NEW
    long remainingQty;
    long matchQty;       // 本次變動的量 (用於增量更新)
}
```
...
class OrderBookDiff {
    long instrumentId;
    // 僅記錄有變動的價格檔位，而非全量快照 (節省頻寬)
    List<PriceLevel> updatedBids; 
    List<PriceLevel> updatedAsks; 
}
```

##### `PriceLevel` (價格檔位快照)

用於 `OrderBookDiff`，描述某個價格的總量變化。
...
class PriceLevel {
    long price;
    long qty;            // 該價格當前的總掛單量 (若為 0 代表該層級移除)
}
```

##### `MatchResult` (單次請求的完整結果)

Java

```
class MatchResult {
    long inputOffset;       // Kafka Offset
    long sourceOrderId;     // 用於去重
    
    List<Trade> trades;
    List<OrderUpdate> orderUpdates;
    OrderBookDiff bookDiff;
}
```
...
class ResultBuffer {
    ...
    long batchLastSeq;      // 本批次最後一個 WAL Seq
    long batchLastOffset;   // 本批次最後一個 Kafka Offset
}
```
...
2. **DB Transaction Begin:**
    - **Insert Trades:** 批量寫入 `trade_tickers`。
    - **Insert Outbox (關鍵):**
