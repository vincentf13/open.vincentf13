...
- 若 **Intent = INCREASE**
   開倉流程:
	- **風控預檢**: Order Svc 呼叫  `/api/risk/orders/precheck`，
		...
		- fee 先以 Taker fee 預收
	- **資金凍結**:
		- 將狀態改為 `FREEZING_MARGIN` 並發布 `OrderSubmitted`。
    ...
    - **結算 (異步)**:
	    - Ledger Svc 消費 `TradeExecuted`: 
		    - 將 `Spot Reserved` 轉入 `Isolated Margin` (正式扣款)。
		    - 若結果為 maker ，將多收 fee 退回，
		    - 發布事件 `TradeMarginSettled` ，內含 `TradeExecuted`事件信息與逐倉保證金額。 
        - Positions Svc 消費 `TradeMarginSettled` 事件
	        - 建立新倉位，記錄  `entry_price` 與 `margin` 與更新相關信息。
        - Order Svc 消費 `TradeExecuted`
	        - 更新 filled_quantity、remaining_quantity、avg_fill_price、fee、filled_at
- 若 Intent = REDUCE、CLOSE
   平倉流程:
	- **倉位鎖定**:
		- Order 轉為 `LOCKING_POSITION` 狀態並發布 `PositionReserveRequested` 事件。
        - Positions Svc 消費後檢查可平倉數量，
           成功回 `PositionReserved` 內含  `closing_entry_price`，不足回 `PositionReserveRejected`。
        - Order Svc 收到 `PositionReserved` ，紀錄 `closing_entry_price`與 submitted_at，將狀態改為 `NEW`，發布 `OrderCreated` 給 Matching Svc。
    ...
    - **結算 (異步)**:
        - Positions Svc 消費 `TradeExecuted`: 
	        - 扣減持倉數量
	        - 計算 `magin 釋放金額`、 `Realized PnL`
	        - 發布事件 `PositionMarginReleased`，告知 `TradeExecuted`信息與`magin 釋放金額`、 `Realized PnL`
        - Ledger Svc 消費 `PositionMarginReleased` : 
	        - 將保證金從 `Isolated Margin`扣除手續費後 轉回 `Spot Available`，並紀錄盈虧。
        - Order Svc 消費 `TradeExecuted`: 
	        - 更新 filled_quantity、remaining_quantity、avg_fill_price、fee、filled_at
...
| **`TradeExecuted`**    | 兩筆訂單撮合成交      | `instrumentId`, `quoteAsset`, `tradeId`, `orderId`(maker), `counterpartyOrderId`(taker), `orderSide`(maker), `counterpartyOrderSide`(taker), `price`, `quantity`, `makerUserId`, `takerUserId`, `makerFee`, `takerFee`, `executedAt` |
...
| 模組             | 處理                                                                       |
|----------------|--------------------------------------------------------------------------|
| account | 雙分錄記帳、扣除凍結、計算手續費、發布 `AccountEntryCreated`                                 |
...
- 取消分散式事務的簡化要點
	...
	- Order 可增加字段標記，讓撮合前事務階段可以打上標記，一旦走到NEW時，自動取消。
		...
		- 自動取消流程
    		...
    		- **標準撤銷：** 系統發布 `OrderCancelRequested` 事件，觸發 **Matching 服務** 和 **Ledger 服務** 執行與手動撤單相同的標準解鎖和狀態更新流程。
- 冪等性與錯誤處理
	- **冪等性 (Idempotency):**
	    ...
	    - **LedgerSvc:** 資金解凍操作必須基於唯一的 `reference_id`（例如 `orderId` + `CANCEL` 類型）寫入 `ledger_entries`，防止重複解凍資金。   
- **錯誤與補償 (Compensation):**
    - 重試 
	    ...
	    - 若 **LedgerSvc 失敗**（如資料庫鎖定）：LedgerSvc 應有重試機制。
	- 人工處理  
...
| 完成  | HTTP Method | Endpoint                | 說明   | 調用者                          | 授權            |
| --- | ----------- | ----------------------- | ---- | ---------------------------- | ------------- |
|     | POST        | `/api/orders`           | 建立委託 | gateway / liquidation-worker | jwt / private |
...
| 完成  | 發布場景             | Topic                  | 事件名稱                   |
| --- | ---------------- | ---------------------- | ---------------------- |
|     | 收到用戶/強平委託並完成基本校驗 | order.submitted        | `OrderSubmitted`       |
|     | 保證金凍結成功、委託正式建檔   | order.created          | `OrderCreated`         |
|     | 用戶或系統請求撤單        | order.cancel-requested | `OrderCancelRequested` |
|     | 平倉前鎖定可用倉位        | positions.reserve-requested | `PositionReserveRequested` |

**事件消費**:

| 完成  | Topic                   | 事件              | 處理邏輯                        | 事件發布 |
| --- | ----------------------- | --------------- | --------------------------- | ---- |
|     | matching.trade-executed | `TradeExecuted` | 訂單更新成交量、均價、手續費與 `filled_at` | -    |
|     | account.funds-frozen     | `FundsFrozen`   | 保證金凍結成功，狀態轉 `NEW` 並發布 `OrderCreated` | `OrderCreated` |
|     | account.funds-freeze-failed | `FundsFreezeFailed` | 保證金凍結失敗，狀態轉 `REJECTED` 並紀錄原因 | - |
|     | positions.reserve       | `PositionReserved` | 平倉鎖倉成功，紀錄 `closing_entry_price`，狀態轉 `NEW` 並發布 `OrderCreated` | `OrderCreated` |
|     | positions.reserve       | `PositionReserveRejected` | 平倉鎖倉失敗，狀態轉 `REJECTED` 並紀錄原因 | - |
...
- TradeExecuted 由 ledger/positions/risk/market 並行消費，完成資金扣解凍、倉位調整與行情推播；
...
| 完成 | HTTP Method | Endpoint                              | 說明                   | 調用者         | 授權        |
|----|-------------|---------------------------------------|----------------------|-------------|-----------|
| ✔️ | GET         | `/api/account/balances?userId=&asset=` | 查詢 SPOT_MAIN 餘額/可用資金 | gateway,risk | jwt,private |
| ✔️ | POST        | `/api/account/deposits`                | 鏈上/法幣儲值入帳            | wallet-sync,admin | private     |
| ✔️️ | POST        | `/api/account/withdrawals`             | 出金申請與扣帳              | wallet,admin  | private     |
...
| 完成  | 發布場景                          | Topic                       | 事件名稱                 |
| --- | ----------------------------- | --------------------------- | -------------------- |
|     | 記錄任一資產異動                      | account.entry-created        | `AccountEntryCreated` |
|     | 消費 `TradeExecuted` 完成保證金結轉/返還 | account.trade-margin-settled | `TradeMarginSettled` |
|     | 開倉保證金凍結成功                    | account.funds-frozen         | `FundsFrozen` |
|     | 開倉保證金凍結失敗                    | account.funds-freeze-failed  | `FundsFreezeFailed` |
...
| 完成 | Topic                   | 事件              | 處理邏輯               | 事件發布               |
|----|-------------------------|-----------------|--------------------|--------------------|
|    | matching.trade-executed | `TradeExecuted` | 扣除凍結、計算手續費、進行雙分錄記帳，將 SPOT reserved 結轉至逐倉保證金，maker 成交退回超收手續費，發布保證金結轉事件 | `AccountEntryCreated` / `TradeMarginSettled` |
|    | positions.margin-released | `PositionMarginReleased` | 針對平倉成交，將逐倉保證金轉回 SPOT available，紀錄已實現盈虧 | `AccountEntryCreated` |
...
四張核心表：兩張帳戶表 (Accounts) + 兩張日記帳表 (Journals)。

### 平台總帳科目表 (`platform_accounts`)
...
(
    ...
    account_name    VARCHAR(128)    NOT NULL COMMENT '科目名稱 (如: 熱錢包, 手續費收入, 用戶存款負債)',
    
    -- 2. 會計屬性
    -- ASSET: 公司資產 (錢包)
    -- LIABILITY: 對用戶負債
    -- EQUITY: 公司權益 (風險準備金, 股本)
    -- REVENUE: 公司收入 (手續費)
    -- EXPENSE: 公司支出 (營運成本)
    category        VARCHAR(32)     NOT NULL COMMENT '類別: ASSET, LIABILITY, EQUITY, REVENUE, EXPENSE',
    asset           VARCHAR(20)     NOT NULL COMMENT '幣種 (如: USDT)',
...
(
    ...
    -- 1. 核心識別
    account_code    VARCHAR(32)     NOT NULL COMMENT '業務代碼: SPOT, MARGIN, FEE_EXPENSE, FUNDING_INCOME',
    account_name    VARCHAR(64)     NOT NULL COMMENT '顯示名稱: 現貨錢包, 手續費支出, 資金費收入',
    ...
    -- REVENUE: 用戶收入 (資金費, 返傭)
    -- EXPENSE: 用戶費用 (手續費, 利息)
    category        VARCHAR(20)     NOT NULL COMMENT '性質: ASSET, LIABILITY, EQUITY, REVENUE, EXPENSE',
    asset           VARCHAR(20)     NOT NULL COMMENT '幣種',
    
    -- 3. 資金
    balance         DECIMAL(30, 18) NOT NULL DEFAULT 0 COMMENT '總額 (各類別增加方向均為正數)',
    ...
    PRIMARY KEY (account_id),
    -- 確保一個用戶在同一種業務場景下的同一種幣，各類別帳戶是分開的
...
(
    ...
    INDEX idx_ref (reference_type, reference_id),
    INDEX idx_user_asset_time (user_id, asset, event_time DESC)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='用戶日記帳';
```

---

## 交易所帳務系統設計筆記 (Double-Entry Ledger System)

### 核心設計原則 (Core Principles)

本系統採用 **雙視角雙分錄 (Dual-View Double-Entry)** 架構。系統同時維護「平台」與「用戶」兩套獨立的帳本，兩者透過業務單號 (Reference ID) 邏輯關聯，但在資料庫層面完全解耦。

#### 1.1 會計恆等式 (The Accounting Equation)

無論是平台帳還是用戶帳，都必須時刻滿足以下恆等式：

$$資產 (Asset) + 費用 (Expense) = 負債 (Liability) + 權益 (Equity) + 收入 (Revenue)$$
- **移項後 (淨值觀點)**：$Equity (淨值) = Assets - Liabilities + (Revenue - Expense)$
    

#### 1.2 借貸法則 (Debit & Credit Rules)

**DEBIT (借方)** 與 **CREDIT (貸方)** 的增減定義適用於所有帳本：

|**要素 (Category)**|**DEBIT (借方)**|**CREDIT (貸方)**|**平台帳範例**|**用戶帳範例**|
|---|---|---|---|---|
|**資產 (ASSET)**|**增加 (+)**|減少 (-)|熱錢包、冷錢包|現貨餘額、保證金|
|**費用 (EXPENSE)**|**增加 (+)**|減少 (-)|行銷支出、返傭支出|手續費支出、融資利息|
|**負債 (LIABILITY)**|減少 (-)|**增加 (+)**|用戶存款總負債|逐倉借款 (融資)|
|**權益 (EQUITY)**|減少 (-)|**增加 (+)**|風險準備金、保留盈餘|投入本金、已實現盈虧|
|**收入 (REVENUE)**|減少 (-)|**增加 (+)**|手續費收入|資金費收入、返傭收入|
...
- **平台側 (`platform_journal`)**：
    ...
    - _說明：平台持有資產增加，但對用戶的負債也增加。_
        

#### 3.2 平台注入風險準備金 (Insurance Fund Injection)

平台將公司自有資金 5000 USDT 注入風險準備金（這屬於平台權益）。
- **平台側 (`platform_journal`)**：
    - 借 (Dr) **Asset** (準備金錢包) +5000
    - 貸 (Cr) **Equity** (風險準備金) +5000
    - _說明：這是平台內部的權益注資，不涉及用戶負債。_
- **用戶側**：無動作。
    

#### 3.3 下單預扣 (Order Freezing)

用戶下單，凍結 100 USDT。僅涉及用戶資產重分類。
- **用戶側 (`user_journal`)**：
    - 貸 (Cr) **Asset** (Available) 100
    - 借 (Dr) **Asset** (Reserved) 100
    - _說明：資產總額不變，僅狀態變更。_
        

#### 3.4 交易手續費 (Trading Fee)
...
#### 3.5 實現盈虧 (Realized PnL)

用戶平倉獲利 50 USDT (由虧損用戶或市場支付)。
- **用戶側 (`user_journal`)**：
    - 借 (Dr) **Asset** (保證金餘額) 50
...
### 報表能力 (Reporting Capabilities)

基於此架構，系統可直接產出以下報表：

1. 用戶個人損益表：
    
    SELECT SUM(amount) FROM user_journal WHERE user_id=? AND category IN ('REVENUE', 'EXPENSE')
    - 可精確顯示：手續費支出總計、資金費收入總計。
        
2. 用戶資產負債表：
    
    Asset (存款) vs Liability (借款) + Equity (本金+損益)。
    
3. 平台資產負債表：
    
    Asset (錢包餘額) vs Liability (用戶總資產) + Equity (準備金)。
    - **PoR (儲備證明)**：驗證 `Platform Asset >= Platform Liability`。
        
4. 平台損益表：
    
    直接統計 platform_accounts 中 REVENUE 與 EXPENSE 的餘額。
# Positions 服務
...
| 完成 | HTTP Method | Endpoint                                 | 說明                     | 調用者         | 授權    |
|----|-------------|------------------------------------------|------------------------|-------------|-------|
| ✔️ | POST        | `/api/positions/intent`                  | 判斷 LONG/SHORT 是否為加倉/平倉 | order-service | jwt   |
| ✔️ | GET         | `/api/positions/{userId}/{instrumentId}` | 即時倉位快照                 | risk   | private |
...
| 完成 | 發布場景           | Topic             | 事件名稱            |
|----|----------------|-----------------|-----------------|
|    | 倉位數量/價格/風險指標更新 | positions.updated | `PositionUpdated` |
|    | 平倉撮合後計算已實現損益與釋放保證金 | positions.margin-released | `PositionMarginReleased` |
|    | 平倉預鎖倉位成功        | positions.reserve | `PositionReserved` |
|    | 平倉預鎖倉位失敗        | positions.reserve | `PositionReserveRejected` |
...
| 完成 | Topic                   | 事件                   | 處理邏輯                | 事件發布          |
|----|-------------------------|----------------------|---------------------|----------------|
|    | matching.trade-executed | `TradeExecuted`      | 調整倉位數量、均價與未實現盈虧，計算釋放保證金與已實現損益並發布保證金釋放事件 | `PositionUpdated` / `PositionMarginReleased` |
|    | account.entry-created    | `AccountEntryCreated` | 依資產結算結果更新已實現盈虧並關閉倉位 | `PositionUpdated` |
|    | market.mark-price       | `MarkPriceUpdated`   | 以最新標記價格重算未實現盈虧與強平價  | `PositionUpdated` |
|    | account.trade-margin-settled | `TradeMarginSettled` | 建立/更新逐倉倉位，記錄 entry_price 與已入帳保證金，回寫倉位資訊 | `PositionUpdated` |
|    | positions.reserve-requested | `PositionReserveRequested` | 平倉前檢查可用倉位，鎖定 `closing_reserved_quantity`，發布 `PositionReserved` 或 `PositionReserveRejected` | `PositionReserved` / `PositionReserveRejected` |
...
- 用戶在調整槓桿時會把希望的倍數帶到 order → risk 的預檢流程；
- risk 會根據 instrument_metadata.max_leverage 與該交易對的risk_limits.max_leverage 驗證合法性，兩者都定義了每個商品/風險等級允許的最大槓桿 。
