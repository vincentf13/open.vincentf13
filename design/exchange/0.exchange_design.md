...
- **流程邏輯**:   
    1. **Call Precheck**: 呼叫上述 `/api/risk/precheck/leverage`，獲取 `allow` 和 `marginDelta`。若 `allow=false` 直接報錯。
    2. **決策分支**:
    - **分支 A：調低槓桿 (Need to Add Margin, `marginDelta > 0`)**
	    - Step 1 
		    - Position Service 必須先在自己的資料庫寫入一筆 `PENDING` 狀態的任務單。
		    - (Error Handling):  定期掃描 Position DB 中超過一定時間（如 10 秒）仍處於 `PENDING` 的任務。
				針對這些卡住的任務，去 Wallet Service 反查交易狀態：
				- **若錢包已扣款**：Position Service 繼續完成倉位更新，將任務改為 `SUCCESS`。
				- **若錢包未扣款**：視情況重試扣款或標記失敗。
        - **Step 2 (RPC):** 同步呼叫 Wallet Service `POST /internal/wallet/balance/transfer`。
        ...
        - **Step 3 (DB):** 若 Step 1 成功，開啟 DB Transaction：
            ...
            - 寫入 `PositionModifiedEvent`
        - **Step 3 (Error Handling):** 若 Step 2 DB 寫入失敗，發起 **補償請求** 呼叫 Wallet Service 把錢退回（Refund）。
    - **分支 B：調高槓桿 (Release Margin, `marginDelta < 0`)**
        - **Step 1 (DB):** 開啟 DB Transaction：
            ...
            - (Error Handling): 若 Step 2 失敗（極端情況，如網路超時），發送一個 MQ 消息或是寫入本地 `PendingRefund` 表，由後台 Job 無限重試直到錢加給用戶。**（絕對不能回滾 Step 1，否則用戶倉位變危險了錢還沒退出來，或者造成系統少錢）**。
        - **Step 2 (RPC):** 若 Step 1 成功，同步呼叫 Wallet Service。
            - Action: `ADD` (增加可用餘額)
            - Amount: `abs(marginDelta)`

### `POST /internal/wallet/balance/transfer`
...
- **Request**:
    ...
    - `refId`: string (冪等性 ID，建議用 `position_update_request_id`)
...
### `GET` `/internal/wallet/transaction/{refId}`

- 供 Position Service 的 Recovery Job 反查使用。
- 回傳該筆交易的最終狀態（已執行 / 不存在）。
## 事件定義: PositionModifiedEvent

既然槓桿改變導致了保證金改變，這個事件需要包含完整的狀態快照，供 Downstream (如 WebSocket 推送、Risk Engine 監控) 使用。
...
{
  "eventId": "uuid",
  "eventType": "LEVERAGE_CHANGED",
  "data": {
    "userId": "123",
    "instrumentId": "BTC-USDT",
    "oldLeverage": "10",
    "newLeverage": "20",
    "marginChange": "-50.00", // 負數代表釋放
    "currentMargin": "50.00",
    "currentLiquidationPrice": "95000.5",
...
1. **快速響應**:
    ...
    * 若:
    ...
    - **Intent = INCREASE**：
        - 立即以 `FREEZING_MARGIN` 狀態寫入 `orders` 表並回傳 `orderId`，再透過 Outbox 發布 `OrderSubmitted`。
        * `Risk-Margin` 消費 `OrderSubmitted` 完成預檢並發 `MarginPreCheckPassed`；
        * `Account-Ledger` 預扣保證金後發 `FundsFrozen`；
        * `Order` 消費 `FundsFrozen` 將訂單狀態改為 `NEW` 並推送 `OrderCreated` 至 `Matching`。
    - **Intent = REDUCE/CLOSE**：
        - 立即以 `LOCKING_POSITION` 狀態寫入 `orders` 表並回傳 `orderId`。
        ...
        * `Order` 服務新增的 Kafka Consumer 會：
            * 收到 `PositionReserved` 時更新訂單狀態為 `FREEZING_MARGIN` 並補發 `OrderSubmitted`（此時才允許進入風控/撮合流程）。
            * 收到 `PositionReserveRejected` 時立即將訂單標記為 `REJECTED`，原因寫入事件供報表/前端查詢使用。

3. **補償與失敗處理**:
    * 若風控或資金凍結失敗，`Order` 消費 `MarginPreCheckFailed`、`FundsFreezeFailed` 將訂單置為 `REJECTED`。平倉訂單若在倉位預凍結階段失敗，
...
1. **請求**: 用戶 `POST /orders` (BUY, 1 BTC, Open Intent)。    
2. **風控預檢**: Order Svc 呼叫 Risk Svc，計算 `required_margin` 與 `fee`。
3. **資金凍結**:
    - Order Svc 發布 `OrderSubmitted`。
    - Ledger Svc 消費並執行：`DEBIT Spot Available` -> `CREDIT Spot Reserved`。
...
8. 查詢接口 GET `/api/ledger/balances?userId=` 或使用 `risk_snapshots.equity` 快照評估`保證金餘額` `>`
...
### accout 消費 MarginPreCheckPassed
...
流程步驟：
- **請求**: 用戶 `POST /orders` (SELL, 1 BTC, Close Intent)。
- **倉位鎖定**:
    - Order Svc 判斷為平倉單，轉為 `LOCKING_POSITION` 狀態。
    - Order Svc 呼叫 Positions Svc 預鎖倉位 (`closing_reserved_quantity`)。
    - Positions Svc 回覆 `PositionReserved` (成功) 或 `Rejected`。
- **撮合**:
    - Order Svc 收到 `PositionReserved`，狀態轉 `NEW`，發送至 Matching Svc。
- **結算 (異步)**:
...
### position 消費`PositionReserveRequested`
...
## 反手流程 (The Flip - 單向持倉特有)

**場景**: 持有 10 多單，賣出 15 空單 (平 10 開 5)。

1. **拆單與計算**:
    - Order Svc 識別反手意圖。
    - 計算：(平倉 10 單釋放的保證金 + 錢包餘額) 是否 >= (開倉 5 單所需保證金)。
        
2. **原子鎖定 (Atomic Lock)**:
    - 同時執行：Position 鎖定 10 單位 + Ledger 凍結 (新保證金 - 預估釋放)。
    - 若任一不足，全單拒絕。
        
3. **撮合**:
    - Matching Svc 執行一筆 15 單位的 SELL 成交。
        
4. **複雜結算**:
    - Positions Svc 收到 15 SELL：先平掉 10 Long，再開 5 Short (Entry Price 更新為本次成交價)。
    - Ledger Svc：處理多退少補的資金劃轉。
...
| 完成 | HTTP Method | Endpoint                | 說明   | 調用者                          | 授權            | 輸入                                                              | 輸出                                                           | IO操作 |
|----|-------------|-------------------------|------|------------------------------|---------------|-----------------------------------------------------------------|--------------------------------------------------------------|------|
|    | POST        | `/api/orders`           | 建立委託 | gateway / liquidation-worker | jwt / private | OrderCreateRequest (instrument_id, side, type, price, quantity) | OrderResponse (orderId, status, intent, closingEntryPrice, ...) | 事件: Saga 最初 `PositionReserveRequested` / `OrderSubmitted`，最終 `OrderCreated`<br/>DB: insert `orders`<order_id,user_id,instrument_id,client_order_id,side,intent,closing_entry_price,type,status,price,quantity,filled_quantity,remaining_quantity,avg_fill_price,fee,version,created_at,updated_at><br/>服務: `POST /api/positions/intent` |
...
|----|----------------------------|---------------------------|---------------------------------------------|------------------|------|
| ✔️ | positions.reserved         | `PositionReserved`        | 預凍結成功 → 訂單標記為 `FREEZING_MARGIN` 並補發 `OrderSubmitted` | `OrderSubmitted` | DB: update `orders`<status, submitted_at, updated_at, version, closing_entry_price> |
| ✔️ | positions.reserve-rejected | `PositionReserveRejected` | 預凍結失敗 → 訂單 `REJECTED`                         | -                | DB: update `orders`<status, updated_at, version> |
|    | risk.margin-check-failed   | `MarginPreCheckFailed`    | 風控拒絕 → 訂單 `REJECTED`                          | -                | DB: update `orders`<status, updated_at, version> |
|    | ledger.funds-frozen        | `FundsFrozen`             | 資金凍結成功 → 訂單 `NEW` 並發布 `OrderCreated`          | `OrderCreated`   | DB: update `orders`<status, submitted_at, updated_at, version> |
|    | ledger.funds-freeze-failed | `FundsFreezeFailed`       | 資金凍結失敗 → 訂單 `REJECTED`                        | -                | DB: update `orders`<status, updated_at, version> |
|    | ledger.funds-unfrozen      | `FundsUnfrozen`           | 訂單狀態改為 `CANCELLED`                          | -                | DB: update `orders`<status, updated_at, version> |
...
| 完成  | Topic                        | 事件                       | 處理邏輯               | 事件發布                                 | IO操作 |
| --- | ---------------------------- | ------------------------ | ------------------ | ------------------------------------ |------|
| ✔️  | risk.margin-check-passed     | `MarginPreCheckPassed`   | 凍結保證金並記錄資產變動       | `FundsFrozen`<br>`FundsFreezeFailed` | DB: insert `ledger_entries`<freeze><br/>update `accounts`<reserved_balance> |
...
|     | ~~matching.order-cancelled~~ | ~~`OrderCancelled`~~     | ~~解凍對應資金並回補餘額~~    | ~~`FundsUnfrozen`~~                  | ~~DB: update `accounts`<reserved_balance>~~ |
|     | ~~market.funding-rate~~      | ~~`FundingRateUpdated`~~ | ~~依資金費率結算多空支付~~    | ~~`LedgerEntryCreated`~~             | ~~DB: insert `ledger_entries`<funding>~~ |
...
|     | POST        | `/api/positions/{instrumentId}/leverage` | 調整倉位槓桿倍數               | risk-margin   | private | path instrumentId、body: targetLeverage（userId 取自 JWT）                | PositionLeverageResponse（appliedLeverage, effectiveAt）                                    | DB: update `positions`<leverage, updated_at><br/>服務: 呼叫 `/api/risk/precheck/leverage` |
...
| 完成  | Topic                            | 事件                         | 處理邏輯                                                       | 事件發布                                           | IO操作 |
| --- | -------------------------------- | -------------------------- | ---------------------------------------------------------- | ---------------------------------------------- |------|
| ✔️  | matching.trade-executed          | `TradeExecuted`            | 調整倉位數量、均價與未實現盈虧                                            | `PositionUpdated` / `PositionClosed`           | DB: update `positions`<quantity, entry_price, unrealized_pnl, liquidation_price> |
| ✔️  | order.position-reserve-requested | `PositionReserveRequested` | 平倉前凍結倉位 (userId, instrumentId, side, quantity, intentType) | `PositionReserved` / `PositionReserveRejected` | DB: update `positions`<closing_reserved_quantity> |
|     | ledger.entry-created             | `LedgerEntryCreated`       | 依資產結算結果更新已實現盈虧並關閉倉位                                        | `PositionUpdated` / `PositionClosed`           | DB: update `positions`<realized_pnl, status> |
...
| 完成  | HTTP Method | Endpoint                          | 說明             | 調用者       | 授權      | 輸入                      | 輸出                       | IO操作 |
| --- | ----------- | --------------------------------- | -------------- | --------- | ------- | ----------------------- | ------------------------ | ---- |
| ️   | POST        | `/api/risk/precheck/leverage`     | 槓桿調整預檢，回傳允許與缺口 | positions | private | LeveragePrecheckRequest | LeveragePrecheckResponse | -    |
...
| 完成 | Topic             | 事件                 | 處理邏輯           | 事件發布                                            | IO操作 |
|----|-------------------|--------------------|----------------|-------------------------------------------------|------|
|    | order.submitted   | `OrderSubmitted`   | 根據委託資訊試算保證金與風險 | `MarginPreCheckPassed` / `MarginPreCheckFailed` | - |
