...
|----|----------------------------|---------------------------|---------------------------------------------|------------------|------|
| ✔️ | positions.reserved         | `PositionReserved`        | 預凍結成功 → 訂單 `SUBMITTED` 並補發 `OrderSubmitted` | `OrderSubmitted` | DB: update `orders`<status, submitted_at, updated_at, version, closing_entry_price> |
| ✔️ | positions.reserve-rejected | `PositionReserveRejected` | 預凍結失敗 → 訂單 `FAILED`                         | -                | DB: update `orders`<status, updated_at, version> |
|    | risk.margin-check-failed   | `MarginPreCheckFailed`    | 風控拒絕 → 訂單 `FAILED`                          | -                | DB: update `orders`<status, updated_at, version> |
|    | ledger.funds-frozen        | `FundsFrozen`             | 資金凍結成功 → 訂單 `ACCEPTED` 並發布 `OrderCreated`   | `OrderCreated`   | DB: update `orders`<status, submitted_at, updated_at, version> |
|    | ledger.funds-freeze-failed | `FundsFreezeFailed`       | 資金凍結失敗 → 訂單 `FAILED`                        | -                | DB: update `orders`<status, updated_at, version> |
