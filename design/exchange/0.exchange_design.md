...
|     | **Topic**                      | **事件名稱**                   | **處理邏輯**                                                                                                                                   |
| --- | ------------------------------ | -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ |
|     | `positions.reserve-requested`  | `PositionReserveRequested` | **平倉預檢**：檢查 `quantity - closing_reserved` 是否足夠。成功則增加 `closing_reserved_quantity` 並發布 `Reserved` 事件。<br>失敗則發布 `PositionReserveRejected` 事件。 |
...
|     | `positions.reserve-rejected` | `PositionReserveRejected` | 平倉前，可用持倉不足，拒絕鎖定              |
