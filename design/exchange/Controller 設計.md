- Controller 嚴格實現 `sdk-contract` 中定義的 exchange-{service name}-rest-api 定義的 `OpenAPI` 介面，因為 client 端在 exchange-{service name}-rest-client中繼承此介面，自動實現 client 端的接口調用。 
