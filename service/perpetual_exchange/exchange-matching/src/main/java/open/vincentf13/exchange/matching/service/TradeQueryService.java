...
public class TradeQueryService {
  ...
  public List<TradeResponse> listByOrderId(@NotNull Long userId, @NotNull Long orderId) {
    List<Trade> trades =
        tradeRepository.findBy(
            Wrappers.lambdaQuery(TradePO.class)
                .and(
                    wrapper ->
                        wrapper
                            .eq(TradePO::getMakerUserId, userId)
  ...
  @Transactional(readOnly = true)
  public List<TradeResponse> listByInstrument(@NotNull Long userId, @NotNull Long instrumentId) {
    List<Trade> trades =
        tradeRepository.findBy(
            Wrappers.lambdaQuery(TradePO.class)
                .and(
    ...
    Comparator<Trade> comparator =
        Comparator.comparing((Trade trade) -> instrumentId.equals(trade.getInstrumentId()) ? 0 : 1)
