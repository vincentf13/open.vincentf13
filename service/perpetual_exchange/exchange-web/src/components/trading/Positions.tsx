...
const parsePayload = (p?: string | null) => {
    if (!p) return null;
    try {
        return JSON.parse(p);
    } catch {
...
export default function Positions({
    ...
    const renderCalculatorTooltip = (p: any, key: string, parentP?: any, eventContext?: any) => {
        ...
        if (key === 'entryPrice') {
            if (latestPayload && prevPayload && latestEvent.eventType === 'TRADE_EXECUTED') {
                ...
                content = (
                    <div className="flex flex-col gap-1">
                        <div className="font-bold text-slate-800 border-b border-slate-900/10 pb-1 mb-1">Entry Price
                            Transition
                        </div>
                        <div className="text-[9px] text-slate-500 mb-1">Last Trade (Seq {latestEvent.sequenceNumber}):
                        </div>
                ...
                content = (
                    <div className="flex flex-col gap-1">
                        ...
                        <div className="text-[10px] text-slate-600 italic">Expand row to load event history for detailed
                            trace.
                        </div>
            ...
            content = rl ? (
                <div className="flex flex-col gap-1">
                    ...
                    <div className="grid grid-cols-[auto_1fr] gap-x-2 gap-y-1 text-[10px] text-slate-600">
                        ...
                        className="font-mono text-right">{formatNumber(entry)} × {formatNumber(qty)} × {contractSize} = {formatNumber(notional)}</span>
            ...
            content = (
                <div className="flex flex-col gap-1">
                    <div className="font-bold text-slate-800 border-b border-slate-900/10 pb-1 mb-1">Margin Ratio Calc
                    </div>
                    <div className="grid grid-cols-[auto_1fr] gap-x-2 gap-y-1 text-[10px] text-slate-600">
                        <span>Equity:</span> <span
                        className="font-mono text-right">{formatNumber(margin)} + {formatNumber(upnl)} = {formatNumber(equity)}</span>
                        <span>Notional:</span> <span
                        className="font-mono text-right">{formatNumber(mark)} × {formatNumber(qty)} × {contractSize} = {formatNumber(notional)}</span>
                    </div>
                    <div className="mt-1 font-mono text-[10px] text-blue-700 bg-slate-900/5 p-1.5 rounded">
            ...
            content = (
                <div className="flex flex-col gap-1">
                    <div className="font-bold text-slate-800 border-b border-slate-900/10 pb-1 mb-1">Unrealized PnL
                        Calc
                    </div>
                    <div className="grid grid-cols-[auto_1fr] gap-x-2 gap-y-1 text-[10px] text-slate-600">
                        <span>Price Diff:</span> <span
                        className={`font-mono text-right ${diff >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>{side === 'LONG' || side === 'BUY' ? `${formatNumber(mark)} - ${formatNumber(entry)}` : `${formatNumber(entry)} - ${formatNumber(mark)}`} = {formatNumber(diff)}</span>
                        <span>Volume:</span> <span
                        className="font-mono text-right">{formatNumber(qty)} × {contractSize}</span>
                    </div>
                    <div className="mt-1 font-mono text-[10px] text-blue-700 bg-slate-900/5 p-1.5 rounded">
            ...
            if (latestPayload && prevPayload) {
                ...
                content = (
                    <div className="flex flex-col gap-1">
                        <div className="font-bold text-slate-800 border-b border-slate-900/10 pb-1 mb-1">Implied Trade
                            Fee
                        </div>
                        <div className="grid grid-cols-[auto_1fr] gap-x-2 gap-y-1 text-[10px] text-slate-600">
                        ...
                        <div
                            className="mt-1 bg-slate-900/5 p-1.5 rounded border border-slate-900/10 font-mono text-[10px] text-blue-700">
                            {formatNumber(curF)} - {formatNumber(prevF)} = <span
                            className="text-rose-600 font-bold">+{formatNumber(deltaF)}</span>
                        </div>
                        <div className="text-[8px] text-slate-400 italic mt-0.5 text-right">Fee charged in this event
                        </div>
                    </div>
                );
            } else {
            ...
            if (latestPayload && prevPayload) {
                ...
                if (deltaQ > 0) {
                    ...
                    content = (
                        <div className="flex flex-col gap-1">
                            <div className="font-bold text-slate-800 border-b border-slate-900/10 pb-1 mb-1">Implied PnL
                                & Fee Breakdown
                            </div>
                            <div
                                className="grid grid-cols-[1fr_auto] gap-x-4 text-[9px] text-slate-500 border-b border-slate-900/10 pb-1">
                                <span>Close Price:</span> <span
                                className="font-mono text-slate-700">{formatNumber(exitP)}</span>
                                <span>Avg Entry:</span> <span
                                className="font-mono text-slate-700">{formatNumber(prevE)}</span>
                                <span>Closed Qty:</span> <span
                                className="font-mono text-slate-700">{formatNumber(deltaQ)}</span>
                            </div>

                            <div
                                className="bg-slate-900/5 p-1.5 rounded border border-slate-900/10 font-mono text-[9px] text-blue-800 mt-1">
                                ...
                                <div className="flex justify-between border-t border-slate-900/5 pt-0.5">
                                    ...
                                    <span>
                                      = <span
                                        className={`font-bold ${deltaR >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>{deltaR >= 0 ? '+' : ''}{formatNumber(deltaR)}</span>
                                  </span>
                                </div>
                            </div>
                        </div>
                ...
                content = (
                    <div className="flex flex-col gap-1">
                        <div className="font-bold text-slate-800 border-b border-slate-900/10 pb-1 mb-1">Cum Realized
                            PnL
                        </div>
            ...
            content = (
                <div className="flex flex-col gap-1">
                    <div className="font-bold text-slate-800 border-b border-slate-900/10 pb-1 mb-1">Liquidation Price
                        Calc
                    </div>
                    <div className="grid grid-cols-[auto_1fr] gap-x-2 gap-y-1 text-[10px] text-slate-600">
                        <span>Entry:</span> <span className="font-mono text-right">{formatNumber(entry)}</span>
                        <span>Margin/Unit:</span> <span
                        className="font-mono text-right">{formatNumber(margin)} / ({formatNumber(qty)}×{contractSize}) = {formatNumber(marginPerUnit)}</span>
                        <span>MMR Factor:</span> <span
                        className="font-mono text-right">1 {side === 'LONG' || side === 'BUY' ? '-' : '+'} {mmr} = {denominator}</span>
                    </div>
                    <div className="mt-1 font-mono text-[10px] text-blue-700 bg-slate-900/5 p-1.5 rounded">
        ...
        try {
            ...
            if (String(res?.code) === '0') {
                ...
                next.forEach((p: any) => {
                    ...
                    positionHighlightKeys.forEach(k => {
                        if (!prev || getCompareValue(p, k) !== getCompareValue(prev, k)) changed.add(k);
                    });
        ...
        try {
            ...
            if (String(res?.code) === '0') {
                ...
                next.forEach((o: any) => {
                    ...
                    const prev = prevMap.get(id);
                    const changed = new Set<string>();
                    orderHighlightKeys.forEach(k => {
                        if (!prev || getCompareValue(o, k) !== getCompareValue(prev, k)) changed.add(k);
                    });
                    if (changed.size > 0) changeMap.set(id, changed);
                });
...
// ... (rest of file)
    ...
    useEffect(() => {
        getCurrentUser().then(res => {
            if (String(res?.code) === '0' && res.data?.id) setCurrentUserId(String(res.data.id));
        }).catch(() => {
        });
    }, []);
    ...
    const renderCellValue = (p: any, k: string, parentP?: any, eventContext?: any) => {
        ...
        if (k === 'positionId') {
            ...
            content = (
                <span
                    className={`px-1.5 py-0.5 rounded-md border text-[9px] font-black uppercase tracking-tighter ${colorClass}`}>
          {status}
        </span>
            );
        } else if (k === 'side') {
          ...
          {side}
            ...
            content = <span
                className="text-sky-600 font-bold">{k === 'marginRatio' ? formatPercent(v) : formatNumber(v)}</span>;
        ...
        if (['margin', 'marginRatio', 'unrealizedPnl', 'liquidationPrice', 'cumFee', 'cumRealizedPnl'].includes(k)) {
            if (k === 'margin') {
                return <div className="flex items-center justify-end gap-1">{content}<span
                    className="text-[9px] text-slate-400">+</span>{renderCalculatorTooltip(p, k, parentP, eventContext)}
                </div>;
            }
            return <div
                className="flex items-center justify-end gap-1">{content}{renderCalculatorTooltip(p, k, parentP, eventContext)}</div>;
        }
    ...
    const renderOrderCellValue = (o: any, k: string, tradesMap?: Record<string, TradeResponse[]>) => {
        ...
        if (k === 'orderId') {
          ...
          {side}
            ...
            content = (
                <span
                    className="px-1.5 py-0.5 rounded-md border border-violet-200 bg-violet-50 text-violet-600 text-[9px] font-black uppercase tracking-tighter">
          {String(v || '-')}
        </span>
            );
        } else if (k === 'intent') {
            content = (
                <span
                    className="px-1.5 py-0.5 rounded-md border border-slate-200 bg-slate-100 text-slate-600 text-[9px] font-black uppercase tracking-tighter">
          {String(v || '-')}
        </span>
            );
        ...
        if ((k === 'avgFillPrice' || k === 'fee') && tradesMap && tradesMap[o.orderId]) {
            ...
            if (trades.length > 0) {
                ...
                if (k === 'avgFillPrice') {
                    ...
                    tooltipContent = (
                        <div className="flex flex-col gap-1">
                            <div className="font-bold text-slate-800 border-b border-slate-900/10 pb-1 mb-1">Implied Avg
                                Price
                            </div>
                            <div className="text-[9px] text-slate-600 mb-1">From {trades.length} fills:</div>
                            <div
                                className="bg-slate-900/5 p-1.5 rounded border border-slate-900/10 font-mono text-[9px] text-blue-700">
                                <div>Σ(Price × Qty) / Σ(Qty)</div>
                                <div className="mt-1 font-bold">= {formatNumber(calcAvg)}</div>
                            </div>
                            <div className="text-[8px] text-slate-400 italic mt-0.5 text-right">*Implied from loaded
                                fills
                            </div>
                        </div>
                    );
    ...
    useEffect(() => {
        if (expandedOrderId) {
            ...
            getOrderEvents(expandedOrderId).then(res => {
                if (String(res?.code) === '0') setOrderEvents(v => ({...v, [expandedOrderId]: res.data?.events || []}));
            }).finally(() => setOrderEventsLoading(v => ({...v, [expandedOrderId]: false})));
            getTradesByOrderId(expandedOrderId).then(res => {
                if (String(res?.code) === '0') setOrderTrades(v => ({...v, [expandedOrderId]: res.data || []}));
            });
        }
    ...
    function toggleOrder(id: number) {
        ...
        if (!orderEvents[id]) {
            ...
            getTradesByOrderId(id).then(res => {
                if (String(res?.code) === '0') setOrderTrades(v => ({...v, [id]: res.data || []}));
            });
        }
    }
                ...
                <div className="inline-flex items-center gap-1 rounded-full border border-white/20 bg-white/5 p-1">
                    {['Positions', 'Orders', 'Traders'].map((tab) => (
                        <button key={tab} onClick={() => setActiveTab(tab)}
                                className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider transition-all ${tab === activeTab ? 'bg-white text-slate-800 shadow-sm' : 'text-slate-500 hover:text-slate-700 hover:bg-white/20'}`}>
                            {tab}
                        </button>
                    ))}
                </div>
                    ...
                    <div className="min-w-max">
                        <table className="w-full text-xs text-right text-slate-600 border-separate border-spacing-x-0">
                                                    ...
                                                    title={
                                                        c.key === 'margin' ? (
                                                                    <div className="flex flex-col gap-2 p-1">
                                                                        ...
                                                                        <div
                                                                            className="text-[9px] text-slate-500 font-medium italic">
                                                                ...
                                                                c.key === 'marginRatio' ? (
                                                                        <div className="flex flex-col gap-2 p-1">
                                                                            ...
                                                                            <div className="text-[10px] text-slate-600">
                                                                                Current position equity relative to notional
                                                                                value.
                                                                            </div>
                                                                            <div
                                                                                className="mt-1 bg-slate-900/5 p-1.5 rounded border border-slate-900/10 font-mono text-[10px] text-blue-700">
                                                                    ...
                                                                    c.key === 'unrealizedPnl' ? (
                                                                            <div className="flex flex-col gap-2 p-1">
                                                                                <div
                                                                                    className="font-bold text-slate-800">Unrealized
                                                                                    PnL
                                                                                </div>
                                                                                <div className="text-[10px] text-slate-600">
                                                                                    Floating profit/loss based on Mark
                                                                                    Price.
                                                                                </div>
                                                                                <div
                                                                                    className="flex flex-col gap-1 mt-1 bg-slate-900/5 p-1.5 rounded border border-slate-900/10">
                                                                                    <div
                                                                                        className="flex justify-between gap-3 text-[10px]">
                                                                                        <span
                                                                                            className="text-emerald-700 font-bold">Long</span>
                                                                                        <span
                                                                                            className="font-mono text-slate-700">(Mark - Entry) × Qty × Contract Size</span>
                                                                                    </div>
                                                                                    <div
                                                                                        className="flex justify-between gap-3 text-[10px] border-t border-slate-900/5 pt-1">
                                                                                        <span
                                                                                            className="text-rose-700 font-bold">Short</span>
                                                                                        <span
                                                                                            className="font-mono text-slate-700">(Entry - Mark) × Qty × Contract Size</span>
                                                                                    </div>
                                                                                </div>
                                                                            </div>
                                                                        ) :
                                                                        c.key === 'cumRealizedPnl' ? (
                                                                                <div className="flex flex-col gap-2 p-1">
                                                                                    <div
                                                                                        className="font-bold text-slate-800">Cum
                                                                                        Realized PnL
                                                                                    </div>
                                                                                    <div
                                                                                        className="text-[10px] text-slate-600">
                                                                                        Total realized profit/loss including
                                                                                        fees.
                                                                                    </div>
                                                                                    <div
                                                                                        className="mt-1 bg-slate-900/5 p-1.5 rounded border border-slate-900/10 font-mono text-[10px] text-blue-700">
                                                                                    ...
                                                                                    <div
                                                                                        className="flex items-start gap-1.5 mt-1 p-1.5 bg-amber-500/10 border border-amber-600/20 rounded text-[9px] text-amber-800">
                                                                                        <span
                                                                                            className="text-amber-600 font-bold">!</span>
                                                                            ...
                                                                            c.key === 'liquidationPrice' ? (
                                                                                <div
                                                                                    ...
                                                                                    <div
                                                                                        className="font-bold text-slate-800">Liquidation
                                                                                        Price
                                                                                    </div>
                                                                                    <div
                                                                                        className="text-[10px] text-slate-600">
                                                                                        Price at which position hits
                                                                                        Maintenance Margin.
                                                                                    </div>
                                                                                    <div
                                                                                        className="flex flex-col gap-1 mt-1 bg-slate-900/5 p-1.5 rounded border border-slate-900/10">
                                                                                        <div
                                                                                            ...
                                                                                            <span
                                                                                                className="text-emerald-700 font-bold">Long</span>
                                                                                            <span
                                                                                                className="font-mono text-slate-700">(Entry - Margin/Qty) / (1 - MMR)</span>
                                                                                        </div>
                                                                                        <div
                                                                                            className="flex justify-between gap-3 text-[10px] border-t border-slate-900/5 pt-1">
                                                                                            <span
                            ...
                            {positions.length === 0 ? <tr>
                                <td colSpan={columns.length + 1} className="py-8 text-center text-slate-400">No
                                    positions
                                </td>
                            </tr> : positions.map(p => (
                                <Fragment key={p.positionId}>
                                    <tr className="hover:bg-white/10 transition-colors">
                                        <td className="py-2 px-2">
                                            <button onClick={() => togglePosition(p.positionId)}
                                                    className="w-4 h-4 rounded border border-white/30 text-[10px]">{expandedPositionId === p.positionId ? 'v' : '>'}</button>
                                        </td>
                                        {columns.map(c => {
                                            const changed = positionChangedFieldsRef.current.get(String(p.positionId))?.has(c.key);
                                            return <td key={c.key}
                                                       className={`py-2 px-2 font-mono ${changed ? 'bg-rose-100/50 text-rose-900' : ''}`}>{renderCellValue(p, c.key)}</td>
                                        })}
                                    </tr>
                                    {expandedPositionId === p.positionId && (
                                        <tr>
                                            <td colSpan={columns.length + 1} className="p-2">
                                                <div
                                                    className="bg-slate-50/30 rounded-xl p-3 border border-white/20 overflow-x-auto">
                                                    <div
                                                        className="text-[9px] uppercase font-bold text-slate-400 mb-2 text-left">Position
                                                        Events
                                                    </div>
                                                    <table
                                                        className="w-full text-[10px] text-left border-separate border-spacing-x-0">
                                                        <thead>
                                                        <tr className="text-slate-500">
                                                            ...
                                                            <th className="py-1 px-2 whitespace-nowrap font-bold">Trade
                                                                Id
                                                            </th>
                                                            {columns.map((c, i) => (
                                                                <th key={c.key}
                                                                    className={`py-1 px-2 whitespace-nowrap font-bold text-slate-600 bg-yellow-200/50 ${i === 0 ? 'rounded-l-md' : ''} ${i === columns.length - 1 ? 'rounded-r-md' : ''}`}>{c.label}</th>
                                                            ))}
                                            ...
                                            <span
                                              ...
                                              {e.eventType}
                                                                        ...
                                                                        {columns.map((c, i) => {
                                                                            ...
                                                                            return (
                                                                                <td key={c.key}
                                                                                    className={`py-1 px-2 font-mono bg-yellow-200/50 ${hasValue ? 'text-slate-900 font-bold' : 'text-slate-300'} ${i === 0 ? 'rounded-l-md' : ''} ${i === columns.length - 1 ? 'rounded-r-md' : ''}`}>
                ...
                {activeTab === 'Orders' && (
                    <div className="liquid-tooltip-trigger min-w-max">
                        <table className="w-full text-xs text-right text-slate-600 border-separate border-spacing-x-0">
                            ...
                            <tr className="text-[10px] uppercase text-slate-400 border-b border-white/10">
                                <th className="py-2 px-2"></th>
                                {orderColumns.map(c => <th key={c.key}
                                                           className="py-2 px-2 font-semibold">{c.label}</th>)}
                            </tr>
                            </thead>
                            <tbody className="divide-y divide-white/5">
                            {orders.length === 0 ? <tr>
                                <td colSpan={orderColumns.length + 1} className="py-8 text-center text-slate-400">No
                                    orders
                                </td>
                            </tr> : orders.map(o => (
                                <Fragment key={o.orderId}>
                                    <tr className="hover:bg-white/10 transition-colors">
                                        <td className="py-2 px-2">
                                            <button onClick={() => toggleOrder(o.orderId)}
                                                    className="w-4 h-4 rounded border border-white/30 text-[10px]">{expandedOrderId === o.orderId ? 'v' : '>'}</button>
                                        </td>
                                        {orderColumns.map(c => {
                                            const changed = orderChangedFieldsRef.current.get(String(o.orderId))?.has(c.key);
                                            return <td key={c.key}
                                                       className={`py-2 px-2 font-mono ${changed ? 'bg-rose-100/50 text-rose-900' : ''}`}>{renderOrderCellValue(o, c.key, orderTrades)}</td>
                                        })}
                                    </tr>
                                            ...
                                            <td colSpan={orderColumns.length + 1} className="p-2">
                                                <div className="space-y-3">
                                                    <div
                                                        className="bg-slate-50/30 rounded-xl p-3 border border-white/20 overflow-x-auto">
                                                        <div
                                                            className="text-[9px] uppercase font-bold text-slate-400 mb-2 text-left">Order
                                                            Events
                                                        </div>
                                                        <table
                                                            className="w-full text-[10px] text-left border-separate border-spacing-x-0">
                                                            ...
                                                            <tr className="text-slate-500">
                                                                ...
                                                                {orderColumns.map((c, i) => (
                                                                    <th key={c.key}
                                                                        className={`py-1 px-2 whitespace-nowrap font-bold text-slate-600 bg-yellow-200/50 ${i === 0 ? 'rounded-l-md' : ''} ${i === orderColumns.length - 1 ? 'rounded-r-md' : ''}`}>{c.label}</th>
                                                                ))}
                                                                <th className="py-1 px-2 whitespace-nowrap font-bold">Time</th>
                                                            </tr>
                                                            </thead>
                                            ...
                                            <span
                                              ...
                                              {e.eventType}
                                                                        ...
                                                                        {orderColumns.map((c, i) => {
                                                                            ...
                                                                            return (
                                                                                <td key={c.key}
                                                                                    className={`py-1 px-2 font-mono bg-yellow-200/50 ${hasValue ? 'text-slate-900 font-bold' : 'text-slate-300'} ${i === 0 ? 'rounded-l-md' : ''} ${i === orderColumns.length - 1 ? 'rounded-r-md' : ''}`}>
                                                    ...
                                                    <div
                                                        className="bg-slate-50/30 rounded-xl p-3 border border-white/20">
                                                        <div
                                                            className="text-[9px] uppercase font-bold text-slate-400 mb-2 text-left">Fills
                                                            / Trades
                                                        </div>
                ...
                {activeTab === 'Traders' && (
                    <div className="min-w-max">
                        {(() => {
                            ...
                            return (
                                <table
                                    className="w-full text-xs text-right text-slate-600 border-separate border-spacing-x-0">
                                    <thead>
                                    <tr className="text-[10px] uppercase text-slate-400 border-b border-white/10">
                                        ...
                                        <th className={`py-2 px-2 ${takerHeaderBg} ${hasMyTaker ? 'rounded-tl-md' : ''}`}>Taker
                                            User
                                        </th>
                                        ...
                                        <th className={`py-2 px-2 ${takerHeaderBg} ${hasMyTaker ? 'rounded-tr-md' : ''}`}>Taker
                                            Filled
                                        </th>

                                        <th className={`py-2 px-2 ${makerHeaderBg} ${hasMyMaker ? 'rounded-tl-md' : ''}`}>Maker
                                            User
                                        </th>
                                        ...
                                        <th className={`py-2 px-2 ${makerHeaderBg} ${hasMyMaker ? 'rounded-tr-md' : ''}`}>Maker
                                            Filled
                                        </th>
                                    ...
                                    <tbody className="divide-y divide-white/5">
                                    {instrumentTrades.length === 0 ? <tr>
                                        <td colSpan={22} className="py-8 text-center text-slate-400">No trades</td>
                                    </tr> : instrumentTrades.map(t => {
                                        const isTaker = t.takerUserId === Number(currentUserId);
                            ...
                            <span
                              ...
                              <span
                                  className={`px-1.5 py-0.5 rounded-md border text-[9px] font-black uppercase tracking-tighter ${t.counterpartyOrderSide === 'BUY' ? 'bg-emerald-50 text-emerald-600 border-emerald-200' : 'bg-rose-50 text-rose-600 border-rose-200'}`}>
                            ...
                            <span
                              ...
                              <span
                                  className={`px-1.5 py-0.5 rounded-md border text-[9px] font-black uppercase tracking-tighter ${t.orderSide === 'BUY' ? 'bg-emerald-50 text-emerald-600 border-emerald-200' : 'bg-rose-50 text-rose-600 border-rose-200'}`}>
                                {t.orderSide}
                              </span>
                                                </td>
                                                <td className={`py-2 px-2 ${makerBg}`}>
            ...
            {tradeDetail && (
                <div className="fixed inset-0 z-50 bg-slate-900/30" onClick={() => setTradeDetail(null)}>
                    ...
                    >
                        ...
                        {tradeDetailLoading ? (
                            ...
                            <div className="grid grid-cols-[auto_1fr] gap-x-3 gap-y-1">
                                ...
                                className="font-mono">{tradeDetail.tradeId}</span>
                                <span
                                    className="text-slate-400">Instrument</span><span>{instrumentMap.get(String(tradeDetail.instrumentId)) || tradeDetail.instrumentId}</span>
                                <span className="text-slate-400">Maker Order ID</span>
                                <span
                                    className={`font-mono ${String(tradeDetail.makerUserId) === String(currentUserId) ? 'bg-amber-100/70 rounded-sm' : ''}`}>
                  {tradeDetail.orderId}
                </span>
                                <span className="text-slate-400">Taker Orde ID</span>
                                <span
                                    className={`font-mono ${String(tradeDetail.takerUserId) === String(currentUserId) ? 'bg-amber-100/70 rounded-sm' : ''}`}>
                  {tradeDetail.counterpartyOrderId}
                </span>
                                <span className="text-slate-400">Price</span><span
                                className="font-mono text-sky-600 font-bold">{formatNumber(tradeDetail.price)}</span>
                                <span className="text-slate-400">Quantity</span><span
                                className="font-mono text-sky-600 font-bold">{formatNumber(tradeDetail.quantity)}</span>
                  ...
                  {formatNumber(tradeDetail.takerFee)}
                                ...
                                className="font-mono">{formatPayloadValue('executedAt', tradeDetail.executedAt)}</span>
                            </div>
                        )}
                    </div>
                </div>
