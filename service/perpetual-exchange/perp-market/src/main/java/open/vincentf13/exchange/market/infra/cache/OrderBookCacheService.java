...
public class OrderBookCacheService {
    ...
    public void applyUpdates(Long instrumentId,
        ...
        Map<Long, OrderState> instrumentOrders =
                orderStates.computeIfAbsent(instrumentId, k -> new ConcurrentHashMap<>());
        ...
        for (OrderUpdate update : updates) {
            if (update.getRemainingQuantity().compareTo(BigDecimal.ZERO) <= 0) {
                ...
                instrumentOrders.put(
                        update.getOrderId(),
                        new OrderState(update.getPrice(), update.getRemainingQuantity(), update.getSide()));
            }
        }
    ...
    private void rebuildL2(Long instrumentId,
                           Map<Long, OrderState> orders,
                           Instant updatedAt) {
    ...
    private record OrderState(
            BigDecimal price,
            BigDecimal quantity,
            open.vincentf13.exchange.common.sdk.enums.OrderSide side) {
    }
}
