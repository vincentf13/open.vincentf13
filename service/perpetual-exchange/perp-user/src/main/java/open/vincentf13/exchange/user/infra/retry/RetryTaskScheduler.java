...
public class RetryTaskScheduler {
    ...
    public void retryAuthCredentialTasks() {
        ...
        for (RetryTaskPO task : tasks) {
            ...
            switch (taskType) {
                case AUTH_CREDENTIAL_CREATE -> retryTaskService.handleTask(
                        task,
                        retryDelay,
                        retryTask -> {
                            return userCommandService.createAuthCredential(
                                    OpenObjectMapper.fromJson(
                                            retryTask.getPayload(), AuthCredentialCreatePayload.class));
                        });
            }
        }
    }
