...
/**
 In-memory cache for instrument configuration data. This cache is populated at application startup
 by fetching all instruments from the Admin service.
 */
@Component
public class InstrumentCache {
    ...
    public static BigDecimal requireContractSize(InstrumentCache cache,
                                                 Long instrumentId) {
        return cache
                       .get(instrumentId)
    ...
    public static Integer requireDefaultLeverage(InstrumentCache cache,
                                                 Long instrumentId) {
        return cache
                       .get(instrumentId)
                       ...
                       .orElseThrow(
                               () ->
                                       new IllegalStateException(
                                               "Instrument cache missing or invalid defaultLeverage for instrumentId="
                                                       + instrumentId));
    }
     ...
     */
    public void put(Long instrumentId,
