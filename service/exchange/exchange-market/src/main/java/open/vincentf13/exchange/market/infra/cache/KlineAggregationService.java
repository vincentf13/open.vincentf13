...
public class KlineAggregationService {
  ...
  private static final Set<KlinePeriod> SUPPORTED_PERIODS =
      EnumSet.of(
  ...
  private void finalizeBucket(KlineKey key, KlineBucket bucket) {
    ...
    klineBucketRepository.updateSelectiveBy(
        bucket,
  ...
  private BigDecimal findPreviousClose(KlineBucket bucket, KlinePeriod period) {
    ...
    return klineBucketRepository
        .findOne(
            Wrappers.<KlineBucketPO>lambdaQuery()
                .eq(KlineBucketPO::getInstrumentId, bucket.getInstrumentId())
  ...
  private KlineBucket getOrLoadBucket(Long instrumentId, KlinePeriod period, Instant bucketStart) {
    ...
    return activeBuckets.compute(
        key,
        (k, current) -> {
          if (current != null && bucketStart.equals(current.getBucketStart())) {
            return current;
          }
          Optional<KlineBucket> persisted =
              klineBucketRepository.findOne(
                  Wrappers.<KlineBucketPO>lambdaQuery()
                      .eq(KlineBucketPO::getInstrumentId, instrumentId)
                      .eq(KlineBucketPO::getPeriod, period.getValue())
                      .eq(KlineBucketPO::getBucketStart, bucketStart));
          return persisted.orElseGet(
              () -> createNewBucket(instrumentId, period, bucketStart, bucketEnd));
        });
  }

  private KlineBucket createNewBucket(
      Long instrumentId, KlinePeriod period, Instant bucketStart, Instant bucketEnd) {
    KlineBucket bucket =
        KlineBucket.createEmpty(instrumentId, period.getValue(), bucketStart, bucketEnd);
