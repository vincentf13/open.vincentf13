...
export default function OrderBook({
    ...
    const Row = ({price, amount, total, depth, type}: {
        price: number;
        amount: number;
        total: number;
        depth: number;
        ...
        <div
            className="grid grid-cols-3 text-xs py-1 hover:bg-white/30 cursor-pointer rounded-md px-2 transition-all relative overflow-hidden group">
            <div
    ...
    return (
        <div
            className="relative flex flex-col flex-1 min-h-0 bg-gradient-to-b from-white/15 via-white/5 to-transparent">
                ...
                <Tooltip
                    title={(
                        <div className="text-xs">
                            ...
                            <div className="whitespace-nowrap">Powered by LMAX-style high-performance in-memory
                                matching.
                            </div>
                        </div>
                    )}
            ...
            <div className="relative flex-1 overflow-hidden p-2">
                <div
                    className="grid grid-cols-3 text-[10px] uppercase text-slate-400 font-bold tracking-wider px-2 mb-2 items-center">
                    <span>Price</span>
                        ...
                        <Tooltip
                            title={(
                                <div className="text-xs">
                                    ...
                                    <div className="whitespace-nowrap">計算方式：該檔位及以上所有檔位的 Amount 累加總和
                                    </div>
                                    <div className="h-px bg-white/20 my-1"/>
                                    <div className="whitespace-nowrap font-bold mb-1">Cumulative Total</div>
                                    <div className="whitespace-nowrap">Sum of all amounts from the best price level down
                                        to this level.
                                    </div>
                                </div>
                            )}
                ...
                <div className="space-y-0.5">
                    {displayAsks.length ? displayAsks.map((o, i) => (
                        <Row key={`ask-${i}`} price={o.price} amount={o.amount} total={o.total} depth={o.depth}
                             type="ask"/>
                    )) : (
                        <div
                            className="px-2 py-6 text-xs text-slate-400 text-center">{loading ? 'Loading...' : 'No data'}</div>
                    )}
                </div>
                ...
                <div className="space-y-0.5">
                    {bids.length ? bids.map((o, i) => (
                        <Row key={`bid-${i}`} price={o.price} amount={o.amount} total={o.total} depth={o.depth}
                             type="bid"/>
                    )) : (
                        <div
                            className="px-2 py-6 text-xs text-slate-400 text-center">{loading ? 'Loading...' : 'No data'}</div>
