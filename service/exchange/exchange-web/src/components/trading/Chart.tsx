          ...
          KLINE COUNTS {loading ? '...' : visibleSeries.length}
                ...
                <div className="flex flex-col items-end gap-1">
                    <div className="flex items-center gap-2">
                        <div
                            className="flex items-center gap-1 bg-white/30 rounded-lg p-1 border border-white/40 shadow-inner">
                            {periods.map((t) => (
                                <button
                                    key={t}
                                    onClick={() => setPeriod(t)}
                        ...
                        <div className="flex items-center gap-1 bg-white/20 rounded-lg p-1 border border-white/30">
                            ...
                            <Tooltip
                                title={(
                                    <div className="text-xs">
                                        ...
                                        <div
                                            className="whitespace-nowrap">按住左鍵左右與上下移動，可以調整時間與價格密度。
                                        </div>
                                        <div className="whitespace-nowrap">數據來源：行情服務通過消費撮合引擎發布的 WAL
                                            事件生成。
                                        </div>
                                        <div className="h-px bg-white/20 my-1"/>
                                        <div className="whitespace-nowrap font-bold mb-1">K-Line Chart Interaction</div>
                                        <div className="whitespace-nowrap">Hold left click and drag to adjust time and
                                            price density.
                                        </div>
                                        <div className="whitespace-nowrap">Source: Market service generates data from
                                            matching engine WAL events.
                                        </div>
                                    </div>
                                )}
                            ...
                            >
                                <div
                                    className="liquid-tooltip-trigger w-4 h-4 ml-1 rounded-full bg-slate-100 flex items-center justify-center text-[10px] text-slate-400 cursor-help border border-slate-200">?
                                </div>
                            </Tooltip>
                        </div>
                    </div>
            ...
            <div className="relative flex-1 w-full min-h-[320px] px-2 py-4">
                ...
                >
                    <svg className="w-full h-full" viewBox={`0 0 ${CHART_WIDTH} ${CHART_HEIGHT}`}
                        ...
                        <line x1="0" y1={VOLUME_TOP} x2={CHART_WIDTH} y2={VOLUME_TOP} stroke="rgba(148,163,184,0.7)"
                              strokeWidth="0.6"/>
                                ...
                                {visibleSeries.map((point, index) => {
                                    ...
                                    return (
                                        <g key={point.bucketStart ?? index}>
                                            <line x1={centerX} y1={yHigh} x2={centerX} y2={yLow} stroke={color}
                                                  strokeWidth="1"/>
                                            <rect
                                                x={centerX - candleWidth / 2}
                                                y={bodyTop}
                    ...
                    {crosshair && hoveredPoint && (
                        ...
                        >
                            <div className="border-b border-white/60 px-2 py-1 font-semibold text-slate-600">Kline
                                Detail
                            </div>
                            <div className="grid grid-cols-[92px_1fr] gap-x-2 gap-y-1 px-2 py-2">
                                ...
                                <span
                                    className="text-slate-800">{formatVolumeNumber(hoveredPoint.point.turnover)}</span>
                                <span className="text-slate-500">tradeCount</span>
                                <span
                                    className="text-slate-800">{formatOptionalNumber(hoveredPoint.point.tradeCount, formatCountNumber)}</span>
                            </div>
                        </div>
                    )}
