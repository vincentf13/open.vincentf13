...
export default function Trading() {
    ...
    useEffect(() => {
        ...
        const hmd = (e: MouseEvent) => {
            ...
            triggerAtClick = (e.target as HTMLElement).closest('.liquid-tooltip-trigger');
        };
        ...
        return () => {
            window.removeEventListener('mousedown', hmd, true);
            window.removeEventListener('mousemove', hmm, true);
        };
    }, []);
    ...
    const loadPlatformAccounts = (snapshotMs: number | null) => {
        const snapshotAt = snapshotMs != null ? new Date(snapshotMs).toISOString() : undefined;
        getPlatformAccounts(snapshotAt).then(res => {
            if (String(res?.code) === '0') setPlatformAccountsData(res.data);
        });
    };
    ...
    useEffect(() => {
        if (accountJournalOpen && accountJournalData?.accountId) {
            ...
            getAccountJournals(accountJournalData.accountId, snapshotAt).then(res => {
                if (String(res?.code) === '0') setAccountJournalData(res.data);
            });
        }
    ...
    useEffect(() => {
        if (platformAccountJournalOpen && platformAccountJournalData?.accountId) {
            const snapshotAt = balanceSheetTimestamp != null ? new Date(balanceSheetTimestamp).toISOString() : undefined;
            getPlatformAccountJournals(platformAccountJournalData.accountId, snapshotAt).then(res => {
                if (String(res?.code) === '0') setPlatformAccountJournalResponse(res.data);
            });
        }
    }, [refreshTrigger, balanceSheetTimestamp]);
    ...
    useEffect(() => {
        if (referenceJournalOpen && referenceJournalData?.referenceType && referenceJournalData?.referenceId) {
            getJournalsByReference(referenceJournalData.referenceType, referenceJournalData.referenceId).then(res => {
                if (String(res?.code) === '0') setReferenceJournalData(res.data);
            });
        }
    }, [refreshTrigger]);
    ...
    const handleOpenAccountJournal = (accountId: number) => {
        ...
        const snapshotAt = balanceSheetTimestamp != null ? new Date(balanceSheetTimestamp).toISOString() : undefined;
        getAccountJournals(accountId, snapshotAt).then(res => {
            if (String(res?.code) === '0') setAccountJournalData(res.data);
        }).finally(() => setAccountJournalLoading(false));
    };
    ...
    const handleOpenPlatformAccountJournal = (accountId: number) => {
        ...
        const snapshotAt = balanceSheetTimestamp != null ? new Date(balanceSheetTimestamp).toISOString() : undefined;
        getPlatformAccountJournals(accountId, snapshotAt).then(res => {
            if (String(res?.code) === '0') setPlatformAccountJournalResponse(res.data);
        }).finally(() => setPlatformAccountJournalLoading(false));
    };

    const handleOpenReferenceJournals = (type: string, id: string) => {
        setReferenceJournalOpen(true);
        setReferenceJournalLoading(true);
        getJournalsByReference(type, id).then(res => {
            if (String(res?.code) === '0') setReferenceJournalData(res.data);
        }).finally(() => setReferenceJournalLoading(false));
    };
    ...
    const renderCategoryChip = (value?: string | null) => {
        ...
        return (
            <span
                className="inline-flex items-center px-2 py-0.5 rounded-md border border-slate-200 bg-slate-50 text-[10px] font-semibold uppercase tracking-widest text-slate-500">
        {text}
      </span>
        );
    };

    const renderJournalRows = (items?: AccountJournalItem[], options?: {
        disableReferenceLink?: boolean;
        highlightMode?: 'background' | 'text';
        highlightExpenses?: boolean
    }) => {
        ...
        return (
            <div className="overflow-x-auto">
                <table className="w-full text-xs text-slate-600 border-separate border-spacing-x-0 table-fixed">
                    ...
                    <tr className="text-[10px] uppercase text-slate-400 border-b border-white/60 text-right whitespace-nowrap">
                        ...
                        <th className="py-1 px-1 font-semibold w-[60px]">
                            <div className="flex items-center gap-1 justify-end">
                                        ...
                                        <div className="text-[10px] leading-relaxed min-w-[480px]">
                                            <div className="grid grid-cols-2 gap-6">
                                                ...
                                                <div className="space-y-2 text-slate-700">
                                                    <p className="font-bold text-sky-600 border-b border-slate-200 pb-1">借貸方向
                                                        (Debit / Credit)</p>
                                                    <p className="font-mono text-sky-600">借 (DEBIT) = 貸 (CREDIT)</p>
                                                    <ul className="list-disc pl-4 space-y-1">
                                                        <li><span
                                                            className="font-semibold text-emerald-600">借 (DEBIT)</span>:
                                                            代表資產/費用增加，或負債/權益/收入減少。
                                                        </li>
                                                        <li><span
                                                            className="font-semibold text-rose-600">貸 (CREDIT)</span>:
                                                            代表負債/權益/收入增加，或資產/費用減少。
                                                        </li>
                                                    </ul>
                                                </div>
                                                {/* English Column */}
                                                <div className="space-y-2 text-slate-700">
                                                    <p className="font-bold text-sky-600 border-b border-slate-200 pb-1">Direction
                                                        Accounting Rules</p>
                                                    <p className="font-mono text-sky-600">Total Debits = Total
                                                        Credits</p>
                                                    <ul className="list-disc pl-4 space-y-1">
                                                        <li><span
                                                            className="font-semibold text-emerald-600">DEBIT</span>:
                                                            Increases in Assets/Expenses; decreases in
                                                            Liabilities/Equity/Revenue.
                                                        </li>
                                                        <li><span className="font-semibold text-rose-600">CREDIT</span>:
                                                            Increases in Liabilities/Equity/Revenue; decreases in
                                                            Assets/Expenses.
                                                        </li>
                                                    </ul>
                                                </div>
                                ...
                                >
                                    <div
                                        className="liquid-tooltip-trigger w-3 h-3 rounded-full bg-slate-100 flex items-center justify-center text-[9px] text-slate-400 cursor-help border border-slate-200">?
                                    </div>
                                </Tooltip>
                            </div>
                        </th>
                                        ...
                                        <div className="text-[10px] leading-relaxed min-w-[320px]">
                                            <div className="grid grid-cols-2 gap-6">
                                                <div className="space-y-2 text-slate-700">
                                                    ...
                                                    <p>點擊 ID
                                                        可查詢該筆業務操作（如撮合、手續費）產生的所有關聯帳戶分錄。</p>
                                                </div>
                                                <div className="space-y-2 text-slate-700">
                                                    <p className="font-bold text-sky-500 border-b border-slate-200 pb-1">Reference
                                                        Query</p>
                                                    <p>Click the ID to view all journal entries (e.g., matching, fees)
                    ...
                    <span
                        className={`px-1.5 py-0.5 rounded-md border text-[9px] font-black uppercase tracking-tighter ${isFrozen ? 'border-slate-200 text-slate-400' : 'bg-slate-100 border-slate-200 text-slate-600'}`}>
                      {direction}
                    </span>
                                </td>
                    ...
                    <span
                        className={`px-1.5 py-0.5 rounded-md border text-[9px] font-black uppercase tracking-tighter ${isFrozen ? 'border-slate-200 text-slate-300' : 'bg-slate-100 border-slate-200 text-slate-600'}`}>
                                                ...
                                                title={(
                                                    <div className="text-[10px] leading-relaxed min-w-[320px]">
                                                        <div className="grid grid-cols-2 gap-6">
                                                            ...
                                                            <div className="space-y-2 text-slate-700">
                                                                <p className="font-bold text-sky-500 border-b border-slate-200 pb-1">Freeze/Unfreeze
                                                                    Note</p>
                                                                <p>Freeze/unfreeze operations do not change account
                                                                    balances.</p>
                                                            </div>
                                                        </div>
                                                    </div>
    ...
    const renderPlatformJournalRows = (items?: PlatformJournalItem[], options?: {
        disableReferenceLink?: boolean;
        highlightMode?: 'background' | 'text';
        highlightExpenses?: boolean
    }) => {
                                    ...
                                    title={(
                                        <div className="text-[10px] leading-relaxed min-w-[480px]">
                                            <div className="grid grid-cols-2 gap-6">
                                                {/* Chinese Column */}
                                                <div className="space-y-2 text-slate-700">
                                                    <p className="font-bold text-indigo-600 border-b border-slate-200 pb-1">借貸方向
                                                        (Debit / Credit)</p>
                                                    <p className="font-mono text-indigo-600">借 (DEBIT) = 貸
                                                        (CREDIT)</p>
                                                    <ul className="list-disc pl-4 space-y-1">
                                                        <li><span
                                                            className="font-semibold text-emerald-600">借 (DEBIT)</span>:
                                                ...
                                                {/* English Column */}
                                                <div className="space-y-2 text-slate-700">
                                                    <p className="font-bold text-indigo-600 border-b border-slate-200 pb-1">Direction
                                                        Accounting Rules</p>
                                                    <p className="font-mono text-indigo-600">Total Debits = Total
                                                        Credits</p>
                                                    <ul className="list-disc pl-4 space-y-1">
                                                        <li><span
                                                            className="font-semibold text-emerald-600">DEBIT</span>:
                                    ...
                                    title={(
                                        <div className="text-[10px] leading-relaxed min-w-[320px]">
                                            <div className="grid grid-cols-2 gap-6">
                                                ...
                                                <div className="space-y-2 text-slate-700">
                                                    <p className="font-bold text-indigo-500 border-b border-slate-200 pb-1">Reference
                                                        Query</p>
                                                    <p>Click the ID to view all journal entries related to this
                                                        transaction.</p>
                                                </div>
                                            </div>
                                        </div>
                                                ...
                                                title={(
                                                    <div className="text-[10px] leading-relaxed min-w-[320px]">
                                                        <div className="grid grid-cols-2 gap-6">
                                                            ...
                                                            <div className="space-y-2 text-slate-700">
                                                                <p className="font-bold text-indigo-500 border-b border-slate-200 pb-1">Freeze/Unfreeze
                                                                    Note</p>
                                                                <p>Freeze/unfreeze operations do not change account
                                                                    balances.</p>
                                                            </div>
                        ...
                        return (
                            <tr key={index} className="text-right whitespace-nowrap">
                                <td className="py-1 pr-1 text-left overflow-hidden text-ellipsis">
                                    {item?.accountId != null ? (
                                        <button onClick={() => handleOpenAccountJournal(Number(item.accountId))}
                                                className="text-sky-500 underline">{item.accountId}</button>
                                    ) : '-'}
                                </td>
                    ...
                    {rows.map((item, index) => {
                        ...
                        return (
                            <tr key={index} className="text-right whitespace-nowrap">
                                <td className="py-1 pr-1 text-left overflow-hidden text-ellipsis">
                                    {item?.accountId != null ? (
                                        <button onClick={() => handleOpenPlatformAccountJournal(Number(item.accountId))}
                                                className="text-sky-500 underline">{item.accountId}</button>
                                    ) : '-'}
                                </td>
    ...
    return (
        <div className="flex h-screen flex-col bg-slate-50 overflow-hidden text-slate-900">
            <header
                className="flex h-14 items-center justify-between border-b border-white bg-white/80 px-6 backdrop-blur-md shadow-sm z-50">
                <div className="flex items-center gap-4">
                    <div
                        className="flex h-8 w-8 items-center justify-center rounded-xl bg-gradient-to-br from-sky-500 to-indigo-600 text-white font-black italic shadow-lg">LF
                    </div>
                    <h1 className="text-lg font-bold">Liquid Flow <span
                        className="font-light text-slate-400">| Exchange</span></h1>
                </div>
                <div className="flex items-center gap-2">
                    <div
                        className={`flex items-center rounded-lg border transition-all overflow-hidden ${isPaused ? 'bg-slate-500/10 border-slate-500/20' : 'bg-amber-500/10 border-amber-500/20'}`}>
                        <button
                            onClick={toggleSync}
                        ...
                        >
                            {!isPaused && <span
                                className="w-1.5 h-1.5 rounded-full bg-rose-500 animate-pulse shadow-[0_0_8px_rgba(244,63,94,0.8)]"/>}
                            {isPaused ? 'Paused' : 'Syncing'}
                            <Tooltip title={(
                                <div className="text-[10px] leading-relaxed">
                                    <p>「同步中」每 {activeIntervalRef.current}ms 自動更新數據（前次完成後）。</p>
                                    <p className="text-slate-400 mt-1">"Syncing" auto-updates
                                        every {activeIntervalRef.current}ms (after previous complete).</p>
                                    <div className="h-px bg-white/20 my-1"/>
                                    <p className="text-amber-400">注意：運行中修改數值將在下次啟動時生效。</p>
                                </div>
                            )} classNames={{root: 'liquid-tooltip'}} styles={{root: {maxWidth: 'none'}}}>
                                <div
                                    className={`liquid-tooltip-trigger w-3 h-3 rounded-full flex items-center justify-center text-[9px] cursor-help border ${isPaused ? 'bg-white/50 border-white/30 text-slate-400' : 'bg-amber-200/50 border-amber-300/50 text-amber-700'}`}>?
                                </div>
                            </Tooltip>
                        </button>
                        <div
                            className={`flex items-center px-2 py-1.5 gap-1 border-l transition-colors ${isPaused ? 'border-slate-500/20' : 'border-amber-500/20'}`}>
                            <span
                                className={`text-[10px] font-bold uppercase tracking-wider ${isPaused ? 'text-slate-400' : 'text-amber-600/70'}`}>Poll</span>
                            <input
                                type="number"
                                value={pollInterval}
                            ...
                            />
                            <span
                                className={`text-[10px] font-bold ${isPaused ? 'text-slate-400' : 'text-amber-600/70'}`}>ms</span>
                        </div>
                    </div>

                    <button onClick={handleResetData} disabled={resetting}
                            className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-rose-500/10 border border-rose-500/20 text-rose-600 text-[10px] font-bold uppercase hover:bg-rose-500 hover:text-white disabled:opacity-50 transition-all group">
                        {resetting ? 'Resetting...' : 'Reset Data'}
                        <Tooltip title={(
                            <div className="text-[10px] leading-relaxed">
                                <p>重置所有交易數據、餘額與分錄（僅測試環境）。</p>
                                <p className="text-slate-400 mt-1">Resets all trades, balances, and journals (Test
                                    only).</p>
                            </div>
                        )} classNames={{root: 'liquid-tooltip'}} styles={{root: {maxWidth: 'none'}}}>
                            <div
                                className="liquid-tooltip-trigger w-3 h-3 rounded-full bg-rose-200/50 group-hover:bg-white/20 flex items-center justify-center text-[9px] cursor-help border border-rose-300/50 group-hover:border-white/30">?
                            </div>
                        </Tooltip>
                    </button>

                    <button onClick={handleRefresh}
                            className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-emerald-500/10 border border-emerald-500/20 text-emerald-600 text-[10px] font-bold uppercase hover:bg-emerald-500 hover:text-white transition-all group">
                        Refresh
                        <Tooltip title={(
                            <div className="text-[10px] leading-relaxed">
                                <p>手動強制刷新當前頁面所有可見數據。</p>
                                <p className="text-slate-400 mt-1">Manually force refresh all visible data on the
                                    page.</p>
                            </div>
                        )} classNames={{root: 'liquid-tooltip'}} styles={{root: {maxWidth: 'none'}}}>
                            <div
                                className="liquid-tooltip-trigger w-3 h-3 rounded-full bg-emerald-200/50 group-hover:bg-white/20 flex items-center justify-center text-[9px] cursor-help border border-emerald-300/50 group-hover:border-white/30">?
                            </div>
                        </Tooltip>
                    </button>
                        ...
                        {currentUser ? (
                            <>
                                <div className="flex flex-col items-end mr-2">
                                    <span
                                        className="text-[10px] font-bold text-slate-400 uppercase tracking-wider leading-tight">{currentUser.email || 'No Email'}</span>
                                    <div className="flex items-center gap-1.5">
                                        <span className="text-[9px] font-medium text-slate-400 uppercase">ID</span>
                                        <span
                                            className="text-xs font-mono font-semibold text-indigo-600">{currentUser.id}</span>
                                    </div>
                                </div>
                                <button onClick={handleLogout}
                                        className="rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-[10px] font-bold uppercase text-slate-600 hover:bg-slate-50 hover:text-rose-600 transition-colors">
                                    Logout
                                </button>
                            </>
                        ) : (
            ...
            <main className="flex-1 flex flex-col overflow-hidden">
                <div className="flex-1 flex gap-4 p-4 min-h-0 overflow-hidden">
                    <div className="flex-1 flex flex-col gap-4 min-w-0 overflow-hidden">
                        <div className="flex-[3] flex gap-4 min-h-0">
                            <div className="flex-1 flex flex-col gap-2 min-w-0">
                                <div
                                    className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden p-1">
                                    <MarketStats instruments={instruments} selectedInstrument={selectedInstrument}
                                                 onSelectInstrument={i => setSelectedInstrumentId(i.instrumentId)}
                                                 refreshTrigger={refreshTrigger} isPaused={isPaused}
                            ...
                            <div
                                className="w-[300px] bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden flex flex-col">
                                <OrderBook selectedInstrumentId={selectedInstrumentId} refreshTrigger={refreshTrigger}
                                           isPaused={isPaused}
                                           contractSize={Number(selectedInstrument?.contractSize || 1)}
                                           onSyncComplete={handleSyncComplete}/>
                            </div>
                        </div>
                        <div
                            className="flex-[2] bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden flex flex-col">
                    ...
                    <div className="w-[340px] flex flex-col gap-4">
                        <div
                            className="flex-none bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                            <TradeForm instrument={selectedInstrument} refreshTrigger={refreshTrigger}
                                       onOrderCreated={handleRefresh} isPaused={isPaused}/>
                        </div>
            ...
            {balanceSheetOpen && (
                <div
                    ...
                    onClick={(e) => {
                        if (e.target === e.currentTarget) handleCloseBalanceSheet();
                    }}
                >
                    <div
                        className="relative w-[96vw] max-w-[1860px] bg-white/95 backdrop-blur-sm rounded-3xl border border-white/70 shadow-2xl p-6 overflow-hidden flex flex-col max-h-[95vh]">
                        {(accountJournalOpen || referenceJournalOpen || platformAccountJournalOpen) && (
                            ...
                            >
                                ...
                                <div
                                    ref={referenceJournalOpen ? referenceJournalRef : (accountJournalOpen ? accountJournalRef : platformAccountJournalRef)}
                                    className="relative w-full max-w-[90%] rounded-2xl border border-white bg-white/95 p-6 shadow-2xl flex flex-col pointer-events-auto">
                                        ...
                                        {referenceJournalOpen ? (
                                            referenceJournalLoading && !referenceJournalData ? (
                                                ...
                                                <div className="space-y-8">
                                                    <div>
                                                        <div
                                                            className="text-[10px] font-bold text-slate-400 mb-2 uppercase flex items-center gap-2">
                                                            <div className="w-1 h-1 rounded-full bg-slate-400"></div>
                                                            User
                                                        </div>
                                                        {renderJournalRows(referenceUserRows, {
                                                            disableReferenceLink: true,
                                                            highlightMode: 'text'
                                                        })}
                                                    </div>
                                                    ...
                                                    <div>
                                                        ...
                                                        {(referencePlatformSplit.core.length > 0 || referencePlatformSplit.expenseRevenue.length === 0) && renderPlatformJournalRows(referencePlatformSplit.core, {
                                                            disableReferenceLink: true,
                                                            highlightMode: 'text'
                                                        })}
                                                    </div>
                                                    ...
                                                    <div>
                                                        ...
                                                        <div className="flex flex-col gap-6">
                                                            <div className="space-y-2">
                                                                <div
                                                                    className="text-[10px] font-semibold uppercase text-slate-500">User
                                                                </div>
                                                                {renderJournalRows(referenceUserSplit.expenseRevenue, {
                                                                    disableReferenceLink: true,
                                                                    highlightMode: 'text',
                                                                    highlightExpenses: true
                                                                })}
                                                            </div>
                                                            <div className="space-y-2">
                                                                <div
                                                                    className="text-[10px] font-semibold uppercase text-slate-500">Exchange
                                                                </div>
                                                                {renderPlatformJournalRows(referencePlatformSplit.expenseRevenue, {
                                                                    disableReferenceLink: true,
                                                                    highlightMode: 'text',
                                                                    highlightExpenses: true
                                                                })}
                                            ...
                                            <>
                                                {accountJournalOpen && (accountJournalLoading && !accountJournalData ?
                                                    <div
                                                        className="p-8 text-center text-xs text-slate-400">Loading...</div> : renderJournalRows(accountJournalData?.journals, {highlightMode: 'text'}))}
                                                {platformAccountJournalOpen && (platformAccountJournalLoading && !platformAccountJournalData ?
                                                    <div
                                                        className="p-8 text-center text-xs text-slate-400">Loading...</div> : renderPlatformJournalRows(platformAccountJournalData?.journals, {highlightMode: 'text'}))}
                                            </>
                                        )}
                                    </div>
                                </div>
                        ...
                        <div className="flex items-center justify-between mb-6">
                            <div className="flex items-center gap-3">
                                <h2 className="text-sm font-bold uppercase tracking-widest text-slate-800">Balance
                                    Sheet</h2>
                                <div className="flex items-center gap-2">
                                    <button
                                                ...
                                                <div className="grid grid-cols-2 gap-4">
                                                    <div className="space-y-2 text-slate-700">
                                                        ...
                                                        <ul className="space-y-1">
                                                            <li><span
                                                                className="font-bold text-emerald-600">Latest</span>:
                                                                顯示當前最新狀態。
                                                            </li>
                                                            <li><span
                                                                className="font-bold text-sky-600">Historical</span>:
                                                                查詢歷史時間點的帳戶快照。
                                                            </li>
                                                            <li><span
                                                                className="font-bold text-slate-600">Snapshot</span>:
                                                                數據對應的實際時間點。
                                                            </li>
                                                        </ul>
                                                    </div>
                                                    <div className="space-y-2 text-slate-700">
                                                        <p className="font-bold border-b border-slate-200 pb-1">Snapshot
                                                            Guide</p>
                                                        <ul className="space-y-1">
                                                            <li><span
                                                                className="font-bold text-emerald-600">Latest</span>:
                                                            ...
                                                            <li><span
                                                                className="font-bold text-sky-600">Historical</span>:
                                                                Query account state at past time.
                                                            </li>
                                                            <li><span
                                                                className="font-bold text-slate-600">Snapshot</span>:
                                                                Exact time of data.
                      ...
                      Snapshot: <span className="font-mono text-[11px] text-slate-500">{balanceSheetData?.snapshotAt ?? '-'}</span>
                            ...
                            <button onClick={handleCloseBalanceSheet}
                                    className="h-8 w-8 flex items-center justify-center rounded-full border border-white/60 bg-white/70 text-slate-500 hover:text-slate-700">X
                            </button>
                        </div>
                            ...
                            <div className="mb-4 px-2">
                                <div className="flex flex-col gap-2 text-[10px] text-slate-500 mb-3">
                                    ...
                                    <div className="flex items-center justify-between">
                                        <span className="flex items-center gap-1">Earliest <span
                                            className="font-mono text-[11px] text-slate-400">{formatSnapshotTimestamp(balanceSheetRange.earliest)}</span></span>
                                        <span className="flex items-center gap-1">Selected <span
                                            className="font-mono text-[11px] text-slate-400">{formatSnapshotTimestamp(balanceSheetPendingTimestamp ?? balanceSheetRange.latest)}</span></span>
                                        <span className="flex items-center gap-1">Latest <span
                                            className="font-mono text-[11px] text-slate-400">{formatSnapshotTimestamp(balanceSheetRange.latest)}</span></span>
                                    </div>
                                </div>
                                <input
                                    type="range"
                            ...
                            <div className="space-y-12">
                                ...
                                <section>
                                    <div className="flex items-center gap-2 mb-4 border-b-2 border-sky-100 pb-1">
                                        <div
                                            className="text-xs font-black uppercase tracking-widest text-sky-600">User
                                        </div>
                                        <Tooltip
                                            title={(
                                                <div className="text-[10px] leading-relaxed min-w-[520px]">
                                                    <div className="grid grid-cols-2 gap-6">
                                                        ...
                                                        <div className="space-y-2 text-slate-700">
                                                            ...
                                                            <p>遵循 <span className="text-sky-600 font-mono">資產 = 負債 + 權益 + (收入 - 費用)</span>
                                                            </p>
                                                            <ul className="list-disc pl-4 space-y-1">
                                                                <li><span className="font-semibold text-emerald-600">Assets (資產)</span>:
                                                                    用戶實際持有的代幣。
                                                                </li>
                                                                <li><span className="font-semibold text-rose-600">Liabilities (負債)</span>:
                                                                    用戶對交易所的欠款。
                                                                </li>
                                                                <li><span className="font-semibold text-sky-600">Equity (權益)</span>:
                                                                    用戶的淨資產。
                                                                </li>
                                                                <li><span className="font-semibold text-amber-600">Revenue (收入)</span>:
                                                                    交易獲利或入金。
                                                                </li>
                                                                <li><span className="font-semibold text-slate-500">Expenses (費用)</span>:
                                                                    交易虧損或手續費。
                                                                </li>
                                                            </ul>
                                                            <div className="pt-1 border-t border-slate-200">
                                                                <p className="font-bold text-slate-600">分錄 (Journal
                                                                    Entries)</p>
                                                        ...
                                                        {/* English Column */}
                                                        <div className="space-y-2 text-slate-700">
                                                            <p className="font-bold text-sky-600 border-b border-slate-200 pb-1">User
                                                                Double-Entry Bookkeeping</p>
                                                            <p className="font-mono text-sky-600 text-[9px]">Assets =
                                                                Liabilities + Equity + (Revenue - Expenses)</p>
                                                            <ul className="list-disc pl-4 space-y-1">
                                                                <li><span
                                                                    className="font-semibold text-emerald-600">Assets</span>:
                                                                    Funds/Tokens owned by user.
                                                                </li>
                                                                <li><span
                                                                    className="font-semibold text-rose-600">Liabilities</span>:
                                                                    Debts or loans owed to the exchange.
                                                                </li>
                                                                <li><span
                                                                    className="font-semibold text-sky-600">Equity</span>:
                                                                    Net worth of the account.
                                                                </li>
                                                                <li><span
                                                                    className="font-semibold text-amber-600">Revenue</span>:
                                                                    Trading gains or deposits.
                                                                </li>
                                                                <li><span
                                                                    className="font-semibold text-slate-500">Expenses</span>:
                                                                    Trading losses or fees paid.
                                                                </li>
                                                            </ul>
                                                            <div className="pt-1 border-t border-slate-200">
                                                                <p className="font-bold text-slate-600">Journal
                                                                    Entries</p>
                                                                <p>All balance changes are driven by "Journals". Every
                                                                    trade generates debit/credit entries, ensuring <span
                                                                        className="italic">Total Debits = Total Credits</span>.
                                                                </p>
                                                            </div>
                                                        </div>
                                                    </div>
                                        ...
                                        >
                                            <div
                                                className="liquid-tooltip-trigger w-4 h-4 rounded-full bg-sky-50 flex items-center justify-center text-[10px] text-sky-400 cursor-help border border-sky-200">?
                                            </div>
                                        </Tooltip>
                                    </div>
                                    <div
                                        className="flex flex-col border border-slate-200/80 divide-y divide-slate-200/80">
                                        <div
                                            className="flex flex-col divide-y divide-slate-200/80 min-[1600px]:grid min-[1600px]:grid-cols-2 min-[1600px]:divide-x min-[1600px]:divide-y-0">
                                            <div className="p-4 flex flex-col">
                                                <div
                                                    className="text-[10px] font-bold text-slate-400 uppercase mb-3 tracking-wider">Assets
                                                </div>
                                                <div
                                                    className="flex-1">{renderAccountItems(balanceSheetData?.assets)}</div>
                                            </div>
                                            <div className="flex flex-col divide-y divide-slate-200/80">
                                                <div className="p-4 flex flex-col flex-1">
                                                    <div
                                                        className="text-[10px] font-bold text-slate-400 uppercase mb-3 tracking-wider">Liabilities
                                                    </div>
                                                    <div
                                                        className="flex-1">{renderAccountItems(balanceSheetData?.liabilities)}</div>
                                                </div>
                                                <div className="p-4 flex flex-col flex-1">
                                                    <div
                                                        className="text-[10px] font-bold text-slate-400 uppercase mb-3 tracking-wider">Equity
                                                    </div>
                                                    <div
                                                        className="flex-1">{renderAccountItems(balanceSheetData?.equity)}</div>
                                                </div>
                                            </div>
                                        </div>
                                        <div
                                            ...
                                            <div className="p-4 flex flex-col">
                                                <div
                                                    className="text-[10px] font-bold text-slate-400 uppercase mb-3 tracking-wider">Expenses
                                                </div>
                                                <div
                                                    className="flex-1">{renderAccountItems(balanceSheetData?.expenses)}</div>
                                            </div>
                                            <div className="p-4 flex flex-col">
                                                <div
                                                    className="text-[10px] font-bold text-slate-400 uppercase mb-3 tracking-wider">Revenue
                                                </div>
                                                <div
                                                    className="flex-1">{renderAccountItems(balanceSheetData?.revenue)}</div>
                                            </div>
                                        </div>
                                    </div>
                                </section>
                                ...
                                <section>
                                    <div className="flex items-center gap-2 mb-4 border-b-2 border-indigo-100 pb-1">
                                        <div
                                            className="text-xs font-black uppercase tracking-widest text-indigo-600">Exchange
                                        </div>
                                        <Tooltip
                                            title={(
                                                <div className="text-[10px] leading-relaxed min-w-[520px]">
                                                    <div className="grid grid-cols-2 gap-6">
                                                        ...
                                                        <div className="space-y-2 text-slate-700">
                                                            ...
                                                            <p>交易所作為託管方，其內部的 <span
                                                                className="text-indigo-600 font-semibold">資產</span> 必須恆等於對所有用戶的 <span
                                                                className="text-indigo-600 font-semibold">總負債</span>。
                                                            </p>
                                                        ...
                                                        {/* English Column */}
                                                        <div className="space-y-2 text-slate-700">
                                                            <p className="font-bold text-indigo-600 border-b border-slate-200 pb-1">Exchange
                                                                Accounting</p>
                                                            <p>As a custodian, the exchange's <span
                                                                className="text-indigo-500 font-semibold">Assets</span> must
                                                                always equal its <span
                                                                    className="text-indigo-500 font-semibold">Total Liabilities</span> to
                                                                users.</p>
                                                            <p>This section displays exchange accounts (inventory, fees)
                                                                and overall system equilibrium.</p>
                                                            <div className="pt-1 border-t border-slate-200">
                                                                <p className="font-bold text-slate-600">Journal
                                                                    Entries</p>
                                                                <p>Automated journals record every movement of funds.
                                                                    They are the foundation for auditing and
                                                                    asset-equity alignment.</p>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                        ...
                                        >
                                            <div
                                                className="liquid-tooltip-trigger w-4 h-4 rounded-full bg-indigo-50 flex items-center justify-center text-[10px] text-indigo-400 cursor-help border border-indigo-200">?
                                            </div>
                                        </Tooltip>
                                    </div>
                                    <div
                                        ...
                                        <div
                                            ...
                                            <div className="p-4 flex flex-col">
                                                ...
                                                <div
                                                    className="flex-1">{renderPlatformAccountRows(platformAccountsData?.assets)}</div>
                                            </div>
                                            <div className="flex flex-col divide-y divide-slate-200/80">
                                                <div className="p-4 flex flex-col flex-1">
                                                    <div
                                                    ...
                                                    <div
                                                        className="flex-1">{renderPlatformAccountRows(platformAccountsData?.liabilities)}</div>
                                                </div>
                                                <div className="p-4 flex flex-col flex-1">
                                                    <div
                                                        className="text-[10px] font-bold text-slate-400 uppercase mb-3 tracking-wider">Equity
                                                    ...
                                                    <div
                                                        className="flex-1">{renderPlatformAccountRows(platformAccountsData?.equity)}</div>
                                                </div>
                                            </div>
                                        </div>
                                        <div
                                            ...
                                            <div className="p-4 flex flex-col">
                                                ...
                                                <div
                                                    className="flex-1">{renderPlatformAccountRows(platformAccountsData?.expenses)}</div>
                                            </div>
                                            <div className="p-4 flex flex-col">
                                                <div
                                                    className="text-[10px] font-bold text-slate-400 uppercase mb-3 tracking-wider">Revenue
                                                ...
                                                <div
                                                    className="flex-1">{renderPlatformAccountRows(platformAccountsData?.revenue)}</div>
                                            </div>
                                        </div>
                                    </div>
                                </section>
                    ...
                    >
                        <div className="mb-2 flex items-center justify-between">
                            ...
                            <button className="text-slate-400 hover:text-slate-600"
                                    onClick={() => setJournalTradeDetail(null)}>x
                            </button>
                        </div>
                        {journalTradeDetailLoading ? (
                            ...
                            <div className="grid grid-cols-[auto_1fr] gap-x-3 gap-y-1">
                                <span className="text-slate-400">Trade Id</span><span
                                className="font-mono">{journalTradeDetail.tradeId}</span>
                                <span
                                    className="text-slate-400">Instrument</span><span>{instrumentMap.get(String(journalTradeDetail.instrumentId)) || journalTradeDetail.instrumentId}</span>
                                <span className="text-slate-400">Order Id</span><span
                                className="font-mono">{journalTradeDetail.orderId}</span>
                                <span className="text-slate-400">Counterparty</span><span
                                className="font-mono">{journalTradeDetail.counterpartyOrderId}</span>
                                <span className="text-slate-400">Price</span><span
                                className="font-mono text-sky-600 font-bold">{formatValue(journalTradeDetail.price)}</span>
                                <span className="text-slate-400">Quantity</span><span
                                className="font-mono text-sky-600 font-bold">{formatValue(journalTradeDetail.quantity)}</span>
                                <span className="text-slate-400">Maker Fee</span><span
                                className="font-mono">{formatValue(journalTradeDetail.makerFee)}</span>
                                <span className="text-slate-400">Taker Fee</span><span
                                className="font-mono">{formatValue(journalTradeDetail.takerFee)}</span>
                                <span className="text-slate-400">Executed At</span><span
                                className="font-mono">{String(journalTradeDetail.executedAt ?? '-')}</span>
                            </div>
                        )}
                    </div>
                </div>
