...
public class AccountPositionDomainService {
  ...
  public void releaseMargin(@NotNull @Valid PositionMarginReleasedEvent event) {
    ...
    UserAccount updatedRealizedEquity =
        realizedPnlEquity.applyAllowNegative(direction, event.realizedPnl().abs());
    UserAccount updatedRealizedRevenue =
        realizedPnlRevenue.applyAllowNegative(direction, event.realizedPnl().abs());
    ...
    UserAccount updatedSpotAfterFee =
        hasFee ? updatedSpot.apply(Direction.CREDIT, feeCharged) : updatedSpot;
    UserAccount updatedFeeExpense =
        hasFee ? feeExpenseAccount.apply(Direction.DEBIT, feeCharged) : feeExpenseAccount;
    UserAccount updatedFeeEquity =
        hasFee
            ? feeEquityAccount.applyAllowNegative(Direction.DEBIT, feeCharged)
            : feeEquityAccount;
    PlatformAccount updatedPlatformLiability =
        hasFee ? platformLiability.apply(Direction.DEBIT, feeCharged) : platformLiability;
    PlatformAccount updatedPlatformRevenue =
        hasFee ? platformRevenue.apply(Direction.CREDIT, feeCharged) : platformRevenue;
    PlatformAccount updatedPlatformFeeEquity =
        hasFee ? platformFeeEquity.apply(Direction.CREDIT, feeCharged) : platformFeeEquity;
    ...
    UserJournal pnlJournal =
        buildUserJournal(
            ...
            event.releasedAt());
    UserJournal equityJournal =
        buildUserJournal(
            updatedRealizedEquity,
            ...
            "Realized PnL equity",
            event.releasedAt());
    UserJournal revenueJournal =
        buildUserJournal(
            updatedRealizedRevenue,
            event.realizedPnl().signum() >= 0 ? Direction.CREDIT : Direction.DEBIT,
            event.realizedPnl().abs(),
            ReferenceType.POSITION_REALIZED_PNL,
            referenceId,
    ...
    if (hasFee) {
      ...
      PlatformJournal platformLiabilityJournal =
          buildPlatformJournal(
              ...
              event.releasedAt());
      PlatformJournal platformRevenueJournal =
          buildPlatformJournal(
              updatedPlatformRevenue,
              Direction.CREDIT,
              feeCharged,
              ReferenceType.TRADE_FEE,
              referenceId,
              ...
              "Trading fee revenue",
              event.releasedAt());
      PlatformJournal platformFeeEquityJournal =
          buildPlatformJournal(
      ...
      platformJournalRepository.insertBatch(
          List.of(platformLiabilityJournal, platformRevenueJournal, platformFeeEquityJournal));
    }
  }

  private UserJournal buildUserJournal(
      UserAccount account,
      Direction direction,
      BigDecimal amount,
  ...
  private PlatformJournal buildPlatformJournal(
  ...
  private void assertNoDuplicateJournal(
      Long accountId, AssetSymbol asset, ReferenceType referenceType, String referenceId) {
    boolean exists =
        userJournalRepository
            .findLatestByReference(accountId, asset, referenceType, referenceId)
            .isPresent();
    if (exists) {
      throw OpenException.of(
