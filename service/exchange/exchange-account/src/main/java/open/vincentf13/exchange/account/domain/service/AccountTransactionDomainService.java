...
public class AccountTransactionDomainService {
  ...
  public AccountDepositResult deposit(@NotNull @Valid AccountDepositRequest request) {
    ...
    UserAccount userAssetAccount =
        userAccountRepository.getOrCreate(request.getUserId(), UserAccountCode.SPOT, null, asset);

    Long accountId = userAssetAccount.getAccountId();
    String referenceId = request.getTxId();
    boolean exists =
        userJournalRepository
            .findLatestByReference(accountId, asset, ReferenceType.DEPOSIT, referenceId)
            .isPresent();
    if (exists) {
    ...
    UserAccount userEquityAccount =
        userAccountRepository.getOrCreate(
            request.getUserId(), UserAccountCode.SPOT_EQUITY, null, asset);

    PlatformAccount platformAssetAccount =
        platformAccountRepository.getOrCreate(PlatformAccountCode.HOT_WALLET, asset);
    PlatformAccount platformLiabilityAccount =
        platformAccountRepository.getOrCreate(PlatformAccountCode.USER_LIABILITY, asset);

    int uSeq = 1;
    var userAssetResult =
        userAssetAccount.applyWithJournal(
            Direction.DEBIT,
            request.getAmount(),
            idGenerator.newLong(),
            ReferenceType.DEPOSIT,
    ...
    var userEquityResult =
        userEquityAccount.applyWithJournal(
            Direction.CREDIT,
            request.getAmount(),
            idGenerator.newLong(),
            ReferenceType.DEPOSIT,
    ...
    var platformLiabilityResult =
        platformLiabilityAccount.applyWithJournal(
            Direction.CREDIT,
            request.getAmount(),
            idGenerator.newLong(),
            ReferenceType.DEPOSIT,
    ...
    // Note: deposit result record expects single journals. We pick the first one which is usually
    // the only one for 'apply'.
    return new AccountDepositResult(
        updatedUserAsset,
        updatedUserEquity,
        updatedPlatformAsset,
        updatedPlatformLiability,
        userAssetJournal.get(0),
        userEquityJournal.get(0),
        platformAssetJournal,
        platformLiabilityJournal);
  }

  @Transactional(rollbackFor = Exception.class)
  public FundsFreezeResult freezeForOrder(
      @NotNull @Valid FundsFreezeRequestedEvent event,
    ...
    boolean exists =
        userJournalRepository
            .findLatestByReference(accountId, asset, ReferenceType.ORDER_MARGIN_FROZEN, referenceId)
            .isPresent();
    if (exists) {
      throw OpenException.of(
          AccountErrorCode.DUPLICATE_REQUEST,
    ...
    if (totalAmount.compareTo(BigDecimal.ZERO) <= 0) {
      throw OpenException.of(
          ...
          Map.of("orderId", event.orderId(), "amount", totalAmount));
    }
    if (!userSpot.hasEnoughAvailable(totalAmount)) {
      throw OpenException.of(
          ...
          Map.of(
              ...
              asset,
              "available",
              userSpot.getAvailable(),
              "amount",
              totalAmount));
    }

    int uSeq = 1;
    var marginFrozenResult =
        userSpot.freezeWithJournal(
            requiredMargin,
            idGenerator.newLong(),
            idGenerator.newLong(),
            ReferenceType.ORDER_MARGIN_FROZEN,
    ...
    var feeFrozenResult =
        marginFrozenAccount.freezeWithJournal(
            fee,
            idGenerator.newLong(),
            idGenerator.newLong(),
    ...
    userAccountRepository.updateSelectiveBatch(
        List.of(feeFrozenAccount), List.of(userSpot.safeVersion()), "order-freeze");
  ...
  public AccountWithdrawalResult withdraw(@NotNull @Valid AccountWithdrawalRequest request) {
    ...
    boolean exists =
        userJournalRepository
            .findLatestByReference(accountId, asset, ReferenceType.WITHDRAWAL, referenceId)
            .isPresent();
    if (exists) {
      throw OpenException.of(
          AccountErrorCode.DUPLICATE_REQUEST,
  ...
  public void settleTrade(
      @NotNull @Valid TradeExecutedEvent event,
    ...
    UserAccount userSpot =
        userAccountRepository.getOrCreate(userId, UserAccountCode.SPOT, null, asset);
    ...
    boolean exists =
        userJournalRepository
            .findLatestByReference(
                accountId, asset, ReferenceType.TRADE_MARGIN_SETTLED, refIdWithSide)
            .isPresent();
    if (exists) {
      throw OpenException.of(
          AccountErrorCode.DUPLICATE_REQUEST,
    ...
    if (intent != PositionIntentType.INCREASE) {
      throw OpenException.of(
          AccountErrorCode.UNSUPPORTED_ORDER_INTENT, Map.of("orderId", orderId, "intent", intent));
    }
    ...
    BigDecimal marginReserved =
        userJournalRepository
            ...
            .orElseThrow(
                () ->
                    OpenException.of(
                        AccountErrorCode.FREEZE_ENTRY_NOT_FOUND,
                        Map.of("orderId", orderId, "userId", userId)));
    BigDecimal feeReserved =
        userJournalRepository
            .findLatestByReference(
    ...
    if (tradeQuantity.compareTo(BigDecimal.ZERO) <= 0) {
      throw OpenException.of(
          AccountErrorCode.INVALID_EVENT, Map.of("orderId", orderId, "userId", userId));
    }
    BigDecimal orderQuantity = isMaker ? event.orderQuantity() : event.counterpartyOrderQuantity();
    BigDecimal orderFilledQuantity =
        isMaker ? event.orderFilledQuantity() : event.counterpartyOrderFilledQuantity();
    if (orderQuantity == null
        || orderFilledQuantity == null
    ...
    UserAccount userMargin =
        userAccountRepository.getOrCreate(
            userId, UserAccountCode.MARGIN, event.instrumentId(), asset);
    UserAccount userFeeExpense =
        userAccountRepository.getOrCreate(userId, UserAccountCode.FEE_EXPENSE, null, asset);
    UserAccount userFeeEquity =
        userAccountRepository.getOrCreate(userId, UserAccountCode.FEE_EQUITY, null, asset);
    ...
    if (userSpot.getReserved().compareTo(totalReservedUsed) < 0) {
      throw OpenException.of(
          AccountErrorCode.INSUFFICIENT_RESERVED_BALANCE,
          Map.of(
              "userId", userId, "reserved", userSpot.getReserved(), "required", totalReservedUsed));
    }
    ...
    var marginOutResult =
        userSpot.applyReservedOutWithJournal(
            marginPortion,
            idGenerator.newLong(),
            ReferenceType.TRADE_MARGIN_SETTLED,
            refIdWithSide,
    ...
    if (feeRefund.signum() > 0) {
      var feeRefundResult =
          spotAfterMargin.refundReservedWithJournal(
              feeRefund,
              idGenerator.newLong(),
              idGenerator.newLong(),
    ...
    if (actualFee.signum() > 0) {
      var feePaymentResult =
          spotAfterFeeRefund.applyReservedOutWithJournal(
              actualFee,
              idGenerator.newLong(),
    ...
    var feeExpenseResult =
        userFeeExpense.applyWithJournal(
            Direction.DEBIT,
            actualFee,
            idGenerator.newLong(),
            ReferenceType.TRADE_FEE,
    ...
    var feeEquityResult =
        userFeeEquity.applyAllowNegativeWithJournal(
            Direction.DEBIT,
            actualFee,
            idGenerator.newLong(),
            ReferenceType.TRADE_FEE,
    ...
    userAccountRepository.updateSelectiveBatch(
        ...
        List.of(
            userSpot.safeVersion(),
            userMargin.safeVersion(),
            userFeeExpense.safeVersion(),
            userFeeEquity.safeVersion()),
        "trade-settle");
  ...
  @Transactional(rollbackFor = Exception.class)
  public void settleCloseToOpenCompensation(
    ...
    var spotMarginResult =
        userSpot.applyAllowNegativeWithJournal(
            Direction.CREDIT,
            marginUsed,
            idGenerator.newLong(),
            ReferenceType.TRADE_MARGIN_SETTLED,
    ...
    var spotFeeResult =
        spotAfterMargin.applyAllowNegativeWithJournal(
            Direction.CREDIT,
            actualFee,
            idGenerator.newLong(),
            ReferenceType.TRADE_FEE,
    ...
    userJournalRepository.insertBatch(
        List.of(marginJournal, marginOutJournal, feeJournal, feeExpenseJournal, feeEquityJournal));
    platformJournalRepository.insertBatch(
        List.of(platformLiabilityJournal, revenueJournal, platformFeeEquityJournal));
    ...
    if (updatedSpot.getBalance().signum() < 0 || updatedSpot.getAvailable().signum() < 0) {
      OpenLog.warn(
          ...
          "asset",
          updatedSpot.getAsset(),
          "balance",
          updatedSpot.getBalance(),
          "available",
          updatedSpot.getAvailable(),
          "tradeId",
          event.tradeId(),
          "orderId",
          event.orderId());
  ...
  @Transactional(rollbackFor = Exception.class)
  public void settleOpenToCloseCompensation(
    ...
    BigDecimal contractMultiplier = requireContractSize(event.instrumentId());
    BigDecimal marginToRefund =
        event.price().multiply(event.quantity()).multiply(contractMultiplier);
