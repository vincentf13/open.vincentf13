...
public class AccountQueryService {
  ...
  public AccountBalanceResponse getBalances(@NotNull Long userId, @NotBlank String assetCode) {
  ...
  public AccountBalanceSheetResponse getBalanceSheet(@NotNull Long userId, Instant snapshotAt) {
    ...
    Map<Long, BigDecimal> snapshotBalances =
        isHistorical ? buildUserSnapshotBalances(accounts, snapshotAt) : Map.of();
    List<AccountBalanceItem> items =
        accounts.stream()
            .map(
                account ->
                    toAccountBalanceItem(
                        account, snapshotBalances.get(account.getAccountId()), isHistorical))
            .filter(Objects::nonNull)
            .sorted(accountBalanceComparator())
            .toList();
    Map<AccountCategory, List<AccountBalanceItem>> grouped =
        items.stream()
            .sorted(accountBalanceComparator())
    ...
    List<AccountBalanceItem> liabilities =
        grouped.getOrDefault(AccountCategory.LIABILITY, List.of());
    ...
    return new AccountBalanceSheetResponse(
        userId,
        effectiveSnapshot,
        earliestUpdate,
        latestUpdate,
  ...
  public AccountJournalResponse getAccountJournals(
      @NotNull Long userId, @NotNull Long accountId, Instant snapshotAt) {
    boolean isHistorical = snapshotAt != null;
    List<UserJournal> journals =
        isHistorical
  ...
  public AccountReferenceJournalResponse getJournalsByReference(
      @NotNull Long userId, @NotNull ReferenceType referenceType, @NotNull String referenceId) {
    String normalizedReferenceId = referenceId.trim();
    if (normalizedReferenceId.isBlank()) {
      return new AccountReferenceJournalResponse(
          userId, referenceType, normalizedReferenceId, Instant.now(), List.of(), List.of());
    }
    List<UserJournal> accountJournals =
        userJournalRepository.findByReference(referenceType, normalizedReferenceId);
    List<AccountJournalItem> accountItems = buildAccountJournalItems(accountJournals);
    List<PlatformJournal> platformJournals =
        platformJournalRepository.findByReference(referenceType, normalizedReferenceId);
    List<PlatformJournalItem> platformItems = buildPlatformJournalItems(platformJournals);
    Instant snapshotAt = Instant.now();
    return new AccountReferenceJournalResponse(
        userId, referenceType, normalizedReferenceId, snapshotAt, accountItems, platformItems);
  }

  private Comparator<AccountBalanceItem> accountBalanceComparator() {
    return Comparator.comparing(
            (AccountBalanceItem item) ->
                item.accountCode() != null ? item.accountCode().name() : "")
  ...
  private Map<Long, BigDecimal> buildUserSnapshotBalances(
      List<UserAccount> accounts, Instant snapshotAt) {
    ...
    Map<Long, UserJournal> journals =
        userJournalRepository.findLatestBefore(accountIds, snapshotAt);
  ...
  private AccountBalanceItem toAccountBalanceItem(
      UserAccount account, BigDecimal overrideBalance, boolean isHistorical) {
    if (account == null) {
      return null;
    }
  ...
  private Comparator<PlatformAccountItem> platformAccountComparator() {
    return Comparator.comparing(
            (PlatformAccountItem item) ->
                item.accountCode() != null ? item.accountCode().name() : "")
        .thenComparing(item -> item.asset() != null ? item.asset().name() : "")
        .thenComparing(item -> item.category() != null ? item.category().name() : "");
  }

  private Map<Long, BigDecimal> buildPlatformSnapshotBalances(
      List<PlatformAccount> accounts, Instant snapshotAt) {
    Map<Long, BigDecimal> snapshot = new HashMap<>();
    if (snapshotAt == null || accounts.isEmpty()) {
      return snapshot;
    }
    ...
    Map<Long, PlatformJournal> journals =
        platformJournalRepository.findLatestBefore(accountIds, snapshotAt);
  ...
  private PlatformAccountItem toPlatformAccountItem(
      PlatformAccount account, BigDecimal overrideBalance, boolean isHistorical) {
    if (account == null) {
      return null;
    }
  ...
  private List<AccountJournalItem> buildAccountJournalItems(@NotNull List<UserJournal> journals) {
    ...
    List<UserAccount> accounts =
        userIds.isEmpty()
            ? List.of()
            : userAccountRepository.findByUserIds(new ArrayList<>(userIds));
  ...
  public PlatformAccountResponse getPlatformAccounts(Instant snapshotAt) {
    ...
    Instant effectiveSnapshot = isHistorical ? snapshotAt : now;
    Map<Long, BigDecimal> snapshotBalances =
        isHistorical ? buildPlatformSnapshotBalances(accounts, snapshotAt) : Map.of();
    List<PlatformAccountItem> items =
        accounts.stream()
            .map(
                account ->
                    toPlatformAccountItem(
                        account, snapshotBalances.get(account.getAccountId()), isHistorical))
            .filter(Objects::nonNull)
            .sorted(platformAccountComparator())
            .toList();
    Comparator<PlatformAccountItem> comparator = platformAccountComparator();
    Map<AccountCategory, List<PlatformAccountItem>> grouped =
        items.stream()
            .sorted(comparator)
    ...
    return new PlatformAccountResponse(
        effectiveSnapshot, assets, liabilities, equity, expenses, revenue);
  }

  public PlatformAccountJournalResponse getPlatformAccountJournals(
      @NotNull Long accountId, Instant snapshotAt) {
    boolean isHistorical = snapshotAt != null;
    List<PlatformJournal> journals =
        isHistorical
  ...
  private List<PlatformJournalItem> buildPlatformJournalItems(
      @NotNull List<PlatformJournal> journals) {
    if (journals.isEmpty()) {
      return List.of();
    }
    List<Long> accountIds =
        journals.stream()
            ...
            .distinct()
            .toList();
    Map<Long, PlatformAccount> accountMap =
        platformAccountRepository.findByAccountIds(accountIds).stream()
            .filter(Objects::nonNull)
            .filter(item -> item.getAccountId() != null)
            .collect(
                Collectors.toMap(
