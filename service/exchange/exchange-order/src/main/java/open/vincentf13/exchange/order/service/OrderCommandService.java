...
public class OrderCommandService {
      ...
      RetryTaskPO retryTask =
          transactionTemplate.execute(
              status -> {
                ...
                return pendingTaskRepository.insertPendingTask(
                    RetryTaskType.ORDER_PREPARE_INTENT, request.getClientOrderId(), diff);
              });
      Order response =
              ...
              msg ->
                  OpenException.of(
                      ...
                      Map.of(
                          ...
                          msg)));
      Order response =
          processOrderByIntent(
              payload.getOrderId(), payload.getUserId(), payload.getRequest(), intentResponse);
  ...
  public void processFundsFrozen(Long orderId, Instant eventTime) {
    transactionTemplate.executeWithoutResult(
        status -> {
          ...
          Order originalOrder =
              OpenObjectMapper.fromJson(OpenObjectMapper.toJson(order), Order.class);
          ...
          boolean updated =
              orderRepository.updateSelective(
                  order,
  ...
  public void processTradeExecution(
    ...
    transactionTemplate.executeWithoutResult(
        status -> {
          if (orderEventRepository.existsByReference(
              orderId, OrderEventReferenceType.TRADE, String.valueOf(tradeId))) {
            return;
          }
  ...
  private Order processOrderByIntent(
      Long orderId,
      Long userId,
        ...
        status -> {
          ...
          if (intentResponse.rejectReason() != null) {
            return rejectPersistedOrder(
                order, intentResponse.rejectReason(), OrderEventReferenceType.REQUEST);
          }
          // 風控
          ...
          if (intentType == PositionIntentType.INCREASE) {
            orderEventPublisher.publishFundsFreezeRequested(
                new FundsFreezeRequestedEvent(
                    order.getOrderId(),
                    userId,
                    request.getInstrumentId(),
  ...
  private OrderPrecheckResponse precheckOrder(
      Long userId, Order order, PositionIntentType intentType, PositionResponse position) {
  ...
  private OrderResponse rejectOrder(Order order, String reason) {
    String truncatedReason =
        reason != null && reason.length() > MAX_REJECTED_REASON_LENGTH
  ...
  private Order rejectPersistedOrder(
      Order order, String reason, OrderEventReferenceType referenceType) {
    String truncatedReason =
        reason != null && reason.length() > MAX_REJECTED_REASON_LENGTH
            ? reason.substring(0, MAX_REJECTED_REASON_LENGTH)
            : reason;
    ...
    String payload = OpenObjectDiff.diff(originalOrder, order);
    String referenceId =
        order.getClientOrderId() != null ? order.getClientOrderId() + ":reject" : null;
    orderEventRepository.append(
        order,
        OrderEventType.ORDER_REJECTED,
        actorFromUser(order.getUserId()),
