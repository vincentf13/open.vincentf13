...
public class WsHandler extends SimpleChannelInboundHandler<TextWebSocketFrame> {
    ...
    protected void channelRead0(ChannelHandlerContext ctx,
        ...
        if (text.contains(OP_PING)) {
            ctx.channel().writeAndFlush(new TextWebSocketFrame(RESP_PONG));
            return;
        }
    ...
    private void handleAuth(Channel channel,
        ...
        Storage.self().gatewayQueue().acquireAppender().writeDocument(wire -> {
            wire.write("msgType").int32(MSG_TYPE_AUTH);
            wire.write(FIELD_USER_ID).int64(userId);
        });
    }
