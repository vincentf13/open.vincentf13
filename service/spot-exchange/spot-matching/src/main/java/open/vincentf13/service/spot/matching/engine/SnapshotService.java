...
/** 
 ...
 職責：同步備份 Chronicle Map 文件與 OrderBook 內存狀態
 */
@Slf4j
@Service
...
public class SnapshotService {
    ...
    public void executeCreateSnapshot(WalProgress currentProgress) {
        long seq = currentProgress.getLastProcessedMsgSeq();
        log.debug("--- 開始執行同步系統快照 (Sequence: {}) ---", seq);
        ...
        try {
            ...
            log.debug("✅ 同步快照創建成功，存儲路徑: {}", finalDir.getAbsolutePath());
    ...
    public boolean recoverFromLatestSnapshot() {
        ...
        java.util.Arrays.sort(snapshots, (a, b) -> {
            try {
                return Long.compare(Long.parseLong(b.getName()), Long.parseLong(a.getName()));
            } catch (NumberFormatException e) {
                return 0;
            }
        });
