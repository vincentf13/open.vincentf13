...
AI 寫代碼很快，但它難以預判在**分散式高流量場景**下，微妙的時序差異會如何引發**競態條件 (Race Conditions)**，也無法預知**節點故障**時的狀態一致性挑戰。而這些複雜場景若依賴人工測試，不僅費力耗時且難以覆蓋邊界條件，因此我們必須建立一套嚴謹的自動化測試體系。
...
首先是**正常運作階段**：我們注入 1000 筆訂單觸發系統自動生成 Snapshot，接著再追加 2 筆訂單寫入 WAL。為了提高測試深度，我們讓每 10 筆訂單共用一個價格，藉此驗證系統在相同價格下能否嚴格遵守 **FIFO (先進先出)** 的還原順序。同時，我們也設計了包含小數點後 3 位的價格，以及極大的數量邊界值，確保序列化機制在複雜場景下依然保持精準。
