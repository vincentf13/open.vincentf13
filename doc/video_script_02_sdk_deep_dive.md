...
| 1:40 | **[審計與驗證]**<br>顯示 `OpenObjectDiff.diff(old, new)` 的結果 JSON。<br>顯示 `OpenValidator` 的 Group 校驗代碼。     | 為了滿足審計需求，我開發了 **`OpenObjectDiff`** 工具，能高效比對物件差異並生成 Delta JSON，這在記錄操作日誌時極為強大。<br><br>此外，透過 `OpenValidator` 的 Group 分組設計，我們能在 CRUD 不同階段，對 Domain、DTO 與持久化物件執行精確且標準化的參數校驗。                                                      |      |
