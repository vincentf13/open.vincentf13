...
#### 4.1 模組分層與依賴 (Layered Dependencies)

**設計哲學：** 職責單一，由下而上構建安全基座。

| 時間   | 畫面 (Visual)                                                                                                            | 旁白腳本 (Audio)                                                                                                                                                                                                                         | 執行建議 |
| :--- | :----------------------------------------------------------------------------------------------------------------------- | :----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :--- |
| 4:25 | **[Auth 依賴金字塔]**<br>底層：`sdk-auth` (API/Annotation)。<br>中層：`sdk-auth-jwt` (Validation/Filter)。<br>頂層：`sdk-auth-server` (Issuance/Controller)。 | 我們的安全模組採用了嚴謹的分層設計。最底層的 `sdk-auth` 定義了通用的權限註解與上下文接口；中層的 `sdk-auth-jwt` 封裝了 JWT 解析與驗證邏輯，供 Gateway 與微服務使用；而頂層的 `sdk-auth-server` 則負責 Token 的簽發與生命週期管理。這種層次分明的依賴關係，確保了每個模組只關注自己的核心職責。 |      |

#### 4.2 無狀態架構與自動裝配 (Stateless & Auto-Config)

**設計哲學：** 簽發與驗證分離，減少單點瓶頸。

| 時間   | 畫面 (Visual)                                                                                                              | 旁白腳本 (Audio)                                                                                                                                                                                                                                               | 執行建議 |
| :--- | :------------------------------------------------------------------------------------------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :--- |
| 4:40 | **[Auth 架構圖與自動裝配]**<br>圖示：`Auth Server` (發證) -> 用戶 -> `Gateway` (驗證)。<br>顯示 `sdk-auth-server` 內建的 Controller 代碼。 | 在安全架構上，我們採用了標準的 OAuth2 輕量化設計。透過 **Spring Boot 自動裝配**，任何服務只需引入 `sdk-auth-server`，就能自動獲得標準的 `/login`, `/logout` 與 `/refresh` 接口。這意味著所有業務系統的認證入口都是統一治理的，開發者無需重複編寫登入邏輯，且確保了私鑰簽發邏輯被嚴格隔離在 Auth 服務內部，而 Gateway 與各微服務則透過公鑰進行高效的無狀態驗證，避免身份驗證變成單點瓶頸。 |      |

#### 4.3 可撤銷的 JWT 與混合模式 (Revocable JWT & Hybrid Mode)

**設計哲學：** 解決 JWT 無法撤銷的痛點，實現即時踢人下線。

| 時間   | 畫面 (Visual)                                                                                                                  | 旁白腳本 (Audio)                                                                                                                                                                                                                                       | 執行建議 |
| :--- | :------------------------------------------------------------------------------------------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :--- |
| 4:55 | **[Redis Session Check]**<br>顯示 `JwtFilter` 中的 `checkSessionActive` 邏輯。<br>動畫：用戶 Token 有效，但 Redis 中 Session 狀態為 Invalid -> 拒絕請求。 | 純粹的 JWT 雖然高效，但最大的缺點是無法即時撤銷。為此，我們在 `sdk-auth` 中引入了「混合模式」。在關鍵的金融操作中，Filter 會額外檢查 Redis 中的 Session 狀態。這讓我們既享受了 JWT 的無狀態便利，又保留了在發現風險時能立即將用戶「踢下線」的控制權。                                                                |      |

#### 4.4 內部傳遞與開發體驗 (Internal Propagation & DX)

**設計哲學：** 安全對業務開發者透明，像寫本地代碼一樣獲取用戶資訊。

| 時間   | 畫面 (Visual)                                                                                                        | 旁白腳本 (Audio)                                                                                                                                                                                               | 執行建議 |
| :--- | :------------------------------------------------------------------------------------------------------------------ | :--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :--- |
| 5:10 | **[ThreadLocal 與註解]**<br>顯示 `UserContext.getUserId()` 的調用。<br>顯示 `@PrivateAPI` 與 `@PublicAPI` 註解在 Controller 上。 | 對於內部服務調用，SDK 利用 **ThreadLocal** 實現了用戶上下文的無感傳遞。開發者在任何一層代碼中，只需調用 `UserContext.get()` 即可獲取當前用戶資訊。同時，我們提供了 `@PrivateAPI` 與 `@PublicAPI` 註解，讓接口權限控制變得像宣告變數一樣簡單直觀。                                       |      |
