...
| 時間 | 畫面 (Visual) | 旁白腳本 (Audio) | 執行建議 |
| :--- | :--- | :--- | :--- |
| 3:10 | **[IDE 專案結構特寫]**<br>展開 `sdk` 目錄，快速掃過：<br>- `sdk-infra-kafka`<br>- `sdk-core`<br>- `sdk-auth`<br>背景淡出架構圖，聚焦代碼。 | 極致的效能往往伴隨著極高的複雜度。為了駕馭這套架構，我開發了一套標準化的 SDK，封裝了所有的底層細節。 | |
| 3:25 | **[程式碼展示：Spring Boot Starter]**<br>左側：業務邏輯代碼，乾淨簡單。<br>右側：SDK 內的 `KafkaAutoConfiguration`，顯示複雜的序列化、批量發送、冪等性檢查邏輯。 | 透過 Spring Boot Starter 機制，開發者只需要引入依賴，就能獲得經過調優的 Kafka 配置、Redis 連接池以及分佈式鎖實現。這確保了每一行業務代碼，都運行在最優的基礎設施之上。 | |
| 3:45 | **[視覺化效益：全鏈路追蹤]**<br>畫面顯示 Log：`[TraceId: xxxxx]`。<br>切換到 Zipkin/Grafana，顯示一條橫跨 Gateway -> Matching -> Risk 的完整調用鏈，時間軸精確到微秒。 | 同時，SDK 內建了全鏈路可觀測性。從用戶下單的那一刻起，唯一的 Trace ID 就貫穿了整個分佈式系統，讓我們能精確追蹤每一筆資金在微秒間的流動軌跡。 | |
...
| 時間 | 畫面 (Visual) | 旁白腳本 (Audio) | 執行建議 |
| :--- | :--- | :--- | :--- |
| 4:05 | **[回到全景架構圖 + 關鍵字]**<br>文字依序浮現：<br>1. **CQRS & LMAX**<br>2. **Flip Protocol**<br>3. **Multi-Level Caching** | Open Exchange Core 的架構哲學很簡單：透過 CQRS 分離讀寫，透過 Flip 協議確保一致性，透過多級緩存極大化吞吐。這是一個為速度與準確性而生的金融核心。 | |
...
1.  **Flip 協議圖解**：
    *   這是一個抽象概念，建議用三個方塊表示：`Account A`, `Account B`, `Transaction Manager`。
    *   動畫顯示：A 扣款 (Pending) -> B 加款 (Pending) -> 全部 Confirm (Solid)。
    *   或者顯示衝突：A 同時被兩個 Tx 扣款 -> 其中一個失敗回滾 (Red X)。
2.  **時長控制**：
    *   腳本現在約 4 分鐘，這是黃金長度。錄音時語速要穩，不要趕。